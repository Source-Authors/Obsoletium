...
void FourVectors::RotateManyBy(FourVectors * RESTRICT pVectors, unsigned int numVectors, const matrix3x4_t& rotationMatrix )
{
...

	{
		...
		fltx4 matCol0 = DirectX::XMLoadFloat4(rotationMatrix.XmBase());
		fltx4 matCol1 = DirectX::XMLoadFloat4(rotationMatrix.XmBase() + 1);
		fltx4 matCol2 = DirectX::XMLoadFloat4(rotationMatrix.XmBase() + 2);
	...
	while (pVectors < stop)
	{
		outX0 = MaddSIMD( pVectors->z, matSplat02, MaddSIMD( pVectors->x, matSplat00, MulSIMD( pVectors->y, matSplat01 ) ) );
		outY0 = MaddSIMD( pVectors->z, matSplat12, MaddSIMD( pVectors->x, matSplat10, MulSIMD( pVectors->y, matSplat11 ) ) );
		outZ0 = MaddSIMD( pVectors->z, matSplat22, MaddSIMD( pVectors->x, matSplat20, MulSIMD( pVectors->y, matSplat21 ) ) );
