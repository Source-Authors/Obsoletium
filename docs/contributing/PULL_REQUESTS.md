...
Obsoletium has several bundled dependencies in the _thirdparty/_, _external/_ and _ivp/_ directories that are not part of the project.
Changes to files in those directories should be sent
to their respective projects.
Do not send a patch to Obsoletium. We cannot accept such patches.

In case of doubt, open an issue in the
[issue tracker](https://github.com/Source-Authors/Obsoletium/issues/).
...
Fork the project [on GitHub](https://github.com/Source-Authors/Obsoletium) and clone your fork
locally.

```bash
git clone git@github.com:username/Obsoletium.git
cd Obsoletium
...
```bash
git config user.name "J. Smith User"
git config user.email "j.smith.user@example.com"
```
...
Add GPG key to Git for signing your commits.
See [Adding a GPG key to your GitHub account][] for details.
...
Pull requests in Obsoletium typically involve changes to
one or more of a few places in the repository.

* C/C++ code contained in the root of the repository
* Documentation in `docs`
* Tests within the `tests` directory

For contributing C++ code, you may want to look at the
[C++ Style Guide](CPP_STYLE_GUIDE.md), as well as the
[Architecture Internals](ARCHITECTURE_INTERNALS.md) for an overview of Obsoletium internals.
...
1. The first line should:
   ...
   * `datacache: add caching of preloaded models`
   * `launcher: default to en_US.utf8 locale`
...
4. If your patch fixes an open issue, you can add a reference to it at the end
   ...
   * `Fixes: https://github.com/Source-Authors/Obsoletium/issues/128`
   * `Refs: https://github.com/Source-Authors/Obsoletium/pull/113`
...
If you are new to contributing to Obsoletium, please try to do your best at
...
This ensures that your working branch has the latest changes from `Source-Authors/Obsoletium`.

### Step 6: Test

Bug fixes and features should always come with tests. A
[guide for writing tests in Obsoletium][] has been
provided to make the process easier. Looking at other tests to see how they
should be structured can also help.

The `tests` directory within the `Source-Authors/Obsoletium` repository is complex and it is
often not clear where a new test file should go. When in doubt, add new tests
to the `tests/` directory and the right location will be sorted out
later.

Before submitting your changes in a pull request, always run the full Obsoletium
test suite.
...
at least one Obsoletium Collaborator and pass a
...
All Obsoletium contributors who choose to review and provide feedback on Pull
...
1. Does this change make sense for Obsoletium?
2. Does this change make Obsoletium better, even if only incrementally?
...
Obsoletium collaborator landing the pull request but they can also be an
...
a particular change that makes Obsoletium better, but the individual might just
not want to have anything to do with Obsoletium ever again. The goal is not just
having good code.
...
Any Obsoletium core collaborator (any GitHub user with commit rights in the
`Source-Authors/Obsoletium` repository) is authorized to approve any other contributor's
...
Obsoletium has always optimized for speed of execution. If a particular change
can be shown to make some part of Obsoletium faster, it's quite likely to be
accepted. Claims that a particular pull request will make things faster will
almost always be met by requests for performance that demonstrate the improvement.

That said, performance is not the only factor to consider. Obsoletium also
...
All pull requests that contain changes to code run through
continuous integration (CI) testing at [Obsoletium Actions][].

Ideally, the code change will pass ("be green") on all platform configurations
supported by Obsoletium. This means that all tests pass and there are no linting
...
[771eefc](https://github.com/Source-Authors/Obsoletium/commit/771eefc402872cad143c5ceef8297abc41a4dd07)
can be a good example. All tests should always pass
when each individual commit lands on one of the `Source-Authors/Obsoletium` branches.
...
[the official documentation](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/reviewing-changes-in-pull-requests).
...
longer to be merged in. All these precautions are important because Obsoletium is
...
[Obsoletium Actions]: https://github.com/Source-Authors/Obsoletium/actions
...
[pull request template]: https://raw.githubusercontent.com/Source-Authors/Obsoletium/HEAD/.github/PULL_REQUEST_TEMPLATE.md
