...
CQC_EyesDlg::CQC_EyesDlg(CWnd *pParent /*=NULL*/)
    : CBaseDlg(CQC_EyesDlg::IDD, pParent) {
...
CQC_EyesDlg::~CQC_EyesDlg()
{
  for (CBitmapRef *pCur = m_pBitmapHead; pCur; pCur = pCur->m_pNext) {
    DeleteObject(pCur->m_hBitmap);
    pCur->m_hBitmap = nullptr;
  }
}
...
BEGIN_MESSAGE_MAP(CQC_EyesDlg, CBaseDlg)
...
void CQC_EyesDlg::AddText(PRINTF_FORMAT_STRING const char *pFormat, ...) {
  char tempMsg[4096];
  va_list marker;
...
HBITMAP CQC_EyesDlg::GetCachedBitmap(UINT id) {
  for (CBitmapRef *pCur = m_pBitmapHead; pCur; pCur = pCur->m_pNext) {
    if (pCur->m_iResource == id) return pCur->m_hBitmap;
  }

  CBitmapRef *pNew = new CBitmapRef;
  pNew->m_iResource = id;
  pNew->m_hBitmap = se::windows::ui::ScaleBitmapForDpi
  (
      ::LoadBitmap(AfxGetInstanceHandle(), MAKEINTRESOURCE(id)),
...
BOOL CQC_EyesDlg::OnInitDialog() {
  ...
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_Y_AXIS_UP), BM_SETCHECK, BST_CHECKED,
                0);
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_DEFAULT_CONTROLS), BM_SETCHECK,
                BST_CHECKED, 0);
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_IRIS_COLOR_BROWN), BM_SETCHECK,
                BST_CHECKED, 0);
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_EYE_COLOR_LIGHT), BM_SETCHECK,
                BST_CHECKED, 0);
...
HCURSOR CQC_EyesDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
bool CQC_EyesDlg::IsOptionChecked(UINT option) {
  return (::SendMessage(::GetDlgItem(m_hWnd, option), BM_GETCHECK, 0, 0) ==
          BST_CHECKED);
}
...
void CQC_EyesDlg::GetDialogParams(CDialogParams &p) {
  ...
  GetDlgItemText(IDC_REFERENCE_FILENAME, p.m_ReferenceFilename,
                 sizeof(p.m_ReferenceFilename));
  GetDlgItemText(IDC_EXPRESSIONS_FILENAME, p.m_ExpressionsFilename,
                 sizeof(p.m_ExpressionsFilename));
  GetDlgItemText(IDC_MODEL_FILENAME, p.m_ModelFilename,
                 sizeof(p.m_ModelFilename));
  ...
  if (IsOptionChecked(IDC_EYE_COLOR_LIGHT))
    V_strcpy_safe(p.m_EyeballPrefix, "eyeball");
  else
    V_strcpy_safe(p.m_EyeballPrefix, "dark_eyeball");

  // Figure out the pupil prefix.
  if (IsOptionChecked(IDC_IRIS_COLOR_BROWN))
    V_strcpy_safe(p.m_PupilPrefix, "pupil");
  else if (IsOptionChecked(IDC_IRIS_COLOR_GREEN))
    V_strcpy_safe(p.m_PupilPrefix, "grn_pupil");
  else
    V_strcpy_safe(p.m_PupilPrefix, "bl_pupil");
}
...
void CQC_EyesDlg::GenerateQCText() {
  ...
  AddText(
      "$attachment \"eyes\" \"ValveBiped.Bip01_Head1\" %.2f %.2f %.2f "
      "absolute\n",
  ...
  AddText(
      "$attachment \"mouth\" \"ValveBiped.Bip01_Head1\" 0.80 -5.80 -0.15 "
      "rotate 0 -80 -90\n\n");

  AddText("$model %s \"%s.smd\" {\n", p.m_ModelFilename, p.m_ReferenceFilename);

  AddText(
      "\teyeball righteye \"ValveBiped.Bip01_Head1\" %.2f %.2f %.2f \"%s_r\" "
      "%.2f 4 \"%s_r\" %.2f\n",
      p.m_flRightEye[0], p.m_flRightEye[1], p.m_flRightEye[2],
      p.m_EyeballPrefix, p.m_flEyeballSize, p.m_PupilPrefix, p.m_flIrisSize);

  AddText(
      "\teyeball lefteye \"ValveBiped.Bip01_Head1\" %.2f %.2f %.2f \"%s_l\" "
      "%.2f -4 \"%s_l\" %.2f\n\n",
      p.m_flLeftEye[0], p.m_flLeftEye[1], p.m_flLeftEye[2], p.m_EyeballPrefix,
      p.m_flEyeballSize, p.m_PupilPrefix, p.m_flIrisSize);

  AddText(
      "\teyelid  upper_right \"%s\" lowerer 1 %.2f neutral 0 %.2f raiser 2 "
      "%.2f split 0.1 eyeball righteye\n",
      p.m_ExpressionsFilename, p.m_flRightUpperLidLowered - p.m_flRightEye[2],
      p.m_flRightUpperLidNeutral - p.m_flRightEye[2],
      p.m_flRightUpperLidRaised - p.m_flRightEye[2]);

  AddText(
      "\teyelid  lower_right \"%s\" lowerer 3 %.2f neutral 0 %.2f raiser 4 "
      "%.2f split 0.1 eyeball righteye\n",
      p.m_ExpressionsFilename, p.m_flRightLowerLidLowered - p.m_flRightEye[2],
      p.m_flRightLowerLidNeutral - p.m_flRightEye[2],
      p.m_flRightLowerLidRaised - p.m_flRightEye[2]);

  AddText(
      "\teyelid  upper_left \"%s\" lowerer 1 %.2f neutral 0 %.2f raiser 2 %.2f "
      "split -0.1 eyeball lefteye\n",
      p.m_ExpressionsFilename, p.m_flLeftUpperLidLowered - p.m_flLeftEye[2],
      p.m_flLeftUpperLidNeutral - p.m_flLeftEye[2],
      p.m_flLeftUpperLidRaised - p.m_flLeftEye[2]);

  AddText(
      "\teyelid  lower_left \"%s\" lowerer 3 %.2f neutral 0 %.2f raiser 4 %.2f "
      "split -0.1 eyeball lefteye\n\n",
      p.m_ExpressionsFilename, p.m_flLeftLowerLidLowered - p.m_flLeftEye[2],
...
bool CQC_EyesDlg::CheckNumericInputs() {
  struct {
    ...
    UINT controlID;
  } controls[] = {{"Right Eye X", IDC_RIGHT_EYE_X},
  ...
  for (auto &c : controls) {
    char text[512];
    GetDlgItem(c.controlID)->GetWindowText(text, sizeof(text));

    for (intp z = 0; z < V_strlen(text); z++) {
      if (text[z] < '0' || text[z] > '9') {
        if (text[z] != '.' && text[z] != '-') {
          char errMsg[512];
          V_sprintf_safe(errMsg, "The '%s' control must have a numeric value.",
                         c.pControlName);
...
void CQC_EyesDlg::OnIrisColorBrown() {
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_IRIS_COLOR_BROWN), BM_SETCHECK,
                BST_CHECKED, 0);
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_IRIS_COLOR_GREEN), BM_SETCHECK,
                BST_UNCHECKED, 0);
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_IRIS_COLOR_BLUE), BM_SETCHECK,
                BST_UNCHECKED, 0);
...
void CQC_EyesDlg::OnEyeColorDark() {
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_EYE_COLOR_LIGHT), BM_SETCHECK,
                BST_UNCHECKED, 0);
  ::SendMessage(::GetDlgItem(m_hWnd, IDC_EYE_COLOR_DARK), BM_SETCHECK,
                BST_CHECKED, 0);
  SetupBitmapLabel(IDB_EYE_DEFAULT, "");
}
...
void CQC_EyesDlg::OnSetfocusRightEyeX() {
  SetupBitmapLabel(
      IDB_EYE_XY_R,
      "Enter the X position of the center vertex of the right eye");
}

void CQC_EyesDlg::OnSetfocusRightEyeY() {
  SetupBitmapLabel(
      IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_XY_R : IDB_EYE_Z_R,
      "Enter the Y position of the center vertex of the right eye");
}

void CQC_EyesDlg::OnSetfocusRightEyeZ() {
  SetupBitmapLabel(
      IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_Z_R : IDB_EYE_XY_R,
      "Enter the Z position of the center vertex of the right eye");
}
...
void CQC_EyesDlg::OnSetfocusLeftEyeY() {
  SetupBitmapLabel(
      IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_XY_L : IDB_EYE_Z_L,
      "Enter the Y position of the center vertex of the right eye");
}

void CQC_EyesDlg::OnSetfocusLeftEyeZ() {
  SetupBitmapLabel(
      IsOptionChecked(IDC_Y_AXIS_UP) ? IDB_EYE_Z_L : IDB_EYE_XY_L,
      "Enter the Z position of the center vertex of the right eye");
}

void CQC_EyesDlg::OnSetfocusUpperLidLowered() {
  const char *pCoord = IsOptionChecked(IDC_Y_AXIS_UP) ? "Y" : "Z";
  SetupBitmapLabel(IDB_EYE_UPPER_LO,
                   "At Frame 1, enter the %s position of the center vertex of "
...
void CQC_EyesDlg::OnSetfocusUpperLidNeutral() {
  const char *pCoord = IsOptionChecked(IDC_Y_AXIS_UP) ? "Y" : "Z";
  SetupBitmapLabel(IDB_EYE_UPPER_MID,
                   "At Frame 0, enter the %s position of the center vertex of "
                   "the right upper eye lid",
                   pCoord);
}
...
void CQC_EyesDlg::OnSetfocusUpperLidRaised() {
  const char *pCoord = IsOptionChecked(IDC_Y_AXIS_UP) ? "Y" : "Z";
  SetupBitmapLabel(IDB_EYE_UPPER_HI,
                   "At Frame 2, enter the %s position of the center vertex of "
                   "the right upper eye lid",
                   pCoord);
}
...
void CQC_EyesDlg::OnSetfocusLowerLidLowered() {
  const char *pCoord = IsOptionChecked(IDC_Y_AXIS_UP) ? "Y" : "Z";
  SetupBitmapLabel(IDB_EYE_LOWER_LO,
                   "At Frame 3, enter the %s position of the center vertex of "
...
void CQC_EyesDlg::OnSetfocusLowerLidNeutral() {
  const char *pCoord = IsOptionChecked(IDC_Y_AXIS_UP) ? "Y" : "Z";
  SetupBitmapLabel(IDB_EYE_LOWER_MID,
                   "At Frame 0, enter the %s position of the center vertex of "
                   "the right lower eye lid",
                   pCoord);
}
...
void CQC_EyesDlg::OnSetfocusLowerLidRaised() {
  const char *pCoord = IsOptionChecked(IDC_Y_AXIS_UP) ? "Y" : "Z";
  SetupBitmapLabel(IDB_EYE_LOWER_HI,
                   "At Frame 4, enter the %s position of the center vertex of "
                   "the right lower eye lid",
                   pCoord);
}
...
int g_AdvancedControls[] = {IDC_EYE_DETAIL_CONTROL_FRAME, IDC_IRIS_SIZE,
                            IDC_IRIS_SIZE_LABEL,          IDC_EYEBALL_SIZE,
                            IDC_EYEBALL_SIZE_LABEL,       IDC_LEFT_LID_CONTROL};

int g_LeftLidPositionControls[] = {
    IDC_UPPER_LEFT_LID_PANEL,         IDC_UPPER_LEFT_LID_RAISED,
    IDC_UPPER_LEFT_LID_RAISED_LABEL,  IDC_UPPER_LEFT_LID_NEUTRAL,
    IDC_UPPER_LEFT_LID_NEUTRAL_LABEL, IDC_UPPER_LEFT_LID_LOWERED,
    IDC_UPPER_LEFT_LID_LOWERED_LABEL, IDC_LOWER_LEFT_LID_PANEL,
    IDC_LOWER_LEFT_LID_RAISED,        IDC_LOWER_LEFT_LID_RAISED_LABEL,
    IDC_LOWER_LEFT_LID_NEUTRAL,       IDC_LOWER_LEFT_LID_NEUTRAL_LABEL,
    IDC_LOWER_LEFT_LID_LOWERED,       IDC_LOWER_LEFT_LID_LOWERED_LABEL};

void CQC_EyesDlg::OnDefaultControls() {
  GetDlgItem(IDC_PICTURES)->ShowWindow(SW_SHOW);

  // Hide all the advanced controls.
  for (auto &c : g_AdvancedControls) {
    GetDlgItem(c)->ShowWindow(SW_HIDE);
  }

  for (auto &c : g_LeftLidPositionControls) {
    GetDlgItem(c)->ShowWindow(SW_HIDE);
  }
}
...
void CQC_EyesDlg::OnAdvancedControls() {
  ...
  for (auto &c : g_AdvancedControls) {
    GetDlgItem(c)->ShowWindow(SW_SHOW);
    GetDlgItem(c)->InvalidateRect(NULL);
  }
