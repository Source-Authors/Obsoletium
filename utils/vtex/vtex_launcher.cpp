...
int main(int argc, char *argv[]) {
  ...
  constexpr char kRedirectFileName[]{"vtex.redirect"};
  constexpr char kModuleName[]{"vtex_dll" DLL_EXT_STRING};

  char full_path[MAX_FILEPATH], redirect_path[MAX_FILEPATH];
  std::error_code err{MakeFullPath(argv[0], full_path)};
  if (err) {
    fprintf(stderr, "vtex err: Can't get full path to '%s'.\n", argv[0]);
    return err.value();
  }
  ...
  // First, look for vtex.redirect and load the dll specified in there if
  // possible.
  if (auto [f, err2] =
    ...
    if (std::tie(name, err2) = f.gets(module_name); !err2) {
      ...
      if (!module) {
        fprintf(stderr, "vtex warn: Can't find '%s' specified in '%s'.\n", name,
                kRedirectFileName);
        return module.error_code().value();
      } else {
        printf("vtex: Loaded alternate vtex module '%s' specified in '%s'.\n",
  ...
  if (!factory) {
    fprintf(stderr, "vtex error: Can't get '%s' factory from '%s'.\n",
  ...
  ILaunchableDLL *vtex{factory(kInterfaceName, &ret)};
  if (!vtex) {
    fprintf(stderr, "vtex error: Can't get '%s' interface from '%s'.\n",
            kModuleName, kInterfaceName);
    return EINVAL;
  }

  return vtex->main(argc, argv);
}
