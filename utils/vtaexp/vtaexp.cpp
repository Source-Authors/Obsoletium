...
DESTRUCTOR VtaExportClass::~VtaExportClass(void) {
  if (m_rgmaxnode) delete[] m_rgmaxnode;
}

int VtaExportClass::DoExport(const TCHAR *name, ExpInterface *ei, Interface *i,
  ...
  FILE *pFile;
  if ((pFile = fopen(szFile, "w")) == NULL) return FALSE /*failure*/;
  ...
  if (!CollectNodes(pexpiface)) return 0; /*fail*/
  ...
  // Output bone rotations, for each frame. Do only first frame if this is the
  // reference frame MAX file
  DumpRotations(pFile, pexpiface);

  // Output triangle meshes (first frame/all frames), if this is the reference
  // frame MAX file
...
BOOL VtaExportClass::DumpRotations(FILE *pFile, ExpInterface *pexpiface) {
  ...
  // Also dumps root-node translation info (the model's world-position at each
  // frame)
...
int CollectNodesTEP::callback(INode *node) {
  ...
  m_phec->m_rgmaxnode[iNode].imaxnodeParent =
      VtaExportClass::UNDESIRABLE_NODE_MARKER;
...
int DumpNodesTEP::callback(INode *pnode) {
  ...
  int iNode = ::GetIndexOfINode(pnode);
  int iNodeParent =
      ::GetIndexOfINode(pnodeParent, !fNodeIsRoot /*fAssertPropExists*/);

  // Convenient time to cache this
  m_phec->m_rgmaxnode[iNode].imaxnodeParent =
      fNodeIsRoot ? VtaExportClass::UNDESIRABLE_NODE_MARKER : iNodeParent;

  // Root node has no parent, thus no translation
  if (fNodeIsRoot) iNodeParent = -1;
...
int DumpFrameRotationsTEP::callback(INode *pnode) {
  ...
  // Get the rotation (via decomposition into "affine parts", then
  // quaternion-to-Euler) Apparently the order of rotations returned by
  // QuatToEuler() is X, then Y, then Z.
...
int DumpModelTEP::callback(INode *pnode) {
  ...
  if (!pnode->Selected()) return TREE_CONTINUE;

  int iNode = ::GetIndexOfINode(pnode);
  TSTR strNodeName(pnode->GetName());

  // The Footsteps node apparently MUST have a dummy mesh attached!  Ignore it
  // explicitly.
  if (FStrEq((char *)strNodeName, "Bip01 Footsteps")) return TREE_CONTINUE;

  // Helper nodes don't have meshes
  Object *pobj = pnode->GetObjectRef();
  if (pobj->SuperClassID() == HELPER_CLASS_ID) return TREE_CONTINUE;
  ...
  // Get Node's object, convert to a triangle-mesh object, so I can access the
  // Faces
  ...
  if (pobj->SuperClassID() == HELPER_CLASS_ID) {
    sprintf(st_szDBG,
            "ERROR--Helper node %s has an attached mesh, and it shouldn't.",
            (char *)strNodeName);
    ASSERT_AND_ABORT(FALSE, st_szDBG);
  }

  // convert mesh to triobject
  if (!pobj->CanConvertToType(triObjectClassID)) return TREE_CONTINUE;
  TriObject *ptriobj =
      (TriObject *)pobj->ConvertToType(m_tvToDump, triObjectClassID);
  ...
  if (m_tvToDump == m_tvBase) {
    if (m_baseVert)
      m_baseVert = (Point3 *)realloc(
          m_baseVert, (m_baseVertCount + cVerts) * sizeof(Point3));
    else
      m_baseVert =
          (Point3 *)malloc((m_baseVertCount + cVerts) * sizeof(Point3));
    ...
    for (int iVert = 0; iVert < cVerts; iVert++) {
      ...
      fprintf(m_pfile, "%5d %8.4f %8.4f %8.4f  %9.6f %9.6f %9.6f\n", iAdjVert,
              m_baseVert[iAdjVert].x, m_baseVert[iAdjVert].y,
              m_baseVert[iAdjVert].z, pt3Normal1.x, pt3Normal1.y, pt3Normal1.z);
    }
  } else {
    for (int iVert = 0; iVert < cVerts; iVert++) {
      Point3 pt3Vertex1 = pmesh->getVert(iVert);
      ...
      if (Length(m_baseVert[iAdjVert] - v1) > 0.01) {
        ...
        fprintf(m_pfile, "%5d %8.4f %8.4f %8.4f  %9.6f %9.6f %9.6f\n", iAdjVert,
                v1.x, v1.y, v1.z, pt3Normal1.x, pt3Normal1.y, pt3Normal1.z);
      }
    }
  }
...
Point3 DumpModelTEP::Pt3GetRVertexNormal(RVertex *prvertex, DWORD smGroupFace) {
  ...
  if (cNormals == 1)
    ...
    for (int irn = 0; irn < cNormals; irn++)
      if (prvertex->ern[irn].getSmGroup() & smGroupFace) break;
...
int GetIndexOfINode(INode *pnode, BOOL fAssertPropExists) {
  ...
  if (fAssertPropExists) ASSERT_MBOX(FALSE, "No NODEINDEXSTR property");
  return -7777;
}
...
void SetIndexOfINode(INode *pnode, int inode) {
  ...
  NAMEMAP *pnm;
  for (int inm = 0; inm < g_inmMac; inm++)
    if (FStrEq(g_rgnm[inm].szNodeName, (char *)strNodeName)) break;
  if (inm < g_inmMac)
    pnm = &g_rgnm[inm];
  else {
...
BOOL FUndesirableNode(INode *pnode) {
  ...
  if (pobj->SuperClassID() == CAMERA_CLASS_ID) return TRUE;
  if (pobj->SuperClassID() == LIGHT_CLASS_ID) return TRUE;
