...
//
// This number can be changed on the command line with the -mpi_FileTransmitRate
// parameter.
int MULTICAST_TRANSMIT_RATE = (1024 * 1000);  // N megs per second

// Defines when we'll stop transmitting a file to a client.
// (After we've transmitted the file to the client N times and we haven't heard
// an ack back for M seconds).
...
static void SendMulticastIP(const IpV4 *pAddr) {
  unsigned char packetID[2] = {VMPI_PACKETID_FILESYSTEM,
                               VMPI_FSPACKETID_MULTICAST_ADDR};
  VMPI_Send2Chunks(packetID, sizeof(packetID), pAddr, sizeof(*pAddr),
                   VMPI_PERSISTENT);
}

static bool IsOpeningForWriteAccess(const char *pOptions) {
  return strchr(pOptions, 'w') || strchr(pOptions, 'a') ||
         strchr(pOptions, '+');
}
...
class CVMPIFile_PassThru : public IVMPIFile {
 public:
  ...
  virtual void Flush() { m_pPassThru->Flush(m_fp); }
...
// ----------------------------------------------------------------------------------------------------
// // CTransmitRateMgr coordinates with any other currently-running VMPI jobs,
// and they all will cut down their transmission rate to stay within
// MULTICAST_TRANSMIT_RATE.
// ----------------------------------------------------------------------------------------------------
// //

#define TRANSMITRATEMGR_BROADCAST_INVERVAL \
  (1.0 / 3.0)  // How many times per second we broadcast our presence.
#define TRANSMITRATEMGR_EXPIRE_TIME \
  0.7  // How long it'll go before deciding a guy is not transmitting anymore.
...
void CTransmitRateMgr::ReadPackets() {
  ...
  while (1) {
    ...
    if (len == sizeof(s_cTransmitRateMgrPacket) + sizeof(unsigned long) &&
      ...
      if (i == m_MachineRecords.Count()) {
        intp index = m_MachineRecords.AddToTail();
  ...
  for (intp i = 0; i < m_MachineRecords.Count(); i++) {
    if ((flCurTime - m_MachineRecords[i].m_flLastTime) >
        TRANSMITRATEMGR_EXPIRE_TIME) {
      m_MachineRecords.Remove(i);
      --i;
    }
  }

  // Recalculate our transmit rate (assuming we're receiving our own broadcast
  // packets).
  m_nMicrosecondsPerByte = 1000000.0 / (double)(MULTICAST_TRANSMIT_RATE /
                                                (m_MachineRecords.Count() + 1));
}
...
// ----------------------------------------------------------------------------------------------------
// // CRateLimiter manages waiting for small periods of time between packets so
// the rate is whatever we want it to be.
//
...
class CRateLimiter {
 ...
 public:
  ...
  // up the transmit rate until we've accounted for the time lost during the
  // sleep.
...
void CRateLimiter::GiveUpTimeSlice() {
  ...
  if (dtSinceLastSleep.GetMilliseconds() >= m_SleepIntervalMS) {
    ...
    m_AccumulatedSleepMicroseconds +=
        sleepTimer.GetDuration().GetMicroseconds();
    m_LastSleepTime.Sample();
  }
}
...
void CRateLimiter::NoteExcessTimeTaken(unsigned long excessTimeInMicroseconds) {
  ...
  if (excessTimeInMicroseconds > m_AccumulatedSleepMicroseconds) {
    ...
    while (1) {
      ...
      if (diff.GetMicroseconds() >= excessTimeInMicroseconds) break;
    }
  } else {
...
class CMasterMulticastThread {
 public:
  ...
  // This creates the socket and starts the thread (initially in an idle state
  // since it doesn't know of any files anyone wants).
  bool Init(IBaseFileSystem *pPassThru, unsigned short localPort,
            const IpV4 *pAddr, int maxFileSystemMemoryUsage);
  void Term();
  ...
  int AddFileRequest(const char *pFilename, const char *pPathID, int clientID,
                     bool *bZeroLength);
  ...
  // Call this if a client disconnects so it can stop sending anything this
  // client wants.
  void OnClientDisconnect(int clientID, bool bGrabCriticalSection = true);

  void CreateVirtualFile(const char *pFilename, const void *pData,
 ...
 private:
  class CChunkInfo {
   public:
    ...
    int m_iActiveChunksIndex;   // Index into m_ActiveChunks.
  };

  // This stores a client's reference to a file so it knows which pieces of the
  // file the client needs.
  class CClientFileInfo {
   ...
   public:
    ...
    CUtlVector<unsigned char>
        m_ChunksToSend;  // One bit for each chunk that this client still wants.
    int m_nChunksLeft;
    ...
    double m_flTransmitStartTime;

    double m_flLastAckTime;  // Last time we heard an ack back from this client
                             // about this file. If this goes for too long, then
                             // we assume that the client is in a screwed state,
                             // and we stop sending the file to him.
    int m_nTimesFileCycled;  // How many times has the master multicast thread
                             // cycled over this file? We won't kick the client
                             // until we've cycled over the file a few times
                             // after the client asked for it.
  };

  class CMulticastFile {
   public:
    ~CMulticastFile() { m_Clients.PurgeAndDeleteElements(); }

    const char *GetFilename() { return m_Filename.Base(); }
    const char *GetPathID() { return m_PathID.Base(); }

   public:
    int m_nCycles;  // How many times has the multicast thread visited this
                    // file?

    // This is sent along with every packet. If a client gets a chunk and
    // doesn't have that file's info, the client will receive that file too.
    ...
    // This gets set to false if we run over our memory limit and start caching
    // file data out. Then it'll reload the data if a client requests the file.
    bool m_bDataLoaded;

    // m_Chunks holds the chunks by index.
    // m_ActiveChunks holds them sorted by whether they're active or not.
    //
    // Each chunk has a refcount. While the refcount is > 0, the chunk is in the
    // first half of m_ActiveChunks. When the refcount gets to 0, the chunk is
    // moved to the end of m_ActiveChunks. That way, we can iterate through the
    // chunks that need to be sent and stop iterating the first time we hit one
 ...
 private:
  ...
  // Called after pFile->m_UncompressedData has been setup. This compresses the
  // data, prepares the header, copies the filename, and adds it into the queue
  // for the multicast thread.
  int FinishFileSetup(CMulticastFile *pFile, const char *pFilename,
                      const char *pPathID, bool bFileAlreadyExisted);

  void IncrementChunkRefCount(CMasterMulticastThread::CMulticastFile *pFile,
                              int iChunk);
 ...
 private:
  ...
  unsigned long
      m_nCurMemoryUsage;  // Total of all the file data we have loaded.
...
bool CMasterMulticastThread::Init(IBaseFileSystem *pPassThru,
                                  unsigned short localPort, const IpV4 *pAddr,
                                  int maxMemoryUsage) {
  Term();
  ...
  if (VMPI_GetFileSystemMode() == VMPIFileSystemMode::VMPI_FILESYSTEM_TCP) {
    ...
    // Bind to INADDR_ANY.
    IpV4 localAddr(0, 0, 0, 0, localPort);
    ...
    if (VMPI_GetFileSystemMode() ==
        VMPIFileSystemMode::VMPI_FILESYSTEM_BROADCAST) {
      // Set up for broadcast
      BOOL bBroadcast = TRUE;
      if (setsockopt(m_Socket, SOL_SOCKET, SO_BROADCAST, (char *)&bBroadcast,
                     bBroadcast) == SOCKET_ERROR) {
        Term();
        Warning(
            "CMasterMulticastThread::Init - setsockopt() failed to set "
            "broadcast mode\n");
        return false;
      }
    }
    ...
    DWORD dwThreadID = 0;
    m_hThread =
        CreateThread(NULL, 0, &CMasterMulticastThread::StaticMulticastThread,
                     this, 0, &dwThreadID);
    if (!m_hThread) {
      Term();
  ...
  // For debug mode to verify that we don't try to open files while in another
  // thread.
...
void CMasterMulticastThread::TCP_SendNextChunk(CMulticastFile *pFile,
                                               CClientFileInfo *pClient) {
  // No more chunks to send?
  if ((pClient->m_TCP_LastChunkSent + 1) >= pFile->m_Info.m_nChunks) return;
  ...
  // Assemble the packet.
  unsigned char cPacket[2] = {VMPI_PACKETID_FILESYSTEM,
                              VMPI_FSPACKETID_FILE_CHUNK};

  const void *chunks[5] = {cPacket, &pFile->m_Info, &iChunkToSend,
                           pFile->GetFilename(), &pFile->m_Data[iStartByte]};

  ptrdiff_t chunkLengths[5] = {
      sizeof(cPacket), sizeof(pFile->m_Info), sizeof(m_iCurActiveChunk),
      V_strlen(pFile->GetFilename()) + 1, iEndByte - iStartByte};

  VMPI_SendChunks(chunks, chunkLengths, 5, pClient->m_ClientID);
}

int CMasterMulticastThread::AddFileRequest(const char *pFilename,
                                           const char *pPathID, int clientID,
                                           bool *bZeroLength) {
...
void CMasterMulticastThread::OnChunkReceived(int fileID, int clientID,
                                             int iChunk) {
  if (!m_Files.IsValidIndex(fileID)) {
    Warning(
        "CMasterMulticastThread::OnChunkReceived: invalid file (%d) from "
        "client %d\n",
        fileID, clientID);
    return;
  }
  ...
  if (!pClient) {
    // This will spam sometimes if a worker stops responding and we timeout on
    // it, but then it comes back alive and starts responding. So let's ignore
    // its packets silently.
    // Warning( "CMasterMulticastThread::OnChunkReceived: invalid client ID (%d)
  ...
  if (VMPI_GetFileSystemMode() == VMPIFileSystemMode::VMPI_FILESYSTEM_TCP) {
    ...
    if (!pFile->m_Chunks.IsValidIndex(iChunk)) {
      Warning(
          "CMasterMulticastThread::OnChunkReceived: invalid chunk index (%d) "
          "for file %s\n",
...
void CMasterMulticastThread::OnClientDisconnect(int clientID,
  ...
  if (bGrabCriticalSection) EnterCriticalSection(&m_CS);
  ...
  FOR_EACH_LL(m_Files, iFile) {
    ...
    FOR_EACH_LL(pFile->m_Clients, iClient) {
      ...
      if (pClient->m_ClientID != clientID) continue;

      // Ok, this is our man. Decrement the refcount of any chunks this client
      // wanted.
...
void CMasterMulticastThread::CreateVirtualFile(const char *pFilename,
                                               const void *pData,
                                               unsigned long fileLength) {
  ...
  if (iFile != -1)
    Error(
        "CMasterMulticastThread::CreateVirtualFile( %s ) - file already "
        "exists!",
        pFilename);
...
bool CMasterMulticastThread::CheckClientTimeouts() {
  ...
  int iNext = 0;
  for (int iCur = pFile->m_Clients.Head();
       iCur != pFile->m_Clients.InvalidIndex(); iCur = iNext) {
    iNext = pFile->m_Clients.Next(iCur);
    CClientFileInfo *pInfo = pFile->m_Clients[iCur];

    // If the client has already fully received this file, don't bother timing
    // out on it.
    if (pInfo->m_nChunksLeft == 0) continue;

    ++pInfo->m_nTimesFileCycled;
    if (pInfo->m_nTimesFileCycled >= MIN_FILE_CYCLE_COUNT &&
        (flCurTime - pInfo->m_flLastAckTime) > CLIENT_FILE_ACK_TIMEOUT) {
      // For debug output, get the most recent time we heard any ack from this
      // client at all.
      double flMostRecentTime = pInfo->m_flLastAckTime;
      ...
      Warning("\nClient %s timed out on file %s (latest: %.2f / cur: %.2f).\n",
              VMPI_GetMachineName(pInfo->m_ClientID), pFile->GetFilename(),
              flMostRecentTime, flCurTime);
...
inline bool CMasterMulticastThread::Thread_SendFileChunk_Multicast(
    int *pnBytesSent) {
  ...
  if (m_MulticastAddr.sin_addr.S_un.S_un_b.s_b1 == 127 &&
    ...
    memcpy(&allData[bufs[0].len + bufs[1].len + bufs[2].len], bufs[3].buf,
           bufs[3].len);
    int ret = sendto(m_Socket, allData, nWantedBytes, 0,
  ...
  if (bSuccess) {
    *pnBytesSent = (int)nBytesSent;
    ...
    if (nWarnings < 10) {
      ...
      Warning("%s", std::system_category().message(::GetLastError()).c_str());
    } else if (nWarnings == 10) {
      Warning(
          "\nThis machine's ability to multicast may be broken. Please reboot "
          "and try again.\n");
    }
  }
...
void CMasterMulticastThread::Thread_SeekToNextActiveChunk() {
  ...
  while (1) {
    if (m_iCurActiveChunk ==
            m_Files[m_iCurFile]->m_ActiveChunks.InvalidIndex() ||
        m_Files[m_iCurFile]->m_ActiveChunks[m_iCurActiveChunk]->m_RefCount ==
            0) {
      ...
      if (m_iCurFile == m_Files.InvalidIndex()) m_iCurFile = m_Files.Head();
    ...
    if (m_iCurActiveChunk !=
      ...
      if (m_Files[m_iCurFile]->m_ActiveChunks[m_iCurActiveChunk]->m_RefCount !=
          0) {
        break;
      }

      m_iCurActiveChunk =
          m_Files[m_iCurFile]->m_ActiveChunks.Next(m_iCurActiveChunk);
    }
  }
}
...
DWORD CMasterMulticastThread::MulticastThread() {
  ...
  DWORD msToWait =
      0;  // Only temporarily used if we don't have any data to send.
  ...
  while (WaitForSingleObject(m_hTermEvent, msToWait) != WAIT_OBJECT_0) {
    ...
    // Ok, now we're active, so send out our presence to other CTransmitRateMgrs
    // on the network.
    ...
    // We have to do this check a second time here because the
    // CheckClientTimeouts() call may have booted our last client. If we don't
    // check it here, we might be transmitting something we don't want to
    // transmit. Also, if we don't break out of the loop above, it can prevent
    // the process from ever exiting because it'll never exit that while() loop.
    if (m_nTotalActiveChunks == 0) {
      LeaveCriticalSection(&m_CS);
      msToWait = 50;
      continue;
    ...
    unsigned long optimalTimeTaken =
        (unsigned long)(transmitRateMgr.GetMicrosecondsPerByte() *
                        (nBytesSent + estimatedPacketHeaderSize));

    // If we went faster than we should have, then wait for the difference in
    // time.
...
void CMasterMulticastThread::IncrementChunkRefCount(
    CMasterMulticastThread::CMulticastFile *pFile, int iChunk) {
...
void CMasterMulticastThread::DecrementChunkRefCount(int iFile, int iChunk) {
  ...
  if (pChunk->m_RefCount == 0) {
    Error(
        "CMasterMulticastThread::DecrementChunkRefCount - refcount already "
        "zero!\n");
  }
  ...
  if (pChunk->m_RefCount == 0) {
    ...
    // If this is the current chunk the thread is reading on, seek up to the
    // next chunk so the thread doesn't spin off into the next file and skip its
    // current file's contents.
    if (iFile == m_iCurFile &&
        pChunk->m_iActiveChunksIndex == m_iCurActiveChunk) {
      m_iCurActiveChunk =
          pFile->m_ActiveChunks.Next(pChunk->m_iActiveChunksIndex);
    }
...
bool CMasterMulticastThread::FindWarningSuppression(const char *pFilename) {
  FOR_EACH_LL(m_WarningSuppressions, i) {
    if (Q_stricmp(m_WarningSuppressions[i], pFilename) == 0) return true;
  }
  return false;
}
...
int CMasterMulticastThread::FindOrAddFile(const char *pFilename,
  ...
  if (iFile != -1) {
    ...
    if (pFile->m_bDataLoaded) {
      ...
      // Ok, we have the file entry, but its data has been freed, so we need to
      // reload it.
      EnterCriticalSection(&m_CS);
  ...
  // Can't open a file outside our main thread, because we have to talk to the
  // filesystem and the filesystem doesn't support that.
  Assert(GetCurrentThread() == m_hMainThread);

  // When the worker originally asked for the path ID, they could pass NULL and
  // it would come through as "". Now set it back to null for the filesystem
  // we're passing the call to.
  FileHandle_t fp =
      m_pPassThru->Open(pFilename, "rb", pPathID[0] == 0 ? NULL : pPathID);
  if (!fp) {
...
int CMasterMulticastThread::FinishFileSetup(CMulticastFile *pFile,
                                            const char *pFilename,
                                            const char *pPathID,
                                            bool bFileAlreadyExisted) {
  // Compress the file's contents.
  if (!ZLibCompress(pFile->m_UncompressedData.Base(),
                    pFile->m_UncompressedData.Count(), pFile->m_Data)) {
    delete pFile;
    return -1;
  }
  ...
  if (!bFileAlreadyExisted) {
    ...
    for (intp i = 0; i < pFile->m_Chunks.Count(); i++) {
  ...
  m_nCurMemoryUsage +=
      (pFile->m_Info.m_CompressedSize + pFile->m_Info.m_UncompressedSize);
...
void CMasterMulticastThread::EnsureMemoryLimit(CMulticastFile *pIgnore) {
  if (m_nMaxMemoryUsage != 0 && m_nCurMemoryUsage > m_nMaxMemoryUsage) {
    ...
    FOR_EACH_LL(m_Files, iFile) {
      CMulticastFile *pFile = m_Files[iFile];
      if (pFile == pIgnore || !pFile->m_bDataLoaded) continue;
...
CBaseVMPIFileSystem *CreateMasterVMPIFileSystem(int maxMemoryUsage,
                                                IFileSystem *pPassThru) {
  auto *pRet = new CMasterVMPIFileSystem;

  g_pBaseVMPIFileSystem = pRet;

  if (pRet->Init(maxMemoryUsage, pPassThru)) return pRet;
...
bool CMasterVMPIFileSystem::Init(int maxMemoryUsage, IFileSystem *pPassThru) {
  // Only init the BASE filesystem passthru. Leave the IFileSystem passthru
  // using NULL so it'll crash immediately if they try to use a function we
  // don't support.
  ...
  constexpr unsigned short localPort = 23412;  // This can be anything.

  unsigned short port = RandomInt(22000, 25000);
  if (VMPI_GetRunMode() == VMPIRunMode::VMPI_RUN_NETWORKED) {
    if (VMPI_GetFileSystemMode() ==
        VMPIFileSystemMode::VMPI_FILESYSTEM_MULTICAST) {
    ...
    } else if (VMPI_GetFileSystemMode() ==
               VMPIFileSystemMode::VMPI_FILESYSTEM_BROADCAST) {
      m_MulticastIP = IpV4(0xFF, 0xFF, 0xFF, 0xFF, port);
    }
  } else {
    // Doesn't matter.. we don't use the multicast IP in TCP mode.
    m_MulticastIP = IpV4(0, 0, 0, 0, 0);
  }

  if (!m_MasterThread.Init(pPassThru, localPort, &m_MulticastIP,
                           maxMemoryUsage)) {
    return false;
  }
  ...
  // Make sure we're notified when a client disconnects so we can unlink them
  // from the multicast thread's structures.
...
FileHandle_t CMasterVMPIFileSystem::Open(const char *pFilename,
                                         const char *pOptions,
                                         const char *pPathID) {
  Assert(g_bUseMPI);

  if (g_bDisableFileAccess)
    Error("Open( %s, %s ) - file access has been disabled.", pFilename,
          pOptions);
  ...
  if (bWriteAccess) {
    FileHandle_t fp =
        m_pBaseFileSystemPassThru->Open(pFilename, pOptions, pPathID);
    if (fp == FILESYSTEM_INVALID_HANDLE) return FILESYSTEM_INVALID_HANDLE;

    auto *pFile = new CVMPIFile_PassThru(m_pBaseFileSystemPassThru, fp);
    return (FileHandle_t)pFile;
  }

  // Internally, we require path IDs to be non-null. We'll convert it back to
  // null whenever we make filesystem calls though.
  if (!pPathID) pPathID = "";

  // Have our multicast thread load all the data so it's there when workers want
  // it.
  int iFile = m_MasterThread.FindOrAddFile(pFilename, pPathID);
  if (iFile == -1) return FILESYSTEM_INVALID_HANDLE;

  const CUtlVector<char> &data = m_MasterThread.GetFileData(iFile);

  auto *pFile = new CVMPIFile_Memory(data.Base(), data.Count(),
                                     strchr(pOptions, 't') ? 't' : 'b');
  return (FileHandle_t)pFile;
}

void CMasterVMPIFileSystem::OnClientDisconnect(int procID,
                                               const char *pReason) {
  s_pMasterVMPIFileSystem->m_MasterThread.OnClientDisconnect(procID);
}

void CMasterVMPIFileSystem::CreateVirtualFile(const char *pFilename,
                                              const void *pData,
                                              int fileLength) {
  m_MasterThread.CreateVirtualFile(pFilename, pData, fileLength);
}

bool CMasterVMPIFileSystem::HandleFileSystemPacket(MessageBuffer *pBuf,
                                                   int iSource, int iPacketID) {
  // Handle this packet.
  ...
  switch (subPacketID) {
    case VMPI_FSPACKETID_FILE_REQUEST: {
      ...
      bool bZeroLength;
      int fileID = m_MasterThread.AddFileRequest(pFilename, pPathID, iSource,
                                                 &bZeroLength);

      // Send back the file ID.
      unsigned char cPacket[2] = {VMPI_PACKETID_FILESYSTEM,
                                  VMPI_FSPACKETID_FILE_RESPONSE};
      void *pChunks[4] = {cPacket, &requestID, &fileID, &bZeroLength};
      ptrdiff_t chunkLen[4] = {sizeof(cPacket), sizeof(requestID),
                               sizeof(fileID), sizeof(bZeroLength)};
    ...
    case VMPI_FSPACKETID_CHUNK_RECEIVED: {
      ...
      intp nChunks = (pBuf->getLen() - 2) / 4;
      for (intp i = 0; i < nChunks; i++) {
...
CSysModule *CMasterVMPIFileSystem::LoadModule(const char *pFileName,
                                              const char *pPathID,
                                              bool bValidatedDllOnly) {
  return m_pMasterVMPIFileSystemPassThru->LoadModule(pFileName, pPathID,
                                                     bValidatedDllOnly);
}
