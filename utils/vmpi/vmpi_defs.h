...
// NOTE: the service version (embedded in vmpi_service.exe as a resource) is the
// version that is used to apply patches.
// This matches IDS_VERSION_STRING in vmpi_service.exe.
#define VMPI_SERVICE_IDS_VERSION_STRING 102

// Known packet IDs in various systems.
// The file system reserves this packet ID.
// All application traffic must set its first byte to something other
// than this value.
#define VMPI_PACKETID_FILESYSTEM 0
...
enum VmpiService2UiPacketId {
  // Print some text to the UI's console.
  VMPI_SERVICE_TO_UI_CONSOLE_TEXT = 0,
  // Updates state reflecting whether it's idle, busy, etc.
  VMPI_SERVICE_TO_UI_STATE = 1,
  // Updates state reflecting whether it's idle, busy, etc.
  VMPI_SERVICE_TO_UI_PATCHING = 2,
  // Updates state reflecting whether it's idle, busy, etc.
  VMPI_SERVICE_TO_UI_EXIT = 3
};

// Application state.. these are communicated between the service and the UI.
enum VmpiServiceState {
...
// User chose "exit" from the menu. Kill the service.
#define VMPI_SERVICE_EXIT 5
...
// The VMPI service downloader is still a worker but it uses this port range so
// the master knows it's just downloading the exes.
...
// - null-terminated password string (or VMPI_PASSWORD_OVERRIDE followed by a
// zero to process it regardless of pw).
// - packet ID
// - payload
...
// This is the broadcast message from the main (rank 0) process looking for
// workers.
...
enum VmpiState {
  VMPI_STATE_IDLE = 0,
  VMPI_STATE_BUSY = 1,
  VMPI_STATE_PATCHING = 2,
  VMPI_STATE_DISABLED = 3,
  VMPI_STATE_SCREENSAVER_DISABLED = 4,
