...
#define DW_SUBPACKETID_WU_STARTED \
  6  // A worker telling the master it has started processing a work unit.
// NOTE VMPI_DISTRIBUTE_WORK_EXTRA_SUBPACKET_BASE is where the
// IWorkUnitDistributorX classes start their subpackets.

IWorkUnitDistributorCallbacks *g_pDistributeWorkCallbacks = NULL;

static CDSInfo g_DSInfo;
static unsigned short g_iCurDSInfo =
    (unsigned short)-1;  // This is incremented each time DistributeWork is
                         // called.
static int g_iMasterFinishedDistributeWorkCall =
    -1;  // The worker stores this to know which DistributeWork() calls the
         // master has finished.
static int g_iMasterReadyForDistributeWorkCall = -1;

// This is only valid if we're a worker and if the worker currently has threads
// chewing on work units.
...
static uint64 g_nDuplicatedWUs;  // How many times a worker sent results for a
                                 // work unit that was already completed.

// Set to true if Error() is called and we want to exit early. vrad and vvis
// check for this in their thread functions, so the workers quit early when the
// master is done rather than finishing up potentially time-consuming work units
// they're working on.
...
static CCriticalSectionData<CMasterWorkUnitCompletedList>
    g_MasterWorkUnitCompletedList;
...
void ShowMPIStats(double flTimeSpent, unsigned long nBytesSent,
                  unsigned long nBytesReceived, unsigned long nMessagesSent,
                  unsigned long nMessagesReceived) {
  double flKSent = (nBytesSent + 511.0) / 1024;
  double flKRecv = (nBytesReceived + 511.0) / 1024;

  bool bShowOutput =
      VMPI_IsParamUsed(EVMPICmdLineParam::mpi_ShowDistributeWorkStats);
  ...
  Msg("Total Bytes Sent : %dk (%.2fk/sec, %d messages)\n", (int)flKSent,
      flKSent / flTimeSpent, nMessagesSent);
  Msg("Total Bytes Recv : %dk (%.2fk/sec, %d messages)\n", (int)flKRecv,
      flKRecv / flTimeSpent, nMessagesReceived);
  if (g_bMPIMaster) {
    Msg("Duplicated WUs   : %I64u (%.1f%%)\n", g_nDuplicatedWUs,
        (float)g_nDuplicatedWUs * 100.0f / g_nWUs);
    ...
    for (int i = 0; i < nProcs; i++) sortedProcs[i] = i;
    ...
    for (int i = 0; i < nProcs; i++) {
      ...
      char formatStr[512];
      Q_snprintf(formatStr, sizeof(formatStr), "%%%zus %I64u\n",
...
bool DistributeWorkDispatch(MessageBuffer *pBuf, int iSource, int iPacketID) {
  ...
  if (iCurDistributeWorkCall >= MAX_DW_CALLS)
    Error(
        "Got an invalid DistributeWork packet (id: %d, sub: %d) (iCurDW: %d).",
        pBuf->data[0], pBuf->data[1], iCurDistributeWorkCall);
  ...
  switch (pBuf->data[1]) {
    ...
    case DW_SUBPACKETID_WORKER_READY: {
      if (iCurDistributeWorkCall > g_iCurDSInfo || !g_bMPIMaster)
        Error(
            "State incorrect on master for DW_SUBPACKETID_WORKER_READY packet "
            "from %s.",
    ...
    case DW_SUBPACKETID_WU_RESULTS: {
      ...
      if (iWorkUnit >= pInfo->m_nWorkUnits) {
        Error(
            "DistributeWork: got an invalid work unit index (%I64u for WU "
            "count of %I64u).",
            iWorkUnit, pInfo->m_nWorkUnits);
      }
    ...
    default: {
      if (g_pCurDistributorMaster)
        return g_pCurDistributorMaster->HandlePacket(
            pBuf, iSource, iCurDistributeWorkCall != g_iCurDSInfo);
      else if (g_pCurDistributorWorker)
        return g_pCurDistributorWorker->HandlePacket(
            pBuf, iSource, iCurDistributeWorkCall != g_iCurDSInfo);
      else
        return false;
    }
...
EWorkUnitDistributor VMPI_GetActiveWorkUnitDistributor() {
  if (VMPI_IsParamUsed(EVMPICmdLineParam::mpi_UseSDKDistributor)) {
    Msg("Found %s.\n",
        VMPI_GetParamString(EVMPICmdLineParam::mpi_UseSDKDistributor));
    return k_eWorkUnitDistributor_SDK;
  } else if (VMPI_IsParamUsed(EVMPICmdLineParam::mpi_UseDefaultDistributor)) {
    Msg("Found %s.\n",
        VMPI_GetParamString(EVMPICmdLineParam::mpi_UseDefaultDistributor));
...
void PreDistributeWorkSync(CDSInfo *pInfo) {
  if (g_bMPIMaster) {
    // Send a message telling all the workers we're ready to go on this
    // DistributeWork call.
    MessageBuffer mb;
...
void DistributeWork_Master(CDSInfo *pInfo, ProcessWorkUnitFn processFn,
...
void TellMasterThatWorkerStartedAWorkUnit(MessageBuffer &mb, CDSInfo *pInfo,
                                          WUIndexType iWU) {
  mb.setLen(0);
...
void VMPI_WorkerThread(int iThread, void *pUserData) {
  ...
  MessageBuffer mbStartedWorkUnit;  // Special messagebuffer used to tell the
                                    // master when we started a work unit.
  ...
  while (g_iMasterFinishedDistributeWorkCall < g_iCurDSInfo &&
    ...
    if (!g_pCurDistributorWorker->GetNextWorkUnit(&iWU)) {
      // Wait until there are some WUs to do. This should probably use event
      // handles.
      VMPI_Sleep(10);
      continue;
    }
    ...
    WUIndexType const *pBegin = &pWorkerInfo->m_WorkUnitsRunning.Get(0ull),
                      *pEnd =
                          pBegin +
                          pWorkerInfo->m_WorkUnitsRunning.PastVisibleIndex();
    WUIndexType const *pRunningWu = GenericFind(pBegin, pEnd, iWU);
    if (pRunningWu != pEnd) continue;
    ...
    VMPI_SendData(mb.data, mb.getLen(), VMPI_MASTER_ID,
                  /*k_eVMPISendFlags_GroupPackets*/ 0);
...
void DistributeWork_Worker(CDSInfo *pInfo, ProcessWorkUnitFn processFn) {
  ...
  RunThreads_Start(
      VMPI_WorkerThread, pInfo,
      g_bSetThreadPriorities
          ? ERunThreadsPriority::k_eRunThreadsPriority_Idle
...
// This is called by VMPI_Finalize in case it's shutting down due to an Error()
// call. In this case, it's important that the worker threads here are shut down
// before VMPI shuts down its sockets.
...
double DistributeWork(
  ...
  if (g_iCurDSInfo == 0) {
    // Register our disconnect handler so we can deal with it if clients bail
    // out.
    if (g_bMPIMaster) {
  ...
  memset(g_wuCountByProcess.Base(), 0,
         sizeof(int) * g_wuCountByProcess.Count());
  ...
  EWorkUnitDistributor eWorkUnitDistributor =
      VMPI_GetActiveWorkUnitDistributor();
  if (g_bMPIMaster) {
    Assert(!g_pCurDistributorMaster);
    g_pCurDistributorMaster =
        (eWorkUnitDistributor == k_eWorkUnitDistributor_SDK
             ? CreateWUDistributor_SDKMaster()
             : CreateWUDistributor_DefaultMaster());
    ...
    g_pCurDistributorWorker =
        (eWorkUnitDistributor == k_eWorkUnitDistributor_SDK
             ? CreateWUDistributor_SDKWorker()
             : CreateWUDistributor_DefaultWorker());
  ...
  ShowMPIStats(flTimeSpent, g_nBytesSent - nBytesSentStart,
