...
void CMapEntities::CheckUpdateMap( char const *mapname )
{
	...
	while ( 1 )
	{
		...
		do
		{
			...
			if ( V_strieq( key, "name" ) )
			{
				Q_strncpy( name, value, sizeof( name ) );
			}
			if ( V_strieq( key, "targetname" ) )
			{
				Q_strncpy( name, value, sizeof( name ) );
			}
			if ( V_strieq( key, "origin" ) )
			{
				Q_strncpy( origin, value, sizeof( origin ) );
			}
			if ( V_strieq( key, "angles" ) )
			{
...
void CBaseEventPropertiesDialog::DrawSpline( HDC hdc, HWND placeholder, CChoreoEvent *e )
{
...

	{
		...
		RunCodeAtScopeExit(DeleteObject( bg ));
...

	{
		...
		RunCodeAtScopeExit(SelectObject( hdc, oldPen ));
	...
	RunCodeAtScopeExit(DeleteObject( frame ));
