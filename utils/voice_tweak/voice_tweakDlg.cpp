...
CVoiceTweakDlg::CVoiceTweakDlg(CWnd *pParent /*=NULL*/)
    : CBaseDlg(CVoiceTweakDlg::IDD, pParent) {
...
BEGIN_MESSAGE_MAP(CVoiceTweakDlg, CBaseDlg)
...
BOOL CVoiceTweakDlg::OnInitDialog() {
  ...
  CString str;
  if (str.LoadString(MapLanguageStringID(IDS_HELPTEXT))) {
    m_InstructionText.SetWindowText(str);
  }

  // Save off their old settings so we can restore if they hit cancel.
  GetAppMixerControls()->GetValue_Float(IMixerControls::Control::MicVolume,
                                        m_OldVolume);

  float bBoostOn;
  if (GetAppMixerControls()->GetValue_Float(IMixerControls::Control::MicBoost,
                                            bBoostOn)) {
  ...
  m_VolumeSlider.SetPos(
      (int)(VOLUMESLIDER_RANGE - m_OldVolume * VOLUMESLIDER_RANGE));
  ...
  CString titleStr;
  if (titleStr.LoadString(MapLanguageStringID(IDS_WINDOWTITLE))) {
...
BOOL CVoiceTweakDlg::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult) {
  ...
  if (pHdr->hwndFrom == m_VolumeSlider.m_hWnd) {
    GetAppMixerControls()->SetValue_Float(
        IMixerControls::Control::MicVolume,
        (VOLUMESLIDER_RANGE - m_VolumeSlider.GetPos()) /
            (float)VOLUMESLIDER_RANGE);
  }

  return CBaseDlg::OnNotify(wParam, lParam, pResult);
}

LRESULT CVoiceTweakDlg::OnIdle(WPARAM, LPARAM) {
  ...
  if (TweakApp()->m_pVoiceRecord) {
    ...
    if (nBufferedSamples < nMinSamples) {
      ...
      // Get the samples.
      int nSamples = TweakApp()->m_pVoiceRecord->GetRecordedData(
          samples, sizeof(samples) / BYTES_PER_SAMPLE);
      if (nSeconds < 0.5f) {
        ...
        int sig = highValue >= 0 ? 1 : -1;
        highValue =
            sig * (((highValue * sig) >> 9) << 9);  // Get rid of flicker.
        ...
        if (TweakApp()->m_pWaveOut) {
          ...
          constexpr DWORD silentTime = 500;
          constexpr DWORD fadeInTime = 1000;
          if (curTime - startTime < silentTime) {
            memset(samples, 0, nSamples * 2);
          } else if ((curTime - silentTime) - startTime < fadeInTime) {
            float flFade =
                ((curTime - silentTime) - startTime) / (float)fadeInTime;
...
void CVoiceTweakDlg::OnHardwareGain() {
  if (m_HardwareGain.GetCheck())
    GetAppMixerControls()->SetValue_Float(IMixerControls::Control::MicBoost,
                                          true);
  else
    GetAppMixerControls()->SetValue_Float(IMixerControls::Control::MicBoost,
                                          false);
...
void CVoiceTweakDlg::OnSystemSetup() {
  ...
  AfxMessageBox(MapLanguageStringID(IDS_NODPLAYVOICE),
