...
//
// WinIdle.h - Defines a class for sending idle messages to a window from a
// secondary thread

#ifndef SE_UTILS_VOICE_TWEAK_WIN_IDLE_H__
#define SE_UTILS_VOICE_TWEAK_WIN_IDLE_H__
...
class CWinIdle {
 ...
 public:
  ...
  virtual BOOL StartIdle(HWND hWnd, UINT uMessage, WPARAM wParam = 0,
                         LPARAM lParam = 0, DWORD dwDelay = 0);
  // Stop idling
  //	Returns TRUE on success
  virtual BOOL EndIdle();
  // Notify the idle process that the message was received.
  //	Note : If this function is not called, the idle thread will not send any
  // messages
...
class IdleChanger {
 public:
  ...
  ~IdleChanger() { m_pIdle->SetDelay(m_OldDelay); }

  CWinIdle *m_pIdle;
  DWORD m_OldDelay;
};

#endif  // !SE_UTILS_VOICE_TWEAK_WIN_IDLE_H__
