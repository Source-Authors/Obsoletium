...
RGBAColor *ConvertToRGBUpsideDown(byte *pBits, int width, int height,
                                  byte *pPalette, bool *bTranslucent) {
...
bool WriteTGAFile(const char *pFilename, bool bAllowTranslucent, byte *pBits,
                  unsigned short width, unsigned short height, byte *pPalette,
                  bool bPowerOf2, bool *bTranslucent, bool *bResized) {
  *bResized = *bTranslucent = false;

  RGBAColor *pRGB =
      ConvertToRGBUpsideDown(pBits, width, height, pPalette, bTranslucent);
  ...
  if (bPowerOf2) {
    ...
    if ((width & (width - 1)) || (height & (height - 1))) {
      ...
      printf("\t(%hux%hu) -> (%hux%hu)", width, height, newWidth, newHeight);
...
int PrintUsage(const char *pExtra, ...) {
  ...
  fprintf(
      ...
      __argv[0]);
  fprintf(stderr,
          "ex: %s -vtex -BaseDir c:\\hl2\\dod -WadFile c:\\hl1\\dod\\*.wad\n",
          __argv[0]);
  fprintf(stderr,
...
void EnsureDirExists(const char *pDir) {
  if (_access(pDir, 0) != 0) {
    ...
    // because md will create all the subdirectories leading up to the bottom
    // one and _mkdir() won't.
    char cmd[1024];
...
void RunVTexOnFile(const char *pBaseDir, const char *pFilename) {
  ...
  if (!GetModuleFileName(nullptr, executableDir, sizeof(executableDir)) ||
      GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
    Error("Can't get exe filename.\n");
  }
  ...
  *pLastSlash = 0;
  ...
  snprintf(envStr, sizeof(envStr), "vproject=%s", pBaseDir);
  envStr[sizeof(envStr) - 1] = '\0';

  if (putenv(envStr) == -1) Error("Can't set env var '%s'.\n", envStr);

  // Call vtex on this texture now.
  char vtexCommand[1024];
  snprintf(vtexCommand, sizeof(vtexCommand),
           "%s\\vtex.exe -quiet -nopause \"%s\"", executableDir, pFilename);
  vtexCommand[sizeof(vtexCommand) - 1] = '\0';

  if (system(vtexCommand) != 0) Error("\tCommand '%s' failed!\n", vtexCommand);
}

void WriteOutputFiles(const char *pBaseDir, const char *pSubDir,
                      const char *pName, bool bAllowTranslucent, byte *buffer,
                      unsigned short width, unsigned short height,
                      byte *pPalette, bool bVTex) {
...
#define MAXLUMP (640 * 480 * 85 / 64)
  ...
  for (int i = 0; i < numlumps; i++) {
    ...
    unsigned short width = LittleULong(qtex->width);
    unsigned short height = LittleULong(qtex->height);
...
void ProcessSPRFile(const char *pBaseDir, const char *pSubDir,
  ...
  dsprite_t hdr;
  SafeRead(fp, &hdr, sizeof(hdr));

  // Make sure it's a sprite file.
  if (((hdr.ident >> 0) & 0xFF) != 'I' || ((hdr.ident >> 8) & 0xFF) != 'D' ||
      ((hdr.ident >> 16) & 0xFF) != 'S' || ((hdr.ident >> 24) & 0xFF) != 'P') {
  ...
  vmtFilename[sizeof(vmtFilename) - 1] = '\0';
...
// it'll try to figure out all the other parameters for them.
...
int main(int argc, char **argv) {
  ...
  bool bVTex = false;
  const char *pBaseDir = nullptr;
  const char *pWadFilenames = nullptr;
  const char *pBMPFilenames = nullptr;
  const char *pSPRFilenames = nullptr;
  const char *pSubDir = nullptr;
  const char *pOnlyTex = nullptr;
