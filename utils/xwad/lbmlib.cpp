...
int LoadBMP(const char *szFile, BYTE **ppbBits, BYTE **ppbPalette) {
  ...
  if (bmih.biClrUsed == 0) {
    bmih.biClrUsed = 256;
    cbPalBytes = (static_cast<size_t>(1) << bmih.biBitCount) * sizeof(RGBQUAD);
  } else {
  ...
  if (!pb || fread(pb, cbBmpBits, 1 /*count*/, pfile) != 1) {
    rc = -7;
    goto GetOut;
  }
...
void LoadLBM(char *filename, byte **picture, byte **palette) {
  ...
  while (LBM_P < LBMEND_P) {
    ...
    switch (chunktype) {
      ...
      case CMAPID:
        cmapbuffer = (unsigned char *)calloc(1, 768);
        if (!cmapbuffer) Error("Unable to allocate 768 bytes.\n");
      ...
      case BODYID:
        ...
        if (formtype == PBMID) {
          ...
          //
          planes = bmhd.nPlanes;
          if (bmhd.masking == ms_mask) planes++;
          ...
          for (y = 0; y < bmhd.h; y++, pic_p += bmhd.w) {
            for (p = 0; p < planes; p++)
              if (bmhd.compression == cm_rle1)
                body_p =
                    LBMRLEDecompress((byte *)body_p, bitplanes[p], rowsize);
              else if (bmhd.compression == cm_none) {
...
void WriteLBMfile(char *filename, byte *data, int width, int height,
  ...
  if (!lbmptr) Error("Unable to allocate %d bytes.\n", width * height + 1000);
  ...
  *bmhdlength = BigLong(length);
  if (length & 1) *lbmptr++ = 0;  // pad chunk to even offset
