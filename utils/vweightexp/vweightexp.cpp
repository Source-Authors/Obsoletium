...
int VWeightExportClass::DoExport(const TCHAR *name, ExpInterface *ei,
                                 Interface *pi, BOOL suppressPrompts,
                                 DWORD options) {
...
#if 1
  FILE *pFile;
  if ((pFile = fopen(szFile, "wb")) == NULL) return FALSE /*failure*/;
  ...
  for (i = 0; i < m_cMaxNode; i++) {
    fprintf(pFile, "%5d \"%s\" %3d\n", i, m_MaxNode[i].szNodeName,
            m_MaxNode[i].imaxnodeParent);
  }
...
int CollectModelTEP::callback(INode *pnode) {
  ...
  if (m_phyMod) {
    ...
    if (m_phyExport) {
      // create a ModContext Export Interface for the specific node of the
      // Physique Modifier
  ...
  for (iVert = 0; iVert < cVerts; iVert++) {
    MaxVertWeight *pweight = m_phec->m_MaxVertex[m_phec->m_cMaxVertex] =
        new MaxVertWeight[m_phec->m_cMaxNode];
