...
int VWeightImportClass::DoImport(const TCHAR *name, ImpInterface *ii,
  ...
  // read src data
  FILE *pFile;
  if ((pFile = fopen(szFile, "rb")) == NULL) return FALSE /*failure*/;
...
int VWeightImportClass::UpdateModel(INode *pnode) {
  ...
  TriObject *ptriobj =
      (TriObject *)pobj->ConvertToType(m_tvStart, triObjectClassID);
  ...
  for (int iVert = 0; iVert < cVerts; iVert++) {
    ...
    for (int iVert2 = 0; iVert2 < m_cSrcMaxVertex; iVert2++) {
      ...
      for (int index = 0; index < m_cMaxNodes && flDist < flMinDist; index++) {
        if (m_iToSrc[index] >= 0) {
          float tmp =
              mvTmp[index].flDist - m_SrcMaxVertex[iVert2][index].flDist;
          flDist += (tmp * tmp);
        }
      }
      if (flDist < flMinDist) {
