...
EXPORT_THIS ClassDesc *LibClassDesc(int iWhichClass) {
  switch (iWhichClass) {
    case 0:
      return &VWeightExportCD;
    case 1:
      return &VWeightImportCD;
    default:
      return 0;
  }
}
...
BOOL FUndesirableNode(INode *pnode) {
  ...
  if (!strstr(pnode->GetName(), "Bip01")) return TRUE;
...
void SetBoneWeights(Modifier *bonesProMod, int iVertex,
                    MaxVertWeight *pweight) {
  int nb = bonesProMod->SetProperty(BP_PROPID_GET_N_BONES, NULL);
