...
#ifndef SE_UTILS_VWEIGHTEXP_VWEIGHTIMP_H_
#define SE_UTILS_VWEIGHTEXP_VWEIGHTIMP_H_
...
class VWeightImportClass : public SceneImport {
  // friend BOOL CALLBACK ImportOptionsDlgProc(HWND hDlg, UINT message, WPARAM
  // wParam, LPARAM lParam); friend class DumpModelTEP; friend class
  // DumpDeformsTEP;
 ...
 public:
  ...
  virtual const TCHAR *CopyrightMessage(void) {
    return _T("Copyright (c) 1998-2024, Valve LLC");
  }
  ...
  virtual int DoImport(const TCHAR *name, ImpInterface *ii, Interface *i,
                       BOOL suppressPrompts = FALSE);  // Export	file
...
#endif  // !SE_UTILS_VWEIGHTEXP_VWEIGHTIMP_H_
