...
bool MD5_Hash_File(byte (&digest)[MD5_DIGEST_LENGTH], const char *fileName,
                   std::ostream &err) {
...
int main(int argc, char *argv[]) {
  ...
  if (fp) {
    if (fwrite(digest, sizeof(int), 1, fp) != 1) {
      std::cerr << "Can't write md5 " << sizeof(int) << " bytes to "
                << datFileName << ".\n";
      fclose(fp);
      return 3;
    }
    ...
    std::cerr << "Can't close " << datFileName << " to write md5 of " << argv[1]
              << " to: file may be not written.\n";
    return 4;
  }

  const auto rc = errno;
  std::cerr << "Can't open " << datFileName << " to write md5 of " << argv[1]
            << " to: " << strerror(rc) << ".\n";
  return 5;
}
