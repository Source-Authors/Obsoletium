...
//
// .VPK extract / compress / info utility.
//
// See https://developer.valvesoftware.com/wiki/Vpk.exe
...
constexpr inline int k_nVPKDefaultChunkAlign = 1;

bool s_bBeVerbose = false;
bool s_bMakeMultiChunk = false;
bool s_bUseSteamPipeFriendlyBuilder = false;
int s_iMultichunkSize = k_nVPKDefaultChunkSize / (1024 * 1024);
int s_iChunkAlign = k_nVPKDefaultChunkAlign;

CUtlString s_sPrivateKeyFile;
CUtlString s_sPublicKeyFile;
...
[[noreturn]] void PrintArgSummaryAndExit(int rc = 1) {
  ...
  printf(
      ...
      //		"  vpk rehash <vpkfile>\n"
      //		"            Recalculate chunk MD5's.  (Does not
      // recalculate file CRC's)\n" 		"            Can be used with -k
      // to sign an existing unsigned VPK.\n"
      "\n"
      "MISC:\n"
      ...
      "         Use with 'k' command.\n"
      "         For optimal incremental build performance, the control file "
      "used\n"
      "         for the previous build should exist and be named the same as "
      "the\n"
      "         input control file, with '.bak' appended, and each file entry\n"
      "         should have an 'md5' value.  The 'md5' field need not be the\n"
      "         actual MD5 of the file contents, it is just a unique "
      "identifier\n"
  ...
  printf(
      ...
      "         With command 'checksig': Check signature using specified key "
      "file.\n");
#endif
...
bool IsRestrictedFileType(const char *pcFileName) {
  return (V_stristr(pcFileName, ".bat") || V_stristr(pcFileName, ".cmd") ||
          V_stristr(pcFileName, ".com") || V_stristr(pcFileName, ".dll") ||
#if defined(OSX)
          V_stristr(pcFileName, ".dylib") || V_stristr(pcFileName, ".so") ||
          V_stristr(pcFileName, ".kext") ||
#elif defined(_LINUX)
          V_stristr(pcFileName, ".so") || V_stristr(pcFileName, ".ko") ||
#endif
          V_stristr(pcFileName, ".exe") || V_stristr(pcFileName, ".msi") ||
          V_stristr(pcFileName, ".rar") || V_stristr(pcFileName, ".reg") ||
...
void BenchMark(CUtlVector<char *> &names) {
  for (const auto *name : names) ReadFile(name);
}

void AddFileToPack(CPackedStore &mypack, char const *pSrcName,
                   uint32 nPreloadSize = 0, char const *pDestName = nullptr) {
  // Check to make sure that no restricted file types are being added to the VPK
  if (IsRestrictedFileType(pSrcName)) {
    fprintf(stderr, "Ignoring %s: unsupported file type.\n", pSrcName);
    return;
  }

  // !FIXME! Make sure they didn't request alignment, because we aren't doing
  // it.
  if (s_iChunkAlign != 1) Error("-a is only supported with -P");

  if (!pDestName || Q_isempty(pDestName)) pDestName = pSrcName;

  CRequiredInputFile f(pSrcName);
  const uint32 fileSize = f.Size();

  std::unique_ptr<uint8[]> pData = std::make_unique<uint8[]>(fileSize);
  f.MustRead(pData.get(), fileSize);

  ePackedStoreAddResultCode rslt =
      mypack.AddFile(pDestName, Min(fileSize, nPreloadSize), pData.get(),
                     fileSize, s_bMakeMultiChunk);

  if (rslt == EPADD_ERROR) Error("Error adding %s\n", pSrcName);
...
#ifdef VPK_ENABLE_SIGNING
static void LoadKeyFile(const char *pszFilename, const char *pszTag,
                        CUtlVector<uint8> &outBytes) {
  KeyValuesAD kv("key");
  ...
  if (!pszEncodedBytes) Error("Key file is missing '%s'", pszTag);

  uint8 rgubDecodedData[k_nRSAKeyLenMax * 2];
  size_t cubDecodedData = std::size(rgubDecodedData);

  if (!CCrypto::HexDecode(pszEncodedBytes, rgubDecodedData, &cubDecodedData) ||
      cubDecodedData <= 0)
    Error("Key file contains invalid '%s' value", pszTag);

  outBytes.SetSize(cubDecodedData);
  memcpy(outBytes.Base(), rgubDecodedData, cubDecodedData);
}
#endif

static void CheckLoadKeyFilesForSigning(CPackedStore &mypack) {
  // Not signing?
  if (s_sPrivateKeyFile.IsEmpty() && s_sPublicKeyFile.IsEmpty()) return;

  // Signatures only supported if creating multi-chunk file
  if (!s_bMakeMultiChunk) {
    Error(
        "Multichunk not specified. Only multi-chunk VPK's support "
        "signatures.\n");
  }
  ...
  if (s_sPrivateKeyFile.IsEmpty() || s_sPublicKeyFile.IsEmpty())
    Error(
        "Must specify both public and private key files in order to sign VPK");
  ...
  mypack.SetKeysForSigning(bytesPrivateKey.Count(), bytesPrivateKey.Base(),
                           bytesPublicKey.Count(), bytesPublicKey.Base());
#else
...
class VPKBuilder {
 ...
 private:
  ...
  struct VPKBuildFile_t {
    ...
    intp m_iOldSortIndex;
    intp m_iNewSortIndex;
  ...
  static int CompareBuildFileByOldPhysicalPosition(VPKBuildFile_t *const *pa,
                                                   VPKBuildFile_t *const *pb) {
  ...
  /// All files as they existed in the old VPK.  (Empty if we are building from
  /// scratch.)
  ...
  struct VPKInputFileRange_t {
    intp m_iFirstInputFile;  // index of first input file in the chunk
    intp m_iLastInputFile;   // index of last input file in the chunk
    intp m_iChunkFilenameIndex;
    bool m_bKeepExistingFile;
    int64 m_nTotalSizeInChunkFile;

    intp FileCount() const {
      intp iResult = m_iLastInputFile - m_iFirstInputFile + 1;
      Assert(iResult > 0);
      return iResult;
    }
  };
  ...
  void SplitRangeAt(intp iFirstInputFile);
  void AddRange(VPKInputFileRange_t range);
  void MapRangeToChunk(int idxRange, intp iChunkFilenameIndex,
...
VPKBuilder::VPKBuilder(CPackedStore &packfile) : m_packfile(packfile) {
  ...
  if (savePublicKey.Count() != m_packfile.GetSignaturePublicKey().Count() ||
      V_memcmp(savePublicKey.Base(), m_packfile.GetSignaturePublicKey().Base(),
               savePublicKey.Count()) != 0) {
...
VPKBuilder::~VPKBuilder() {
  if (m_pInputKeys) m_pInputKeys->deleteThis();
  if (m_pOldInputKeys) m_pOldInputKeys->deleteThis();
}
...
void VPKBuilder::BuildOldSchoolFromInputKeys() {
  ...
  FOR_EACH_VEC(m_vecNewFiles, i) {
    ...
    auto idxInDict = m_dictFiles.Find(f->m_sName.String());
    Assert(idxInDict >= 0);
  ...
  if (s_bBeVerbose) printf("Hashing metadata.\n");
  m_packfile.HashMetadata();

  if (s_bBeVerbose) printf("Writing directory file.\n");
  m_packfile.Write();
}
...
void VPKBuilder::BuildSteamPipeFriendlyFromInputKeys() {
  ...
  FOR_EACH_VEC(m_vecOldFiles, i) {
    ...
    auto idxInDict = m_dictFiles.Find(szNameInVPK);
    ...
    if (m_dictFiles[idxInDict].m_pOld)
      Error(
          "File '%s' is listed in VPK directory multiple times?!  Cannot build "
          "incrementally.\n",
          szNameInVPK);
  ...
  bool bIncremental =
      (m_vecOldFiles.Count() > 0) && !m_sControlFilename.IsEmpty();
  if (bIncremental) {
    printf("Building incrementally in SteamPipe-friendly manner.\n");
    printf("Existing pack file contains %zd files\n", m_vecOldFiles.Count());
    ...
    if (m_pOldInputKeys->LoadFromFile(g_pFullFileSystem, sControlFilenameBak)) {
      ...
      for (KeyValues *i = m_pOldInputKeys; i; i = i->GetNextKey()) {
        ...
        if (szNameInVPK[0] == '\0')
          Error("File '%s' is missing 'destpath' in old KeyValues control file",
                pszNameOnDisk);
        ...
        auto idxInDict = m_dictFiles.Find(szNameInVPK);
        if (idxInDict == m_dictFiles.InvalidIndex() ||
            m_dictFiles[idxInDict].m_pOld == nullptr)
          Error(
              "File '%s' in old KeyValues control file not found in pack "
              "file.\nThat control file was probably not used to build the "
              "pack file\n",
              szNameInVPK);
        VPKBuildFile_t &bf = m_dictFiles[idxInDict];
        ...
        if (bf.m_pOldKey)
          Error(
              "File '%s' appears multiple times in old KeyValues control "
              "file.\nThat control file was probably not used to build the "
              "pack file\n",
              szNameInVPK);
        bf.m_pOldKey = i;
        ...
        iPreloadSizeFromControlFile =
            Min(iPreloadSizeFromControlFile, (int)bf.m_pOld->m_iTotalSize);

        if (iPreloadSizeFromControlFile != (int)bf.m_pOld->m_iPreloadSize)
          Error(
              "File '%s' preload size mismatch in old KeyValues control file "
              "and pack file.\nThat control file was probably not used to "
              "build the pack file\n",
              szNameInVPK);
        ...
        if (*pszMD5) {
          ...
          fprintf(
              stderr,
              "WARNING: Old control file entry '%s' does not have an MD5; we "
              "will have to compare file contents for this file.\n",
              pszNameOnDisk);
        }
      }

      // Now many sure every file in the pack was found in the control file.  If
      // not, then they probably don't match and we should not trust the MD5's.
      FOR_EACH_DICT_FAST(m_dictFiles, idxInDict) {
        VPKBuildFile_t &bf = m_dictFiles[idxInDict];
        if (bf.m_pOld && bf.m_pOldKey == nullptr)
          Error(
              "File '%s' is in pack but not in old control file %s.\n"
              "That control file was probably not used to build the pack file",
              bf.m_pOld->m_sName.String(), sControlFilenameBak.String());
      }

      printf(
          "%s appears to match VPK file.\nUsing MD5s for incremental "
          "building\n",
          sControlFilenameBak.String());
    } else {
      fprintf(stderr,
              "WARNING: %s not present; incremental building will be slow.\n",
              sControlFilenameBak.String());
      printf(
          "         For best results, provide the control file previously used "
          "for building.\n");
  ...
  if (m_vecOldFilesInChunkOrder.Count() > 0)
    nOldChunkCount =
        m_vecOldFilesInChunkOrder[m_vecOldFilesInChunkOrder.Count() - 1]
            ->m_pOld->m_idxChunk +
        1;
  ...
  if (bIncremental && nOldChunkCount > 0) {
    ...
    while (idxOldFile < m_vecOldFilesInChunkOrder.Count()) {
      ...
      m_packfile.GetDataFileName(szDataFilename, sizeof(szDataFilename),
                                 idxChunk);
      const char *pszShortDataFilename = V_GetFileName(szDataFilename);

      int idxInChunk = 0;
      CUtlVector<intp> vecFilesToCompareContents;
      ...
      while (idxOldFile < m_vecOldFilesInChunkOrder.Count()) {
        ...
        Assert(pOld);
        if (idxChunk != pOld->m_idxChunk) break;
        ...
        if (sReasonCannotReuse.IsEmpty()) {
          ...
          intp iExpectedSortIndex = firstFile.m_iNewSortIndex + idxInChunk;
          ...
          if (pNew == nullptr) {
            ...
            sReasonCannotReuse.Format("File '%s' changed preload size.",
                                      pszFilename);
          } else if (f.m_iNewSortIndex != iExpectedSortIndex) {
            // Files reordered in some way.  Try to give an appropriate message
            if (f.m_iNewSortIndex > iExpectedSortIndex &&
                iExpectedSortIndex < m_vecNewFilesInChunkOrder.Count()) {
              VPKContentFileInfo_t const *pInsertedFile =
                  m_vecNewFilesInChunkOrder[iExpectedSortIndex];
              const char *pszInsertedFilename = pInsertedFile->m_sName.String();
              auto idxDictInserted = m_dictFiles.Find(pszInsertedFilename);
              ...
              if (m_dictFiles[idxDictInserted].m_pOld == nullptr)
                ...
                sReasonCannotReuse.Format(
                    "Chunk reordered.  '%s' listed where '%s' used to be.",
                    pszInsertedFilename, pszFilename);
            } else {
              sReasonCannotReuse.Format(
                  "Chunk was reordered.  File '%s' was moved.", pszFilename);
            }
          } else if (f.m_md5Old.IsZero() || f.m_md5New.IsZero()) {
            vecFilesToCompareContents.AddToTail(idxOldFile);
          } else if (f.m_md5Old != f.m_md5New) {
            sReasonCannotReuse.Format(
                "File '%s' changed.  (Based on MD5s in control file.)",
                pszFilename);
          }
        }
      ...
      if (sReasonCannotReuse.IsEmpty() &&
          vecFilesToCompareContents.Count() > 0) {
        ...
        FOR_EACH_VEC(vecFilesToCompareContents, i) {
          VPKBuildFile_t const &f =
              *m_vecOldFilesInChunkOrder[vecFilesToCompareContents[i]];
          Assert(f.m_pOld);
          ...
          // Calculate the CRC
          uint32 crc =
              CRC32_ProcessSingleBuffer(buf.Base(), f.m_pOld->m_iTotalSize);

          // Mismatch?
          if (crc != f.m_pOld->m_crc) {
            sReasonCannotReuse.Format(
                "File '%s' changed.  (CRCs differs from %s.)",
                f.m_pOld->m_sName.String(), f.m_sNameOnDisk.String());
            break;
          }
        }
      }
      ...
      if (sReasonCannotReuse.IsEmpty()) {
        ...
        chunkRange.m_iLastInputFile =
            firstFile.m_iNewSortIndex + idxInChunk - 1;
        chunkRange.m_bKeepExistingFile = true;
        AddRange(chunkRange);
      } else {
        printf("%s cannot be reused.  %s\n", pszShortDataFilename,
               sReasonCannotReuse.String());
      }
    }
  }
  ...
  intp nNewChunkCount = m_llFileRanges.Count();
  printf("Pack file will contain %zd chunk files\n", nNewChunkCount);
  ...
  intp idxChunk = 0;
  intp iChunksToKeep = 0;
  intp iFilesToKeep = 0;
  int64 iChunkSizeToKeep = 0;
  intp iChunksToWrite = 0;
  intp iFilesToWrite = 0;
  ...
  char szDataFilename[MAX_PATH];
  for (idxChunk = 0; idxChunk < nNewChunkCount; ++idxChunk) {
    ...
    printf("%s %s (%zd files, %lld bytes)\n",
           r.m_bKeepExistingFile ? "Keeping" : "Writing", pszShortDataFilename,
           r.FileCount(), (long long)r.m_nTotalSizeInChunkFile);
    if (s_bBeVerbose) {
      printf("  First file: %s\n",
    ...
    if (r.m_bKeepExistingFile) {
      // Mark the input files in this chunk as having been assigned to this
      // chunk.
    ...
    for (auto idxFile = r.m_iFirstInputFile; idxFile <= r.m_iLastInputFile;
      ...
      int nBytesWritten = g_pFullFileSystem->Write(
          buf.Base<byte>() + f->m_iPreloadSize, nBytesToWrite, fChunkWrite);
  ...
  for (auto iChunkToDelete = nNewChunkCount; iChunkToDelete < nOldChunkCount;
       ++iChunkToDelete) {
    m_packfile.GetDataFileName(szDataFilename, sizeof(szDataFilename),
                               iChunkToDelete);
    printf("Deleting %s.\n", szDataFilename);
  ...
  if (s_bBeVerbose) {
    ...
    printf("  Pack file chunks:  %12zd%12zd\n", iChunksToKeep, iChunksToWrite);
    printf("  Data files:        %12zd%12zd\n", iFilesToKeep, iFilesToWrite);
    printf("  Bytes in chunk:    %12lld%12lld\n", (long long)iChunkSizeToKeep,
           (long long)iChunkSizeToWrite);
  }
  ...
  FOR_EACH_DICT(m_dictFiles, idxInDict) {
    ...
    if (pNew == nullptr || pNew->m_idxChunk >= 0) continue;
    ...
    if (iNeedToUpdateFile < 0) {
      Assert(pOld);
      if (pOld->m_crc == pNew->m_crc) continue;
    }
  ...
  if (!bNeedToWriteDir) {
    ...
    printf(
        "VPK contents not changed, but directory needs to be resaved.  %s.\n",
...
void VPKBuilder::SetInputKeys(KeyValues *pInputKeys,
                              const char *pszControlFilename) {
  ...
  for (KeyValues *i = m_pInputKeys; i; i = i->GetNextKey()) {
    ...
    V_strcpy_safe(szNameInVPK, i->GetString("destpath", ""));
    if (szNameInVPK[0] == '\0')
      Error("File '%s' is missing 'destpath' in KeyValues control file",
            pszNameOnDisk);
    V_FixSlashes(szNameInVPK, '\\');  // always use Windows slashes in VPK
    ...
    if (szNameInVPK[0] == '\\')
      Error(
          "destpath '%s' is an absolute path; only relative paths should be "
          "used",
          szNameInVPK);

    // Check to make sure that no restricted file types are being added to the
    // VPK.
    if (IsRestrictedFileType(szNameInVPK)) {
      fprintf(
          stderr,
          "WARNING: Control file lists '%s'.  We cannot put that type of file "
          "in the pack.\n",
          szNameInVPK);
      continue;
    }
    ...
    if (!bf.m_sNameOnDisk.IsEmpty() || bf.m_pNew || bf.m_iNewSortIndex >= 0)
      Error(
          "destpath '%s' in VPK appears multiple times in the KV control "
          "file.\n  (Source files '%s' and '%s')",
          szNameInVPK, bf.m_sNameOnDisk.String(), pszNameOnDisk);
    ...
    f->m_iPreloadSize =
        Min((uint32)i->GetInt("preloadsize", 0), f->m_iTotalSize);
    const char *pszMD5 = i->GetString("md5", "");
    if (*pszMD5) {
      if (V_strlen(pszMD5) != MD5_DIGEST_LENGTH * 2)
        Error("File '%s' has invalid MD5 '%s'", pszNameOnDisk, pszMD5);
  ...
  printf("Control file lists %zd files\n", m_vecNewFiles.Count());
...
void VPKBuilder::MapAllRangesToChunks() {
  ...
  // If a range is NOT at least as big as one chunk file, then we will have to
  // merge it with an adjacent range --- that is, we will need to unmap an
  // adjacent range. So the first step will be to identify which of the
  // currently mapped ranges to unmap in order to get rid of any ranges that
  // cannot get mapped to a chunk. We might have a choice in the matter, and
  // each range that we unmap means another file that will have to be rewritten.
  // So the goal here is to minimize the number/size of chunks that we unmap and
  // force to rewrite.
  //
  // The current state of affairs should be that all mapped regions correspond
  // to chunk files that do not need to be rewritten, and there are no two
  // unmapped chunk files in a row.

  int64 iSizeTooSmallForAChunk =
      (int64)m_packfile.GetWriteChunkSize() * 95 / 100;
  for (;;) {
    for (;;) {
      ...
      FOR_EACH_LL(m_llFileRanges, idxRange) {
        VPKInputFileRange_t &r = m_llFileRanges[idxRange];
        if (r.m_iChunkFilenameIndex < 0) continue;
        ...
        if (idxPrev != m_llFileRanges.InvalidIndex()) {
          ...
          if (p.m_iChunkFilenameIndex < 0 &&
              p.m_nTotalSizeInChunkFile < iSizeTooSmallForAChunk) {
            ++iScore;
        ...
        if (idxNext != m_llFileRanges.InvalidIndex()) {
          ...
          if (n.m_iChunkFilenameIndex < 0 &&
              n.m_nTotalSizeInChunkFile < iSizeTooSmallForAChunk) {
            ++iScore;
        ...
        // When choosing which of two neighbors should absorb a new gap, add it
        // to the smaller one. (That will be less to rewrite and also keep the
        // chunk size at a more desirable level.)
      ...
      if (idxBestRangeToUnmap == m_llFileRanges.InvalidIndex()) break;
    ...
    // OK, at this point, if there were any ranges that were too small to hold
    // their own chunks, then we should have merged them.  (Unless there is
    // exactly one range.) The next step is to split up ranges that are too
    ...
    FOR_EACH_LL(m_llFileRanges, idxRange) {
      ...
      intp iChunks =
          r->m_nTotalSizeInChunkFile / m_packfile.GetWriteChunkSize();
      if (iChunks <= 1) continue;
      ...
      if (r->m_iChunkFilenameIndex >= 0) {
        Assert(r->m_bKeepExistingFile);
        printf(
            "Chunk %zd is currently bigger than desired chunk size of %d "
            "bytes, "
            "but we're not splitting it because the contents have not "
            "changed.\n",
      ...
      // Note that if we have big files inside, we might not have enough
      // granularity to do exactly what they desire and could get caught in a
      // bad state
      int64 iDesiredSize = r->m_nTotalSizeInChunkFile / iChunks;
      Assert(iDesiredSize >= m_packfile.GetWriteChunkSize());
      intp iNewLastInputFile = r->m_iFirstInputFile;
      int64 iNewSize =
          m_vecNewFilesInChunkOrder[iNewLastInputFile]->GetSizeInChunkFile();
      while (iNewSize < iDesiredSize &&
             iNewLastInputFile < r->m_iLastInputFile) {
        ++iNewLastInputFile;
        iNewSize +=
            m_vecNewFilesInChunkOrder[iNewLastInputFile]->GetSizeInChunkFile();
      }
      ...
      // Do the split
      intp iSaveFirstInputFile = r->m_iFirstInputFile;
      ...
      // modified and link the new range AFTER this range.  Verify that
      // assumption.
    ...
    for (auto i = m_vecRangeForChunk.Count() - 1; i >= nNewChunkCount; --i) {
      int idxRange = m_vecRangeForChunk[i];
      if (idxRange == m_llFileRanges.InvalidIndex()) continue;
...
void VPKBuilder::UnmapAllRangesForChangedChunks() {
  ...
  FOR_EACH_LL(m_llFileRanges, idxRange) {
    ...
    // If range was assigned a chunk, but the chunk file will have to be
    // rewitten, then unmap it
...
void VPKBuilder::CoaleseAllUnmappedRanges() {
  ...
  for (;;) {
    int idxNext = m_llFileRanges.Next(idxRange);
    if (idxNext == m_llFileRanges.InvalidIndex()) break;
...
void VPKBuilder::CalculateRangeTotalSizeInChunkFile(
    VPKInputFileRange_t &range) const {
  range.m_nTotalSizeInChunkFile = 0;
  for (auto i = range.m_iFirstInputFile; i <= range.m_iLastInputFile; ++i) {
    range.m_nTotalSizeInChunkFile +=
        m_vecNewFilesInChunkOrder[i]->GetSizeInChunkFile();
  }
}
...
void VPKBuilder::PrintRangeDebug() {
  FOR_EACH_LL(m_llFileRanges, idxRange) {
    ...
    printf("  File range %zd .. %zd\n", r.m_iFirstInputFile,
           r.m_iLastInputFile);
    printf("  Chunk %zd%s\n", r.m_iChunkFilenameIndex,
           r.m_bKeepExistingFile ? "  (keep existing file)" : "");
...
void VPKBuilder::AddRange(VPKInputFileRange_t range) {
  ...
  FOR_EACH_LL(m_llFileRanges, idxRange) {
    ...
    if (p->m_iLastInputFile < range.m_iFirstInputFile) continue;
    ...
    MapRangeToChunk(idxRange, range.m_iChunkFilenameIndex,
                    range.m_bKeepExistingFile);
    return;
  }
...
void VPKBuilder::SplitRangeAt(intp iFirstInputFile) {
  // Sanity check that ranges are in a valid order
  SanityCheckRanges();
  ...
  FOR_EACH_LL(m_llFileRanges, idxRange) {
    ...
    if (p->m_iFirstInputFile == iFirstInputFile ||
        p->m_iLastInputFile + 1 == iFirstInputFile)
      return;
...
void VPKBuilder::MapRangeToChunk(int idxRange, intp iChunkFilenameIndex,
                                 bool bKeepExistingFile) {
  VPKInputFileRange_t *p = &m_llFileRanges[idxRange];
  ...
  if (p->m_iChunkFilenameIndex >= 0) {
    ...
    m_vecRangeForChunk[p->m_iChunkFilenameIndex] =
        m_llFileRanges.InvalidIndex();
  ...
  if (iChunkFilenameIndex >= 0) {
    Assert(m_vecRangeForChunk[iChunkFilenameIndex] ==
           m_llFileRanges.InvalidIndex());
...
void GenerateKeyPair(const char *pszBaseKeyName) {
  ...
  size_t cubPublicKey = std::size(rgubPublicKey);

  uint8 rgubPrivateKey[k_nRSAKeyLenMax] = {0};
  size_t cubPrivateKey = std::size(rgubPrivateKey);

  if (!CCrypto::RSAGenerateKeys(rgubPublicKey, &cubPublicKey, rgubPrivateKey,
                                &cubPrivateKey)) {
    Error("Failed to generate RSA keypair.\n");
  }

  char rgchEncodedPublicKey[k_nRSAKeyLenMax * 4];
  uint cubEncodedPublicKey = std::size(rgchEncodedPublicKey);

  if (!CCrypto::HexEncode(rgubPublicKey, cubPublicKey, rgchEncodedPublicKey,
                          cubEncodedPublicKey)) {
    Error("Failed to encode public key.\n");
  }

  // Don't encrypt
  //	uint8 rgubEncryptedPrivateKey[std::size( rgubPrivateKey )*2];
  //	size_t cubEncryptedPrivateKey = std::size( rgubEncryptedPrivateKey );
  //
  //	if( !CCrypto::SymmetricEncrypt( rgubPrivateKey, cubPrivateKey,
  // rgubEncryptedPrivateKey, &cubEncryptedPrivateKey, (uint8 *)rgchPassphrase,
  // k_nSymmetricKeyLen ) )
  //	{
  ...
  char rgchEncodedEncryptedPrivateKey[std::size(rgubPrivateKey) * 8];
  if (!CCrypto::HexEncode(rgubPrivateKey, cubPrivateKey,
                          rgchEncodedEncryptedPrivateKey,
                          std::size(rgchEncodedEncryptedPrivateKey))) {
    Error("Failed to encode private key.\n");
  }

  // Good Lord.  Use fopen, because it will work without any surprising crap or
  // hidden limitations. I just wasted an hour trying to get CUtlBuffer and our
  // filesystem to print a block of text to a file.
  ...
  // Save public keyfile
  {
    ...
    fprintf(f,
            "// Public key file.  You can publish this key file and share it "
            "with the world.\n"
            "// It can be used by third parties to verify any signatures made "
  ...
  // Save private keyfile
  {
    ...
    fprintf(f,
            ...
            "your signature by using your public key.\n"
            "//\n"
            "// THIS KEY SHOULD BE KEPT SECRET\n"
            "//\n"
            "// You should share your public key freely, but anyone who has "
            "your private key will be able to impersonate you.\n"
            "private_key\n"
            "{\n"
            "\ttype \"rsa\"\n"
            "\trsa_private_key \"%s\"\n"
            "\n"
            "\t// Note: the private key is stored in plaintext.  It is not "
            "encrypted or protected by a password.\n"
            "\t//       Anyone who obtains this key can use it to sign files.\n"
            "\tprivate_key_encrypted 0\n"
            "\n"
            "\t// The public key that corresponds to this private key.  The "
            "public keyfile you can share with others is\n"
            "\t// saved in another file, but the key data is duplicated here "
            "to help you confirm which public key matches\n"
  ...
  printf("\n");
  printf(
      "REMEMBER: Your private key should be kept secret.  Don't share it!\n");
}
...
static void CheckSignature(const char *pszFilename) {
  ...
  if (s_sPublicKeyFile.IsEmpty()) {
    if (!s_sPrivateKeyFile.IsEmpty())
      Error(
          "Private keys are not used to verify signatures.  Did you mean to "
          "use -k instead?");
  ...
  printf("\n");
  fflush(stdout);
  CPackedStore::ESignatureCheckResult result =
      pack.CheckSignature(bytesPublicKey.Count(), bytesPublicKey.Base());
  switch (result) {
    default:
...
static void CheckHashes(const char *pszFilename) {
  ...
  CUtlSortVector<ChunkHashFraction_t, ChunkHashFractionLess_t> &vecHashes =
      pack.AccessPackFileHashes();
  ...
  FOR_EACH_VEC(vecHashes, idx) {
    ...
    pack.HashEntirePackFile(handle, fileSize, frac.m_nFileFraction,
                            frac.m_cbChunkLen, filehash);
    ...
    if (filehash.m_cbFileLen != frac.m_cbChunkLen) {
      ...
      if (nCheckedFractionsOK >= 0) ++nCheckedFractionsOK;
    }
  }
...
static void PrintBinaryBlob(const CUtlVector<uint8> &blob) {
  constexpr intp kRowLen = 32;
  for (intp i = 0; i < blob.Count(); i += kRowLen) {
    intp iEnd = min(i + kRowLen, blob.Count());
    const char *pszSep = "  ";
    for (intp j = i; j < iEnd; ++j) {
...
void BuildRecursiveFileList(const char *pcDirName, CUtlStringList &fileList) {
  ...
  if (pcResult) {
    do {
      ...
      V_sprintf_safe(szFullResultPath, "%s%c%s", pcDirName,
                     CORRECT_PATH_SEPARATOR, pcResult);
...
static void DroppedVpk(const char *pszVpkFilename) {
  ...
  for (auto *name : fileNames) {
    ...
    CPackedStoreFileHandle pData = mypack.OpenFile(name);

    V_sprintf_safe(szDestFilePath, "%s%c%s", szVPKParentDir,
                   CORRECT_PATH_SEPARATOR, name);
    ...
    if (pData) {
      ...
      if (outF.IsOk()) {
        ...
        while (nBytes) {
          char cpBuf[65535];
          int nReadSize = MIN(static_cast<int>(ssize(cpBuf)), nBytes);
...
static void DroppedDirectory(const char *pszDirectoryArg) {
  ...
  V_sprintf_safe(szVPKPath, "%s.vpk", szDirectory);
  ...
  KeyValues *pInputKeys = new KeyValues("packkeys");
  const intp nBaseDirLength = V_strlen(szDirectory);
  for (auto *name : fileList) {
    // .... Ug O(n^2)
    KeyValues *pFileKey = pInputKeys->CreateNewKey();
    pFileKey->SetString("srcpath", name);

    const char *pszDestPath = name + nBaseDirLength;
    if (*pszDestPath == '/' || *pszDestPath == '\\') ++pszDestPath;
...
int main(int argc, char **argv) {
  ...
  if (V_stricmp(pszCommand, "l") == 0) {
    ...
    for (int i = 3; i < argc; i++) {
      ...
      if (pData) {
        ...
        fprintf(stderr, "WARNING: couldn't find file %s\n", argv[i]);
        break;
      }
    }
  ...
  } else if (argc == 2 && V_GetFileExtension(argv[1]) &&
             V_stristr(V_GetFileExtension(argv[1]), "vpk")) {
    DroppedVpk(argv[1]);
  } else {
