...
static constexpr int ice_smod[4][4] = {{333, 313, 505, 369},
                                       {379, 375, 319, 391},
                                       {361, 445, 451, 397},
                                       {397, 425, 395, 505}};

/* XOR values for the S-boxes */
static constexpr int ice_sxor[4][4] = {{0x83, 0x85, 0x9b, 0xcd},
                                       {0xcc, 0xa7, 0xad, 0x41},
                                       {0x4b, 0x2e, 0xd4, 0x33},
                                       {0xea, 0xcb, 0x2e, 0x04}};

/* Permutation values for the P-box */
static constexpr unsigned long ice_pbox[32] = {
...
static constexpr int ice_keyrot[16] = {0, 1, 2, 3, 2, 1, 3, 0,
                                       1, 3, 2, 0, 3, 1, 0, 2};
...
IceKey::~IceKey() {
  ...
  for (i = 0; i < _rounds; i++)
    for (j = 0; j < 3; j++) _keysched[i].val[j] = 0;
...
void IceKey::encrypt(const unsigned char *ptext, unsigned char *ctext) const {
  ...
  l = (((unsigned long)ptext[0]) << 24) | (((unsigned long)ptext[1]) << 16) |
      (((unsigned long)ptext[2]) << 8) | ptext[3];
  r = (((unsigned long)ptext[4]) << 24) | (((unsigned long)ptext[5]) << 16) |
...
void IceKey::decrypt(const unsigned char *ctext, unsigned char *ptext) const {
  ...
  l = (((unsigned long)ctext[0]) << 24) | (((unsigned long)ctext[1]) << 16) |
      (((unsigned long)ctext[2]) << 8) | ctext[3];
  r = (((unsigned long)ctext[4]) << 24) | (((unsigned long)ctext[5]) << 16) |
...
int IceKey::keySize() const { return (_size * 8); }
