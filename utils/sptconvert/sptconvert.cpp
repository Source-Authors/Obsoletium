...
//
// A simple tool for converting SpeedTree .spt files into .smd files for use in
// Source
...
void OutputMaterialFile(char *filestub, bool bEnableAlphaTest,
  ...
  _snprintf(filename, MAX_PATH, "%smaterials\\Trees\\%s_VertexLit.vmt", gamedir,
            filestub);
...
void OutputLeafMaterialFile(char *filestub, bool bEnableAlphaTest,
  ...
  char filename[MAX_PATH];
  _snprintf(filename, MAX_PATH, "%smaterials\\Trees\\%s_TreeLeaf.vmt", gamedir,
            filestub);
  FILE *file = fopen(filename, "wt");
  if (!file) return;
...
void OutputTreeGeometry(CSpeedTreeRT &speedTree, CSpeedTreeRT::SGeometry &sGeom,
                        char *filename) {
  FILE *file = fopen(filename, "wt");
  if (!file) return;
  ...
  _splitpath(sTextures.m_pBranchTextureFilename, NULL, NULL, branchTextureName,
             NULL);
  ...
  for (int nStrip = 0; nStrip < sGeom.m_sBranches.m_usNumStrips; nStrip++) {
    ...
    for (int i = 0; i < nStripLength - 2; i++) {
      int nIndices[3] = {pStripIndices[i], pStripIndices[i + 1],
                         pStripIndices[i + 2]};
      ...
      for (int j = 0; j < 3; j++) {
        ...
        const float *pTexCoord =
            &sGeom.m_sBranches.m_pTexCoords0[nIndices[j] * 2];

        fprintf(file, "0 %f %f %f %f %f %f %f %f 0\n", pPos[0], pPos[1],
                pPos[2], pNormal[0], pNormal[1], pNormal[2], pTexCoord[0],
                pTexCoord[1]);
      }
    }
  }
  ...
  for (unsigned int i = 0; i < sTextures.m_uiFrondTextureCount; i++) {
    ...
    _splitpath(sTextures.m_pFrondTextureFilenames[i], NULL, NULL, filestub,
               NULL);
  ...
  for (int nStrip = 0; nStrip < sGeom.m_sFronds.m_usNumStrips; nStrip++) {
    ...
    for (int i = 0; i < nStripLength - 2; i++) {
      ...
      _splitpath(sTextures.m_pFrondTextureFilenames[0], NULL, NULL,
                 frondTextureName, NULL);
      fprintf(file, "%s_VertexLit\n", frondTextureName);
      ...
      for (int j = 0; j < 3; j++) {
        ...
        const float *pTexCoord =
            &sGeom.m_sFronds.m_pTexCoords0[nIndices[j] * 2];
  ...
  for (int i = 0; i < leaves.m_usLeafCount; i++) {
    ...
    _splitpath(sTextures.m_pLeafTextureFilenames[index], NULL, NULL,
               leafTextureName, NULL);
    ...
    float d[3] = {pCoords[8] - pCoords[0], pCoords[9] - pCoords[1],
                  pCoords[10] - pCoords[2]};
    float size = sqrtf(d[0] * d[0] + d[1] * d[1] + d[2] * d[2]) * 0.5f;

    fprintf(file, "%s_TreeLeaf\n", leafTextureName);
    fprintf(file, "0 %f %f %f %f %f 0.0 %f %f\n", pPos[0], pPos[1], pPos[2],
            pTex[0], pTex[1], -size, -size);
    fprintf(file, "0 %f %f %f %f %f 0.0 %f %f\n", pPos[0], pPos[1], pPos[2],
            pTex[2], pTex[3], size, -size);
    fprintf(file, "0 %f %f %f %f %f 0.0 %f %f\n", pPos[0], pPos[1], pPos[2],
            pTex[4], pTex[5], size, size);
  ...
  for (unsigned int i = 0; i < sTextures.m_uiLeafTextureCount; i++) {
    char filestub[MAX_PATH];
    _splitpath(sTextures.m_pLeafTextureFilenames[i], NULL, NULL, filestub,
               NULL);
...
int main(int argc, char **argv) {
  ...
  if (CommandLine()->ParmCount() != 2) {
    fprintf(stderr, "Usage: sptconvert <SPT file name>\n");
    return EINVAL;
  }

  MathLib_Init(2.2f, 2.2f, 0.0f, 2, false, false, false, false);

  const ScopedFileSystem scoped_file_system{CommandLine()->GetParm(1)};
  ...
  if (speedTree.LoadTree(argv[1])) {
    if (speedTree.Compute(NULL, 1, false)) {
      ...
      fprintf(stderr, "Failed to compute SPT tree from '%s'.\n", argv[1]);
      return 2;
    }
  } else {
    fprintf(stderr, "Failed to load SPT tree from '%s'.\n", argv[1]);
    return 1;
  }
