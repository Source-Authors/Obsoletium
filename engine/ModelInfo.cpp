...
{
	return g_pMDLCache->GetVirtualModelFast( pStudioHdr, pStudioHdr->VirtualModel() );
}
...
{
	return g_pMDLCache->GetAnimBlock( pStudioHdr->VirtualModel(), nBlock );
}

intp CModelInfo::GetAutoplayList( const studiohdr_t *pStudioHdr, unsigned short **pAutoplayList ) const
{
	return g_pMDLCache->GetAutoplayList( pStudioHdr->VirtualModel(), pAutoplayList );
}
...
virtualmodel_t *studiohdr_t::GetVirtualModel( void ) const
{
	if ( numincludemodels == 0 )
		return NULL;
	return g_pMDLCache->GetVirtualModelFast( this, VirtualModel() );
}
