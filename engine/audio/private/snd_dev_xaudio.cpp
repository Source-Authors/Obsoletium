...
class XAudio2VoiceCallback : public IXAudio2VoiceCallback {
 public:
  ...
  STDMETHOD_(void, OnVoiceError)(void *ctx, HRESULT hr) override {
    Warning("XAudio2 voice processing error for buffer %zu: 0x%8x",
...
static bool GetDefaultAudioDeviceFormFactor(
  ...
  if (FAILED(hr)) {
    Warning("XAudio2: Get default audio render endpoint failed w/e 0x%8x.\n",
            hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    Warning(
        "XAudio2: Get default audio render endpoint props failed w/e 0x%8x.\n",
        hr);
    return false;
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (device_friendly_name.as_wide_string(audio_device_name)) {
      ...
      Msg("XAudio2: Using system audio device \"%S\".\n", audio_device_name);
    } else {
      Msg("XAudio2: Using system audio device \"%S\".\n", L"N/A");
    }
  } else {
    Warning(
        "XAudio2: Get default audio render endpoint friendly name failed w/e "
        "0x%8x.\n",
        hr);
    // Continue.
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    Warning(
        "XAudio2: Get default audio render endpoint physical speakers mask "
        "failed w/e 0x%8x.\n",
        hr);
  ...
  if (SUCCEEDED(hr)) {
    ...
    Warning(
        "XAudio2: Get default audio render endpoint form factor failed w/e "
        "0x%8x.\n",
        hr);
    return false;
  }
 ...
 public:
  ...
  OnDefaultDeviceChanged(_In_ EDataFlow flow, _In_ ERole role,
    ...
    if (flow == device_data_flow_ && role == device_role_) {
      Msg("XAudio2: Default system audio render device has been %s.\n",
...
void CAudioXAudio2::TransferSamples(int end_time) {
  if (audio_buffer_head_pos_ - audio_buffer_tail_pos_ >=
      kMaxXAudio2DeviceBuffersCount) {
    DevWarning("XAudio2: No free audio buffers! All %u buffers taken.\n",
               kMaxXAudio2DeviceBuffersCount);
  ...
  if (samples_count > kXAudio2BufferSamplesCount) {
    DevWarning("XAudio2: Overflowed mix buffer! All %u samples taken.\n",
  ...
  const HRESULT hr{xaudio2_source_voice_->SubmitSourceBuffer(&buffer)};
  if (FAILED(hr)) {
    Warning("XAudio2: Submitting samples buffer failed w/e 0x%8x.\n", hr);
  }
}
