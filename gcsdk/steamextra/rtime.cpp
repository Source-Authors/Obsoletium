...
static char* _strptime(const char *s, const char *format, struct tm *tm) {
	while (*format != NULL && *s != NULL) {
	  ...
	  case 'A':
		  tm->tm_wday = -1;
		  for (int i = 0; i < 7; ++i) {
			  len = V_strlen(kWeekAbbr[i]);
			  if (V_strnicmp(kWeekAbbr[i], s, len) == 0) {
				  tm->tm_wday = i;
				  break;
			  }

			  len = V_strlen(kWeekFull[i]);
	  ...
	  case 'h':
		  tm->tm_mon = -1;
		  for (int i = 0; i < 12; ++i) {
			  len = V_strlen(kMonthAbbr[i]);
			  if ( V_strnicmp(kMonthAbbr[i], s, len) == 0) {
				  tm->tm_mon = i;
				  break;
			  }

			  len = V_strlen(kMonthFull[i]);
...
RTime32 CRTime::RTime32FromFmtString( const char *pchFmt, const char* pchValue )
{
	...
	intp cchFmt = Q_strlen( pchFmt );		
	intp cchValue = Q_strlen( rgchValue );
