...
intp TextEntry::PixelToCursorSpace(int cx, int cy)
{
	...
	GetSize(w, h);
	// dimhotepus: Scale UI.
	cx = clamp(cx, 0, w+QuickPropScale( 100 ));
	cy = clamp(cy, 0, h);
...
bool TextEntry::RequestInfo(KeyValues *outputData)
{
	...
	else if (V_strieq(outputData->GetName(), "GetState"))
	{
		char buf[64];
		GetText(buf);
...
void TextEntry::SetToFullWidth()
{
	...
	int tall = (surface()->GetFontTall(_font) + DRAW_OFFSET_Y()) + DRAW_OFFSET_Y() + QuickPropScale( 2 );
