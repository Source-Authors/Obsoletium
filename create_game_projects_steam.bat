...
if ["%~2"]==["x86"] (
  ...
  SET "SOLUTION_NAME=%SOLUTION_NAME%_x86_steam"
) else if ["%~2"]==["x64"] (
  SET CMAKE_MSVC_ARCH_NAME=x64

  ECHO "Generating solution %SOLUTION_NAME% for x64"
  SET "SOLUTION_NAME=%SOLUTION_NAME%_x64_steam"
) else (
...
devtools\bin\vpc.exe /2026 %WIN_X64% /define:WORKSHOP_IMPORT_DISABLE /define:SIXENSE_DISABLE /define:NO_X360_XDK /define:RAD_TELEMETRY_DISABLED /define:DISABLE_ETW /define:NO_ATI_COMPRESS /define:NO_NVTC /define:LTCG /no_ceg /nofpo /%GAME_NAME% +game /mksln %SOLUTION_NAME% /slnitems .slnitems
if ERRORLEVEL 1 (
