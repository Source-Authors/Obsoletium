...
#ifndef SRC_MATERIALSYSTEM_SHADERAPIDX11_SHADER_API_DX11_H_
#define SRC_MATERIALSYSTEM_SHADERAPIDX11_SHADER_API_DX11_H_
...
#define MAX_DX11_VIEWPORTS \
  D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE
#define MAX_DX11_STREAMS D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT

namespace se::shaderapidx11 {

// A record describing the state on the board
struct ShaderIndexBufferStateDx11_t {
  se::win::com::com_ptr<ID3D11Buffer> m_pBuffer;
...
struct ShaderStateDx11_t {
  ...
  se::win::com::com_ptr<ID3D11RasterizerState> m_pRasterState;
  se::win::com::com_ptr<ID3D11VertexShader> m_pVertexShader;
  se::win::com::com_ptr<ID3D11GeometryShader> m_pGeometryShader;
  se::win::com::com_ptr<ID3D11PixelShader> m_pPixelShader;
  se::win::com::com_ptr<ID3D11HullShader> m_pHullShader;
  se::win::com::com_ptr<ID3D11DomainShader> m_pDomainShader;
  se::win::com::com_ptr<ID3D11ComputeShader> m_pComputeShader;
...
using StateCommitFunc_t = void (*)(ID3D11Device *pDevice,
...
class CFunctionCommit {
 public:
  ...
  void CallCommitFuncs(ID3D11Device *pDevice,
                       ID3D11DeviceContext *pDeviceContext,
                       const ShaderStateDx11_t &desiredState,
                       ShaderStateDx11_t &currentState, bool bForce = false);
...
// Shader API on DirectX 11 hardware.
class CShaderAPIDx11 final : public CShaderAPIBase, public IDebugTextureInfo {
  using BaseClass = CShaderAPIBase;
 ...
 public:
  ...
  void ClearBuffers(bool bClearColor, bool bClearDepth, bool bClearStencil,
                    int renderTargetWidth, int renderTargetHeight) override;
  void ClearColor3ub(unsigned char r, unsigned char g,
                     unsigned char b) override;
  void ClearColor4ub(unsigned char r, unsigned char g, unsigned char b,
  ...
  void BindVertexBuffer(int nStreamID, IVertexBuffer *pVertexBuffer,
                        int nOffsetInBytes, int nFirstVertex, int nVertexCount,
                        VertexFormat_t fmt, int nRepetitions = 1) override;
  void BindIndexBuffer(IIndexBuffer *pIndexBuffer, int nOffsetInBytes) override;
  void Draw(MaterialPrimitiveType_t primitiveType, int nFirstIndex,
            int nIndexCount) override;
 ...
 public:
  ...
  void Unbind(HullShaderHandle_t hShader);
  void Unbind(DomainShaderHandle_t hShader);
  void Unbind(ComputeShaderHandle_t hShader);
  ...
  float Knob(char *knobname, float *setvalue = nullptr) override {
    return 0.0f;
  }

 private:
  // Returns a d3d texture associated with a texture handle
  IDirect3DBaseTexture *GetD3DTexture(
      ShaderAPITextureHandle_t hTexture) override {
    Assert(0);
    return nullptr;
  }
  ...
  // Sets the mode...
  bool SetMode(void *hwnd, unsigned nAdapter,
  ...
  VertexFormat_t ComputeVertexFormat(int numSnapshots,
                                     StateSnapshot_t *pIds) const override;

  // Gets the vertex format for a set of snapshot ids
  VertexFormat_t ComputeVertexUsage(int numSnapshots,
                                    StateSnapshot_t *pIds) const override;
  ...
  void SetPixelShaderStateAmbientLightCube(
      int pshReg, bool bForceToBlack = false) override {}
  void SetPixelShaderStateAmbientLightCube(int pshReg) {}
  void GetDX9LightState(LightState_t *state) const override {}

  float GetAmbientLightCubeLuminance(void) override { return 0.0f; }
  ...
  void CopyRenderTargetToTextureEx(ShaderAPITextureHandle_t texID,
                                   int nRenderTargetID, Rect_t *pSrcRect,
                                   Rect_t *pDstRect) override {}

  void CopyTextureToRenderTargetEx(int nRenderTargetID,
                                   ShaderAPITextureHandle_t textureHandle,
                                   Rect_t *pSrcRect = NULL,
  ...
  IMesh *GetDynamicMesh(IMaterial *pMaterial, int nHWSkinBoneCount,
                        bool buffered, IMesh *pVertexOverride,
                        IMesh *pIndexOverride) override;
  IMesh *GetDynamicMeshEx(IMaterial *pMaterial, VertexFormat_t fmt,
  ...
  void Ortho(double left, double top, double right, double bottom, double zNear,
             double zFar) override;
  void PerspectiveX(double fovx, double aspect, double zNear,
                    double zFar) override;
  void PerspectiveOffCenterX(double fovx, double aspect, double zNear,
                             double zFar, double bottom, double top,
                             double left, double right) override;
  ...
  void SceneFogColor3ub(unsigned char r, unsigned char g,
                        unsigned char b) override;
  ...
  void SetVertexShaderConstant(int var, float const *pVec, int numConst = 1,
                               bool bForce = false) override;
  void SetPixelShaderConstant(int var, float const *pVec, int numConst = 1,
                              bool bForce = false) override;

  void SetBooleanVertexShaderConstant(int var, BOOL const *pVec,
                                      int numBools = 1,
                                      bool bForce = false) override {
    Assert(0);
  }

  void SetIntegerVertexShaderConstant(int var, int const *pVec,
                                      int numIntVecs = 1,
                                      bool bForce = false) override {
    Assert(0);
  }
  ...
  void SetIntegerPixelShaderConstant(int var, int const *pVec,
                                     int numIntVecs = 1,
                                     bool bForce = false) override {
    Assert(0);
  }
  ...
  void SetLinearToGammaConversionTextures(
  ...
  void OverrideAlphaWriteEnable(bool bOverrideEnable,
                                bool bAlphaWriteEnable) override;
  // forces color writes on or off
  void OverrideColorWriteEnable(bool bOverrideEnable,
                                bool bColorWriteEnable) override;
  ...
  ImageFormat GetNearestSupportedFormat(
      ImageFormat fmt, bool bFilteringRequired = true) const override;
  ImageFormat GetNearestRenderTargetFormat(ImageFormat fmt) const override;

  // Sets the texture state
  void BindTexture(Sampler_t stage,
                   ShaderAPITextureHandle_t textureHandle) override;

  void SetRenderTarget(ShaderAPITextureHandle_t colorTextureHandle,
                       ShaderAPITextureHandle_t depthTextureHandle) override {}
  ...
  void TexImage2D(int level, int cubeFace, ImageFormat dstFormat, int zOffset,
                  int width, int height, ImageFormat srcFormat,
                  bool bSrcIsTiled, void *imageData) override;
  void TexSubImage2D(int level, int cubeFace, int xOffset, int yOffset,
                     int zOffset, int width, int height, ImageFormat srcFormat,
                     int srcStride, bool bSrcIsTiled, void *imageData) override;
  void TexImageFromVTF(IVTFTexture *pVTF, int iVTFFrame) override;

  bool TexLock(int level, int cubeFaceID, int xOffset, int yOffset, int width,
               int height, CPixelWriter &writer) override;
  ...
  void TexWrap(ShaderTexCoordComponent_t coord,
               ShaderTexWrapMode_t wrapMode) override;
  ...
  void CreateTextures(ShaderAPITextureHandle_t *pHandles, int count, int width,
                      int height, int depth, ImageFormat dstImageFormat,
                      int numMipLevels, int numCopies, int flags,
                      const char *pDebugName,
                      const char *pTextureGroupName) override;
  ShaderAPITextureHandle_t CreateDepthTexture(ImageFormat renderFormat,
  ...
  void ClearBuffersObeyStencilEx(bool bClearColor, bool bClearAlpha,
                                 bool bClearDepth) override;
  ...
  void ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect, unsigned char *data,
                  ImageFormat dstFormat, int nDstStride) override;
  ...
  void SetTextureTransformDimension(TextureStage_t textureStage, int dimension,
                                    bool projected) override;
  ...
  // Scissor Rect
  void SetScissorRect(const int nLeft, const int nTop, const int nRight,
                      const int nBottom, const bool bEnableScissor) override {}
  ...
  void MarkUnusedVertexFields(unsigned int nFlags, int nTexCoordCount,
                              bool *pUnusedTexCoords) override {}
  ...
  int OcclusionQuery_GetNumPixelsRendered(ShaderAPIOcclusionQuery_t handle,
                                          bool bFlush) override {
    return 0;
  }
  ...
  void SetDepthFeatheringPixelShaderConstant(int iConstant,
                                             float fDepthBlendScale) override {}
  ...
  MorphFormat_t ComputeMorphFormat(int numSnapshots,
                                   StateSnapshot_t *pIds) const override {
    return 0;
  }

  // Gets the bound morph's vertex format; returns 0 if no morph is bound
  MorphFormat_t GetBoundMorphFormat() override { return 0; }

  void GetStandardTextureDimensions(int *pWidth, int *pHeight,
                                    StandardTextureId_t id) override;

  // Binds a standard texture
  void BindStandardTexture(Sampler_t stage, StandardTextureId_t id) override {}

  void BindStandardVertexTexture(VertexTextureSampler_t stage,
                                 StandardTextureId_t id) override {}

  void SetFlashlightState(const FlashlightState_t &state,
                          const VMatrix &worldToTexture) override {}

  void SetFlashlightStateEx(const FlashlightState_t &state,
                            const VMatrix &worldToTexture,
                            ITexture *pFlashlightDepthTexture) override {}

  const FlashlightState_t &GetFlashlightState(
      VMatrix &worldToTexture) const override {
    static FlashlightState_t blah;
    return blah;
  }

  const FlashlightState_t &GetFlashlightStateEx(
      VMatrix &worldToTexture,
      ITexture **pFlashlightDepthTexture) const override {
    static FlashlightState_t blah;
    return blah;
  }
  ...
  void BindVertexTexture(VertexTextureSampler_t nStage,
                         ShaderAPITextureHandle_t hTexture) override {}

  // Sets morph target factors
  void SetFlexWeights(int nFirstWeight, int nCount,
                      const MorphWeight_t *pWeights) override {}

  // NOTE: Stuff after this is added after shipping HL2.
  ITexture *GetRenderTargetEx(int nRenderTargetID) override { return NULL; }
  ...
  float GetLightMapScaleFactor(void) const override { return 1.0; }
  ...
  void SetVectorRenderingParameter(int parm_number,
                                   Vector const &value) override {}

  float GetFloatRenderingParameter(int parm_number) const override { return 0; }

  int GetIntRenderingParameter(int parm_number) const override { return 0; }
  ...
  void GetDXLevelDefaults(uint &max_dxlevel,
                          uint &recommended_dxlevel) override {
    max_dxlevel = recommended_dxlevel = 90;
  }

  void GetMaxToRender(IMesh *pMesh, bool bMaxUntilFlush, int *pMaxVerts,
                      int *pMaxIndices) override {
    *pMaxVerts = 32768;
    *pMaxIndices = 32768;
  }

  // Returns the max possible vertices + indices to render in a single draw call
  int GetMaxVerticesToRender(IMaterial *pMaterial) override { return 32768; }

  int GetMaxIndicesToRender() override { return 32768; }
  int CompareSnapshots(StateSnapshot_t snapshot0,
                       StateSnapshot_t snapshot1) override {
    return 0;
  }
  ...
  void ComputeVertexDescription(unsigned char *pBuffer,
                                ...
                                MeshDesc_t &desc) const override {}
  ...
  void SetShadowDepthBiasFactors(float fShadowSlopeScaleDepthBias,
                                 float fShadowDepthBias) override {}
  ...
  intp GetNumActiveDeformations() const override { return 0; }

  void ExecuteCommandBuffer(uint8 *pBuf) override {}

  void SetStandardTextureHandle(StandardTextureId_t,
                                ShaderAPITextureHandle_t) override {}

  void SetPSNearAndFarZ(int pshReg) override {}

  void CopyRenderTargetToScratchTexture(ShaderAPITextureHandle_t srcRt,
                                        ShaderAPITextureHandle_t dstTex,
                                        Rect_t *pSrcRect = NULL,
                                        Rect_t *pDstRect = NULL) override {
    Assert(0);
  }

  void LockRect(void **pOutBits, int *pOutPitch,
                ShaderAPITextureHandle_t texHandle, int mipmap, int x, int y,
                int w, int h, bool bWrite, bool bRead) override {
    Assert(0);
  }
  ...
  // Set the finest mipmap that can be used for the texture which is currently
  // being modified.
  void TexLodClamp(int finest) override { Assert(0); }

  // Set the Lod Bias for the texture which is currently being modified.
  void TexLodBias(float bias) override { Assert(0); }

  void CopyTextureToTexture(ShaderAPITextureHandle_t srcTex,
                            ShaderAPITextureHandle_t dstTex) override {
    Assert(0);
  }

  int GetPackedDeformationInformation(int nMaskOfUnderstoodDeformations,
                                      float *pConstantValuesOut,
                                      int nBufferSize, int nMaximumDeformations,
                                      int *pNumDefsOut) const override {
    *pNumDefsOut = 0;
    return 0;
  }

  bool OwnGPUResources(bool bEnable) override { return false; }
...
#endif  // !SRC_MATERIALSYSTEM_SHADERAPIDX11_SHADER_API_DX11_H_
