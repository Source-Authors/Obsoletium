...
public:
	...
	void ExecuteDefferredBuild( byte *pVertexData, int nVerts, int nBytesVerts, uint16 *pIndexData, int nIndices )
	{
		...
		if ( pVertexData && desc.m_ActualVertexSize ) // if !desc.m_ActualVertexSize, device lost
		{
			...
			if ( pDest )
			{
				FastCopy( pDest, pVertexData, nBytesVerts );
			}
		}
		...
		if ( pIndexData && pIndexData != &gm_ScratchIndexBuffer[0] && desc.m_nIndexSize )
		{
			if ( !desc.m_nFirstVertex )
			{
				...
				FastCopy( desc.m_pIndices, pIndexData, nIndices * sizeof(*pIndexData) );
			}
			else
			{
				alignas(16) uint16 tempIndices[16];
				...
				while ( i < nIndices )
				{
					...
					FastCopy( desc.m_pIndices + i, tempIndices, nToCopy * sizeof(uint16)  );
					i += nToCopy;
				}
			}
		}
...
byte *CMatQueuedRenderContext::AllocVertices( int nVerts, int nVertexSize )
{
	...
	if ( !pResult )
	{
		...
		if ( pNextAlloc > pCommitLimit )
		{
			...
			const size_t commitSize = pNewCommitLimit - pCommitLimit;
		...
		// Now crash.
		*(volatile uint64 *)nullptr = (uint64)status << 32 | m_Vertices.GetUsed();
	}
#endif
	return (byte *) pResult;
}
...
uint16 *CMatQueuedRenderContext::AllocIndices( int nIndices )
{
	...
	if ( !pResult )
	{
		...
		if ( pNextAlloc > pCommitLimit )
		{
			...
			Msg( "VirtualAlloc(%p, %u) returned %p on repeat. VirtualAlloc %s with code %s.\n",
				pCommitLimit, commitSize, pRet, (pRet != NULL) ? "succeeded" : "failed",
				std::system_category().message(status).c_str() );
		}
		else
		...
		// Now crash.
		*(volatile uint64 *)nullptr = (uint64)status << 32 | m_Indices.GetUsed();
	}
#endif
