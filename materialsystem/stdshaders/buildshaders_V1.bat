...
REM ****************
REM usage: buildshaders_V1 <shaderProjectName>
REM ****************
...
set DIRECTX_SDK_BIN_DEP=dx9sdk\utilities\x64\D3DX9_43.dll

if /i "%2" == "-dx9_30" goto dx_sdk_dx9_30
if /i "%2" == "-dx11" goto dx_sdk_dx111
goto dx_sdk_end
:dx_sdk_dx9_30
			set DIRECTX_SDK_VER=pc09.30
			set DIRECTX_SDK_BIN_DIR=dx9sdk\utilities\dx9_30\x64
			set DIRECTX_SDK_BIN_DEP=dx9sdk\utilities\dx9_30\x64\D3DX9_43.dll
			goto dx_sdk_end
:dx_sdk_dx11
			set DIRECTX_SDK_VER=pc11.00
			set DIRECTX_SDK_BIN_DIR=dx11sdk\utilities\dx11\x64
...
:usage
echo.
echo "usage: buildshaders_V1 <shaderProjectName> [-dx11 or -game] [gameDir if -game was specified] [-source sourceDir]"
...
echo "ex   : buildshaders_V1 myshaders"
echo "ex   : buildshaders_V1 myshaders -game c:\steam\steamapps\sourcemods\mymod -source c:\mymod\src"
goto :end
...
echo Building inc files, asm vcs files, and worklist for %inputbase%...
...
if /i "%DIRECTX_SDK_VER%" == "pc09.30" (
	rem echo "Copy extra files for dx 9.3 std"
)
...
if exist "filelist.txt" if exist "uniquefilestocopy.txt" if not "%dynamic_shaders%" == "1" (
	...
	cd %ChangeToDir%
	%shadercompilecommand% -allowdebug -shaderpath "%shader_path_cd:/=\%" %SDKArgs%
	cd %shader_path_cd%
)
