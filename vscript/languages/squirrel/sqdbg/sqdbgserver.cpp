...
constexpr inline SQChar SQDBG_DEBUG_HOOK[]{_SC("_sqdbg_debug_hook_")};
constexpr inline SQChar SQDBG_ERROR_HANDLER[]{_SC("_sqdbg_error_handler_")};
...
SQDbgServer::~SQDbgServer() {
  ...
  if (_accept != INVALID_SOCKET) sqdbg_closesocket(_accept);
  if (_endpoint != INVALID_SOCKET) sqdbg_closesocket(_endpoint);
}
...
bool SQDbgServer::ReadMsg() { return false; }

void SQDbgServer::BusyWait() {
  while (!ReadMsg()) ThreadSleep(0);
}
  ...
  switch (_state) {
    case eDbgState::eDBG_Running:
      if (type == _SC('l') && _breakpoints.size()) {
        auto itr = _breakpoints.find(BreakPoint(line, src));
    ...
    case eDbgState::eDBG_Suspended:
      _nestedcalls = 0;
      // dimhotepus: Add missed break;
      break;

    case eDbgState::eDBG_StepOver:
      switch (type) {
        case _SC('l'):
          if (_nestedcalls == 0) {
    ...
    case eDbgState::eDBG_StepReturn:
      switch (type) {
        case _SC('l'):
          break;
        case _SC('c'):
        ...
        case _SC('r'):
          if (_nestedcalls == 0) {
            _nestedcalls = 0;
            _state = eDbgState::eDBG_StepOver;
          } else {
            _nestedcalls--;
          }
    ...
    case eDbgState::eDBG_Disabled:
      break;
  }
}

constexpr inline unsigned short MSG_ID(char x, char y) {
...
void SQDbgServer::ParseMsg(const char *msg) {
  switch (*((const unsigned short *)msg)) {
    ...
    case MSG_ID('g', 'o'):
      if (_state != eDbgState::eDBG_Running) {
        _state = eDbgState::eDBG_Running;
    ...
    case MSG_ID('s', 'p'):
      if (_state != eDbgState::eDBG_Suspended) {
        _state = eDbgState::eDBG_Suspended;
    ...
    case MSG_ID('s', 'i'):
      if (_state == eDbgState::eDBG_Suspended) {
        _state = eDbgState::eDBG_StepInto;
    ...
    case MSG_ID('s', 'r'):
      if (_state == eDbgState::eDBG_Suspended) {
        _state = eDbgState::eDBG_StepReturn;
    ...
    case MSG_ID('d', 'i'):
      if (_state != eDbgState::eDBG_Disabled) {
        _state = eDbgState::eDBG_Disabled;
...
bool SQDbgServer::ParseBreakpoint(const char *msg, BreakPoint &out) {
  ...
  *dest = '\0';
  ...
  SQChar *p = sq_getscratchpad(
      _v, (SQInteger)(mbstowcs(NULL, stemp, len) + 2) * sizeof(SQChar));
...
void SQDbgServer::BreakExecution() {
  ...
  while (_state == eDbgState::eDBG_Suspended) {
...
void SQDbgServer::AddWatch(Watch &w) { _watches.insert(w); }

void SQDbgServer::RemoveWatch(int id) {
  auto it = _watches.find(Watch(id, _SC("")));
  if (it == _watches.end()) {
    BeginDocument();
...
void SQDbgServer::Attribute(const SQChar *name, const SQChar *value) {
  ...
  scsprintf(&_scratchstring[0], _scratchstring.size(), _SC(" %s=\"%s\""), name,
            escval);
...
void SQDbgServer::EndDocument() { SendChunk(_SC("\r\n")); }

// this can be done much better/faster(do we need that?)
const SQChar *SQDbgServer::escape_xml(const SQChar *s) const {
