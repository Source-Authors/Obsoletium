...
[[nodiscard]] FORCEINLINE float UnitSphereLineSegmentLength( Vector const &a, Vector const &b )
{
	// check unit length
...
[[nodiscard]] FORCEINLINE float UnitSphereTriangleArea( Vector const &a, Vector const &b , Vector const &c )
{
...
[[nodiscard]] float AssociatedLegendrePolynomial( int nL, int nM, float flX );
...
// theta = 0..M_PI
// phi = 0.. 2 * M_PHI
[[nodiscard]] float SphericalHarmonic( int nL, int nM, float flTheta, float flPhi );

// evaluate spherical harmonic with normalized vector direction
[[nodiscard]] float SphericalHarmonic( int nL, int nM, Vector const &vecDirection );
