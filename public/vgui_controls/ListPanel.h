...
public:
	...
	template<intp size>
	void GetCellText(int itemID, int column, OUT_Z_ARRAY wchar_t (&buffer)[size]) // returns the data held by a specific cell
	{
		GetCellText( itemID, column, buffer, static_cast<int>(sizeof(wchar_t)) * size );
	}
