...
{
public:
	virtual	[[nodiscard]] IPhysicsEnvironment		*CreateEnvironment( void ) = 0;
	virtual void DestroyEnvironment( IPhysicsEnvironment * ) = 0;
	virtual [[nodiscard]] IPhysicsEnvironment		*GetActiveEnvironmentByIndex( int index ) = 0;
	...
	virtual [[nodiscard]] IPhysicsObjectPairHash		*CreateObjectPairHash() = 0;
	...
	virtual [[nodiscard]] IPhysicsCollisionSet		*FindOrCreateCollisionSet( unsigned int id, int maxElementCount ) = 0;
	virtual [[nodiscard]] IPhysicsCollisionSet		*FindCollisionSet( unsigned int id ) = 0;
...
public:
	...
	virtual [[nodiscard]] CPhysConvex		*ConvexFromVerts( Vector **pVerts, int vertCount ) = 0;
	// produce a convex element from planes (csg of planes)
	virtual [[nodiscard]] CPhysConvex		*ConvexFromPlanes( float *pPlanes, int planeCount, float mergeDistance ) = 0;
	// calculate volume of a convex element
	virtual [[nodiscard]] float			ConvexVolume( CPhysConvex *pConvex ) = 0;

	virtual [[nodiscard]] float			ConvexSurfaceArea( CPhysConvex *pConvex ) = 0;
	...
	virtual [[nodiscard]] CPhysConvex		*BBoxToConvex( const Vector &mins, const Vector &maxs ) = 0;
	// produce a convex element from a convex polyhedron
	virtual [[nodiscard]] CPhysConvex		*ConvexFromConvexPolyhedron( const CPolyhedron &ConvexPolyhedron ) = 0;
	...
	virtual [[nodiscard]] CPhysPolysoup	*PolysoupCreate( void ) = 0;
	...
	virtual [[nodiscard]] CPhysCollide *ConvertPolysoupToCollide( CPhysPolysoup *pSoup, bool useMOPP ) = 0;
	
	// Convert an array of convex elements to a compiled collision model (this deletes the convex elements)
	virtual [[nodiscard]] CPhysCollide	*ConvertConvexToCollide( CPhysConvex **pConvex, int convexCount ) = 0;
	virtual [[nodiscard]] CPhysCollide	*ConvertConvexToCollideParams( CPhysConvex **pConvex, int convexCount, const convertconvexparams_t &convertParams ) = 0;
	...
	virtual [[nodiscard]] size_t			CollideSize( CPhysCollide *pCollide ) = 0;
	// serialize the collide to a block of memory
	virtual [[nodiscard]] size_t			CollideWrite( char *pDest, CPhysCollide *pCollide, bool bSwap = false ) = 0;
	// unserialize the collide from a block of memory
	virtual [[nodiscard]] CPhysCollide	*UnserializeCollide( char *pBuffer, int size, int index ) = 0;

	// compute the volume of a collide
	virtual [[nodiscard]] float			CollideVolume( CPhysCollide *pCollide ) = 0;
	// compute surface area for tools
	virtual [[nodiscard]] float			CollideSurfaceArea( CPhysCollide *pCollide ) = 0;

	// Get the support map for a collide in the given direction
	virtual [[nodiscard]] Vector		CollideGetExtent( const CPhysCollide *pCollide, const Vector &collideOrigin, const QAngle &collideAngles, const Vector &direction ) = 0;
	...
	virtual [[nodiscard]] Vector			CollideGetOrthographicAreas( const CPhysCollide *pCollide ) = 0;
	...
	virtual [[nodiscard]] int				CollideIndex( const CPhysCollide *pCollide ) = 0;

	// Convert a bbox to a collide
	virtual [[nodiscard]] CPhysCollide	*BBoxToCollide( const Vector &mins, const Vector &maxs ) = 0;
	virtual [[nodiscard]] int				GetConvexesUsedInCollideable( const CPhysCollide *pCollideable, CPhysConvex **pOutputArray, int iOutputArrayLimit ) = 0;
	...
	virtual [[nodiscard]] bool			IsBoxIntersectingCone( const Vector &boxAbsMins, const Vector &boxAbsMaxs, const truncatedcone_t &cone ) = 0;
	...
	virtual [[nodiscard]] IVPhysicsKeyParser	*VPhysicsKeyParserCreate( const char *pKeyData ) = 0;
	...
	virtual [[nodiscard]] int				CreateDebugMesh( CPhysCollide const *pCollisionModel, Vector **outVerts ) = 0;
	...
	virtual [[nodiscard]] ICollisionQuery *CreateQueryModel( CPhysCollide *pCollide ) = 0;
	// destroy the queryable version
	virtual void			DestroyQueryModel( ICollisionQuery *pQuery ) = 0;

	virtual [[nodiscard]] IPhysicsCollision *ThreadContextCreate( void ) = 0;
	virtual void			ThreadContextDestroy( IPhysicsCollision *pThreadContex ) = 0;

	virtual [[nodiscard]] CPhysCollide	*CreateVirtualMesh( const virtualmeshparams_t &params ) = 0;
	virtual [[nodiscard]] bool			SupportsVirtualMesh() = 0;


	virtual [[nodiscard]] bool			GetBBoxCacheSize( size_t *pCachedSize, intp *pCachedCount ) = 0;

	
	// extracts a polyhedron that defines a CPhysConvex's shape
	virtual [[nodiscard]] CPolyhedron		*PolyhedronFromConvex( CPhysConvex * const pConvex, bool bUseTempPolyhedron ) = 0;

	// dumps info about the collide to Msg()
	virtual void			OutputDebugInfo( const CPhysCollide *pCollide ) = 0;
	virtual [[nodiscard]] unsigned int	ReadStat( int statID ) = 0;
};
...
public:
	...
	virtual [[nodiscard]] int		ConvexCount( void ) = 0;
	// triangle count for this convex piece
	virtual [[nodiscard]] int		TriangleCount( int convexIndex ) = 0;
	// get the stored game data
	virtual [[nodiscard]] unsigned int GetGameData( int convexIndex ) = 0;
	...
	virtual [[nodiscard]] int		GetTriangleMaterialIndex( int convexIndex, int triangleIndex ) = 0;
...
public:
	...
	virtual [[nodiscard]] bool VehiclePointInWater( const Vector &vecPoint ) = 0;
};
...
{
public:
	virtual [[nodiscard]] unsigned int GetContents( int convexGameData ) = 0;
};
...
public:
	...
	virtual [[nodiscard]] bool AllowsTranslation() = 0;
	virtual [[nodiscard]] bool AllowsRotation() = 0;
	...
	virtual [[nodiscard]] bool IsPhysicallyControlled() = 0;
	...
	virtual [[nodiscard]] float GetTargetPosition( Vector *pPositionOut, QAngle *pAnglesOut ) = 0;
	
	virtual [[nodiscard]] float GetTeleportDistance( void ) = 0;
...
public:
	...
	virtual [[nodiscard]] simresult_e	Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular ) = 0;
};
...
public:
	...
	virtual [[nodiscard]] intp CountObjects( void ) = 0;
...
{
public:
	virtual [[nodiscard]] int ShouldCollide( IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0, void *pGameData1 ) = 0;
	virtual [[nodiscard]] int ShouldSolvePenetration( IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0, void *pGameData1, float dt ) = 0;
	
	// pObject has already done the max number of collisions this tick, should we freeze it to save CPU?
	virtual [[nodiscard]] bool ShouldFreezeObject( IPhysicsObject *pObject ) = 0;

	// The system has already done too many collision checks, performance will suffer.
	// How many more should it do?
	virtual [[nodiscard]] int AdditionalCollisionChecksThisTick( int currentChecksDone ) = 0;
	...
	virtual [[nodiscard]] bool ShouldFreezeContacts( IPhysicsObject **pObjectList, int objectCount ) = 0;
};
...
{
public:
	virtual [[nodiscard]] bool ShouldHitObject( IPhysicsObject *pObject, int contentsMask ) = 0;
	virtual [[nodiscard]] PhysicsTraceType_t	GetTraceType() const = 0;
};
...
public:
	...
	virtual [[nodiscard]] IVPhysicsDebugOverlay *GetDebugOverlay( void ) = 0;
	...
	virtual [[nodiscard]] float			GetAirDensity( void ) const = 0;
	...
	virtual [[nodiscard]] IPhysicsFluidController	*CreateFluidController( IPhysicsObject *pFluidObject, fluidparams_t *pParams ) = 0;
	...
	virtual [[nodiscard]] IPhysicsSpring	*CreateSpring( IPhysicsObject *pObjectStart, IPhysicsObject *pObjectEnd, springparams_t *pParams ) = 0;
	...
	virtual [[nodiscard]] IPhysicsConstraint *CreateRagdollConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_ragdollparams_t &ragdoll ) = 0;
	virtual [[nodiscard]] IPhysicsConstraint *CreateHingeConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_hingeparams_t &hinge ) = 0;
	virtual [[nodiscard]] IPhysicsConstraint *CreateFixedConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_fixedparams_t &fixed ) = 0;
	virtual [[nodiscard]] IPhysicsConstraint *CreateSlidingConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_slidingparams_t &sliding ) = 0;
	virtual [[nodiscard]] IPhysicsConstraint *CreateBallsocketConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_ballsocketparams_t &ballsocket ) = 0;
	virtual [[nodiscard]] IPhysicsConstraint *CreatePulleyConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_pulleyparams_t &pulley ) = 0;
	virtual [[nodiscard]] IPhysicsConstraint *CreateLengthConstraint( IPhysicsObject *pReferenceObject, IPhysicsObject *pAttachedObject, IPhysicsConstraintGroup *pGroup, const constraint_lengthparams_t &length ) = 0;

	virtual void DestroyConstraint( IPhysicsConstraint * ) = 0;

	virtual [[nodiscard]] IPhysicsConstraintGroup *CreateConstraintGroup( const constraint_groupparams_t &groupParams ) = 0;
	virtual void DestroyConstraintGroup( IPhysicsConstraintGroup *pGroup ) = 0;

	virtual [[nodiscard]] IPhysicsShadowController *CreateShadowController( IPhysicsObject *pObject, bool allowTranslation, bool allowRotation ) = 0;
	virtual void						DestroyShadowController( IPhysicsShadowController * ) = 0;

	virtual [[nodiscard]] IPhysicsPlayerController	*CreatePlayerController( IPhysicsObject *pObject ) = 0;
	virtual void						DestroyPlayerController( IPhysicsPlayerController * ) = 0;

	virtual [[nodiscard]] IPhysicsMotionController	*CreateMotionController( IMotionEvent *pHandler ) = 0;
	virtual void						DestroyMotionController( IPhysicsMotionController *pController ) = 0;

	virtual [[nodiscard]] IPhysicsVehicleController	*CreateVehicleController( IPhysicsObject *pVehicleBodyObject, const vehicleparams_t &params, unsigned int nVehicleType, IPhysicsGameTrace *pGameTrace ) = 0;
	...
	virtual [[nodiscard]] float			GetNextFrameTime( void ) const = 0;
	...
	virtual [[nodiscard]] int				GetActiveObjectCount() const = 0;
	virtual void			GetActiveObjects( IPhysicsObject **pOutputObjectList ) const = 0;
	virtual [[nodiscard]] const IPhysicsObject **GetObjectList( intp *pOutputObjectCount ) const = 0;
	virtual [[nodiscard]] bool			TransferObject( IPhysicsObject *pObject, IPhysicsEnvironment *pDestinationEnvironment ) = 0;

	virtual void			CleanupDeleteList( void ) = 0;
	virtual void			EnableDeleteQueue( bool enable ) = 0;

	// Save/Restore methods
	virtual [[nodiscard]] bool			Save( const physsaveparams_t &params ) = 0;
	virtual void			PreRestore( const physprerestoreparams_t &params ) = 0;
	virtual [[nodiscard]] bool			Restore( const physrestoreparams_t &params ) = 0;
	virtual void			PostRestore() = 0;

	// Debugging:
	virtual [[nodiscard]] bool IsCollisionModelUsed( const CPhysCollide *pCollide ) const = 0;
	...
	virtual [[nodiscard]] unsigned int	GetObjectSerializeSize( IPhysicsObject *pObject ) const = 0;
	virtual void			SerializeObjectToBuffer( IPhysicsObject *pObject, unsigned char *pBuffer, unsigned int bufferSize ) = 0;
	virtual [[nodiscard]] IPhysicsObject *UnserializeObjectFromBuffer( void *pGameData, unsigned char *pBuffer, unsigned int bufferSize, bool enableCollisions ) = 0;
...
public:
	...
	virtual [[nodiscard]] bool			IsStatic() const = 0;
	virtual [[nodiscard]] bool			IsAsleep() const = 0;
	virtual [[nodiscard]] bool			IsTrigger() const = 0;
	virtual [[nodiscard]] bool			IsFluid() const = 0;		// fluids are special triggers with fluid controllers attached, they return true to IsTrigger() as well!
	virtual [[nodiscard]] bool			IsHinged() const = 0;
	virtual [[nodiscard]] bool			IsCollisionEnabled() const = 0;
	virtual [[nodiscard]] bool			IsGravityEnabled() const = 0;
	virtual [[nodiscard]] bool			IsDragEnabled() const = 0;
	virtual [[nodiscard]] bool			IsMotionEnabled() const = 0;
	virtual [[nodiscard]] bool			IsMoveable() const = 0;	 // legacy: IsMotionEnabled() && !IsStatic()
	virtual [[nodiscard]] bool			IsAttachedToConstraint(bool bExternalOnly) const = 0;
	...
	virtual [[nodiscard]] unsigned short	GetGameFlags( void ) const = 0;
	virtual void			SetGameIndex( unsigned short gameIndex ) = 0;
	virtual [[nodiscard]] unsigned short	GetGameIndex( void ) const = 0;
	...
	virtual [[nodiscard]] unsigned short	GetCallbackFlags( void ) const = 0;
	...
	virtual [[nodiscard]] float			GetMass( void ) const = 0;
	// get 1/mass (it's cached)
	virtual [[nodiscard]] float			GetInvMass( void ) const = 0;
	virtual [[nodiscard]] Vector			GetInertia( void ) const = 0;
	virtual [[nodiscard]] Vector			GetInvInertia( void ) const = 0;
	...
	// material index
	virtual [[nodiscard]] int				GetMaterialIndex() const = 0;
	virtual void			SetMaterialIndex( int materialIndex ) = 0;

	// contents bits
	virtual [[nodiscard]] unsigned int	GetContents() const = 0;
	...
	virtual [[nodiscard]] float			GetEnergy() const = 0;
	virtual [[nodiscard]] Vector			GetMassCenterLocalSpace() const = 0;
	...
	virtual [[nodiscard]] float			CalculateLinearDrag( const Vector &unitDirection ) const = 0;
	virtual [[nodiscard]] float			CalculateAngularDrag( const Vector &objectSpaceRotationAxis ) const = 0;
	...
	virtual [[nodiscard]] bool			GetContactPoint( Vector *contactPoint, IPhysicsObject **contactObject ) const = 0;
	...
	virtual [[nodiscard]] int				GetShadowPosition( Vector *position, QAngle *angles ) const = 0;
	virtual [[nodiscard]] IPhysicsShadowController *GetShadowController( void ) const = 0;
	...
	virtual [[nodiscard]] float			ComputeShadowControl( const hlshadowcontrol_params_t &params, float secondsToArrival, float dt ) = 0;


	virtual [[nodiscard]] const CPhysCollide	*GetCollide( void ) const = 0;
	...
	virtual [[nodiscard]] IPhysicsFrictionSnapshot *CreateFrictionSnapshot() = 0;
...
public:
	...
	virtual [[nodiscard]] IPhysicsObject *GetStartObject( void ) = 0;

	// Get the end object
	virtual [[nodiscard]] IPhysicsObject *GetEndObject( void ) = 0;
};
...
public:
	...
	virtual [[nodiscard]] intp		ParseSurfaceData( const char *pFilename, const char *pTextfile ) = 0;
	// current number of entries in the database
	virtual [[nodiscard]] intp		SurfacePropCount( void ) const = 0;

	virtual [[nodiscard]] intp		GetSurfaceIndex( const char *pSurfacePropName ) const = 0;
	...
	virtual [[nodiscard]] surfacedata_t	*GetSurfaceData( intp surfaceDataIndex ) = 0;
	virtual [[nodiscard]] const char		*GetString( unsigned short stringTableIndex ) const = 0;


	virtual [[nodiscard]] const char		*GetPropName( intp surfaceDataIndex ) const = 0;
...
public:
	...
	virtual [[nodiscard]] float	GetDensity() const = 0;
	virtual void	WakeAllSleepingObjects() = 0;
	virtual [[nodiscard]] int		GetContents() const = 0;
};
