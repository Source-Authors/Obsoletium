...
[[nodiscard]] _Check_return_ _Ret_notnull_ _Post_writable_byte_size_(nSize)
    __declspec(allocator) void* __cdecl
    operator new(_In_ size_t nSize, _In_ int nType,
                 _In_z_ const char* lpszFileName, _In_ int nLine);
...
#else
  return ::operator new(nSize, _AFX_CLIENT_BLOCK, "N/A", 0);
...
void PASCAL CObject::operator delete(void* pObject, LPCSTR /* lpszFileName */,
                                     int /* nLine */) {
#ifdef _AFX_NO_DEBUG_CRT
  free(pObject);
#else
...
BOOL AFXAPI _AfxDefaultAllocHook(size_t, BOOL, LONG) { return TRUE; }
...
#ifdef _DEBUG
  if ((pfnAllocHook)(nSize, nBlockUse == _AFX_CLIENT_BLOCK, lRequest))
#else
...
BOOL AFXAPI AfxEnableMemoryTracking(BOOL bTrack) {
  if (_afxMemoryLeakOverride) return TRUE;
...
CMemoryState::CMemoryState() { memset(this, 0, sizeof(*this)); }
...
BOOL CMemoryState::Difference(const CMemoryState& oldState,
  ...
  int nResult = _CrtMemDifference(&m_memState, &oldState.m_memState,
                                  &newState.m_memState);
  UpdateData();
  return nResult != 0;
}
...
void AFXAPI AfxDoForAllObjects(void(AFX_CDECL* pfn)(CObject*, void*),
                               void* pContext) {
  if (pfn == nullptr) {
...
BOOL AFXAPI AfxDumpMemoryLeaks() { return _CrtDumpMemoryLeaks(); }
...
int AFX_CDECL AfxNewHandler(size_t /* nSize */) { AfxThrowMemoryException(); }
...
[[nodiscard]] _Ret_notnull_ _Post_writable_byte_size_(nSize)
    __declspec(allocator) void* __cdecl operator new(size_t nSize) {
  void* pResult;
#ifdef _AFXDLL
  ...
  for (;;) {
    ...
    if (pfnNewHandler == nullptr || (*pfnNewHandler)(nSize) == 0) break;
#else
...
void __cdecl operator delete[](void* p) { ::operator delete(p); }

#ifdef _DEBUG

[[nodiscard]] _Check_return_ _Ret_notnull_ _Post_writable_byte_size_(nSize)
    __declspec(allocator) void* __cdecl operator new(_In_ size_t nSize,
                                                     _In_ int nType,
...
void __cdecl operator delete(void* p, int nType, LPCSTR /* lpszFileName */,
                             int /* nLine */) {
#if !defined(_AFX_NO_DEBUG_CRT) && defined(_DEBUG)
  _free_dbg(p, nType);
#else
...
[[nodiscard]] _Check_return_ _Ret_notnull_ _Post_writable_byte_size_(nSize)
    __declspec(allocator) void* __cdecl operator new[](
        _In_ size_t nSize, _In_ int nType, _In_z_ const char* lpszFileName,
        _In_ int nLine) {
