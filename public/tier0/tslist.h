...
class TSLIST_HEAD_ALIGN CTSListBase : public CAlignedNewDelete<TSLIST_HEAD_ALIGNMENT> {
 public:
  ...
  TSLNodeBase_t *Pop() {
    TSLHead_t next = {}, orig = std::atomic_load_explicit(
                             &head_, std::memory_order::memory_order_relaxed);
...
public:
	...
	CTSQueue()
	{
		...
		{
			Error( "CTSQueue: Misaligned queue head.\n" );
		}
		if ( ((size_t)&m_Tail) % TSLIST_HEAD_ALIGNMENT != 0 )
		{
			Error( "CTSQueue: Misaligned queue tail.\n" );
		}
