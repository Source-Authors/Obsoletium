...
(
	_In_ _CrtDoForAllClientObjectsCallback callback,
	_In_ void* context
)
{
	Assert(0);
...
(
	_In_       int            ReportType,
	_In_opt_   void*          ReturnAddress,
	_In_opt_z_ wchar_t const* FileName,
	_In_       int            LineNumber,
	_In_opt_z_ wchar_t const* ModuleName,
	_In_opt_z_ wchar_t const* Format,
	va_list                   ArgList
)
{
	Assert(0);
...
{
	return static_cast<_CRT_REPORT_HOOK>( g_pMemAlloc->CrtSetReportHook( pfnNewHook ) );
}
...
(
	_In_     int               Mode,
    _In_opt_ _CRT_REPORT_HOOKW PFnNewHook
)
{
	Assert(0);
	return 0;
