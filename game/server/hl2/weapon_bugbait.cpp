...
void CWeaponBugBait::Drop( const Vector &vecVelocity )
{
	...
	if ( m_hSporeTrail )
	{
		...
		// dimhotepus: Use correct color.
		pSporeExplosion->SetRenderColor( 0, 127, 64, 38 );
...
void CWeaponBugBait::ThrowGrenade( CBasePlayer *pPlayer )
{
	...
	CGrenadeBugBait *pGrenade = BugBaitGrenade_Create( vThrowPos, vec3_angle, vThrowVel, QAngle(600,random->RandomFloat(-1200,1200),0), pPlayer );
