...
void CResponseSystem::ParseOneResponse( const char *responseGroupName, ResponseGroup& group )
{
	...
	while ( TokenWaiting() )
	{
		...
		if ( !Q_stricmp( m_token, "weight" ) )
		{
			ParseToken();
			// dimhotepus: atof -> strtof
			newResponse.weight.SetFloat( strtof( m_token, nullptr ) );
			continue;
		}
		...
		if ( !Q_stricmp( m_token, "odds" ) )
		{
			...
			rp->odds = static_cast<short>(clamp( atoi( m_token ), 0, 100 ));
			continue;
		}
		...
		if ( !Q_stricmp( m_token, "soundlevel" ) )
		{
			...
			rp->soundlevel = TextToSoundLevel( m_token );
			continue;
		}
	...
	while ( TokenWaiting() || !gotbody )
	{
		...
		if ( !Q_stricmp( m_token, "{" ) )
		{
			...
			while ( 1 )
			{
				...
				auto idx = m_Criteria.Find( m_token );
		...
		else if ( !Q_stricmp( m_token, "weight" ) )
		{
			ParseToken();
			// dimhotepus: atof -> strtof.
			newCriterion.weight.SetFloat( strtof( m_token, nullptr ) );
		}
		else
		{
...
void CResponseSystem::ParseEnumeration( void )
{
	...
	while ( 1 )
	{
		...
		ParseToken();
		// dimhotepus: atof -> strtof.
		float value = strtof( m_token, nullptr );
...
void CResponseSystem::ParseRule( void )
{
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( m_token, "response" ) )
		{
			...
			{
				ParseToken();
				auto idx = m_Responses.Find( m_token );
