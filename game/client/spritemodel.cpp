						  ...
						  float *pfBottom, int *pw, int *ph, const wrect_t *prcSubRect)
{
	...
	*ph = rc.bottom - rc.top;
	...
	f = 1.0f / (float)pSprite->GetWidth();
	*pfLeft = ((float)rc.left + 0.5f) * f;
	*pfRight = ((float)rc.right - 0.5f) * f;

	f = 1.0f / (float)pSprite->GetHeight();
	*pfTop = ((float)rc.top + 0.5f) * f;
	*pfBottom = ((float)rc.bottom - 0.5f) * f;
