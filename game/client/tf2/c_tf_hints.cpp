	...
	if ( pHudSelection )
	{
		...
		if ( player )
		{
			for ( int slot = 0; slot < MAX_WEAPON_SLOTS; slot++ )
			{
				...
				if ( V_strieq( weapon->GetName(), GetWeaponType() ) )
				{
...
void CHintDeployWeapon::Think( void )
{
	...
	if ( V_strieq( weapon->GetClientClass()->m_pNetworkName, "CWeaponBuilder" ) )
	{
		m_bCompleted = true;
	}
}
...
void CHintBuilderSelection::Think( void )
{
	...
	if ( V_strieq( selection, GetSelection() ) )
	{
		m_bCompleted = true;
	}
}
	...
	if ( player )
	{
		for ( int slot = 0; slot < MAX_WEAPON_SLOTS; slot++ )
		{
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				if ( V_strieq( weapon->GetName(), type ) )
				{
					return weapon;
				}
			}
...
void CHintHudWeaponFlash::SetKeyValue( const char *key, const char *value )
{
	...
	if ( V_strieq( key, "weapon" ) )
	{
		SetWeaponName( value );

		ComputeTitle();
	}
	else if ( V_strieq( key, "weapontype" ) )
	{
...
C_TFBaseHint *C_TFBaseHint::CreateHint( int id, const char *subsection, int entity )
{
	...
	if ( pkvHintSystem )
	{
		...
		if ( pkvHint )
		{
			...
			if ( !classname || !classname[ 0 ] || V_strieq( classname, "default" ) )
			{
