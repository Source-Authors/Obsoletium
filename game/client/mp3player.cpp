...
// Singleton
static CMP3Player *g_pPlayer = nullptr;

constexpr inline char SOUND_ROOT[]{"sound"};
...
constexpr inline char MP3_DEFAULT_MP3DIR[]{"."};
...
void MP3Player_Create( vgui::VPANEL parent )
{
	...
	g_pPlayer = new CMP3Player( parent, "MP3Player" );
}
...
public:
	...
	void		AddSongToFileList( intp songIndex )
	{
		if ( m_pFileList )
		{
	...
	void		AddSongToPlayList( intp songIndex )
	{
		if ( m_pPlayList )
		{
	...
	void		RemoveSongFromPlayList( intp songIndex )
	{
		if ( m_pPlayList )
		{
...
MP3Dir_t *CMP3Player::FindOrAddSubdirectory( MP3Dir_t *parent, char const *dirname )
{
	...
	intp c = parent->m_Subdirectories.Count();
	for ( intp i = 0; i < c; ++i )
	{
	...
	{
		V_sprintf_safe( fullpath, "%s\\%s", parent->m_FullDirPath.String(), dirname );
	}
...
intp CMP3Player::AddSplitFileToDirectoryTree_R( intp songIndex, MP3Dir_t *parent, CUtlVector< CUtlSymbol >& splitList, int level )
{
...
intp CMP3Player::AddFileToDirectoryTree( SoundDirectory_t *dir, char const *relative )
{
	// AddSong
	intp songIndex = AddSong( relative, dir->GetIndex() );
		...
		do
		{
			if ( fn[0] != '.' && Q_strnicmp( fn, "_mp3", 4 ) )
			{
				if ( g_pFullFileSystem->FindIsDirectory( fh ) )
				{
					...
					if ( !Q_isempty( current ) )
					{
						V_sprintf_safe( nextdir, "%s/%s", current, fn );
					}
					else
					{
				...
				else
				{
					...
					if ( V_strieq( ext, "mp3" ) )
					{
						...
						{
							V_sprintf_safe( relative, "%s/%s", current + ssize( SOUND_ROOT ), fn );
						}
						else
						{
							if ( !Q_isempty( current ) )
							{
								V_sprintf_safe( relative, "%s/%s", current, fn );
							}
							else
							{
...
intp CMP3Player::FindSong( char const *relative )
{
	...
	for ( intp i = 0 ; i < c ; ++i )
	{
		const MP3File_t& f = m_Files[ i ];
		if ( f.filename == handle )
		{
			return i;
		}
	}
...
intp CMP3Player::AddSong( char const *relative, intp dirnum )
{
...
void CMP3Player::RecursiveAddToTree( MP3Dir_t *current, intp parentIndex )
{
	...
	intp c = current->m_Subdirectories.Count();
	for ( intp i = 0 ; i < c; ++i )
	{
...
void CMP3Player::PopulateTree()
{
	...
	for ( intp dirnum = 0; dirnum < dircount; ++dirnum )
	{
...
void CMP3Player::GetLocalCopyOfSong( const MP3File_t &f, OUT_Z_CAP(outlen) char *outsong, size_t outlen )
{
	outsong[ 0 ] = '\0';
	char fn[ 512 ];
	...
	if ( f.flags == MP3File_t::FLAG_FROMGAME )
	{
		V_FixSlashes( fn );
	...
	else
	{
		...
		V_sprintf_safe( mp3_temp_path, "sound/_mp3" );
		g_pFullFileSystem->CreateDirHierarchy( mp3_temp_path, "MOD" );

		char destpath[ 512 ];
		V_sprintf_safe( destpath, "%s/%s", engine->GetGameDirectory(), hexfilename );
		V_FixSlashes( destpath );

		char sourcepath[ 512 ];

		Assert( f.dirnum >= 0 && f.dirnum < m_SoundDirectories.Count() );
		SoundDirectory_t *sdir = m_SoundDirectories[ f.dirnum ];
		V_sprintf_safe( sourcepath, "%s/%s", sdir->m_Root.String(), fn );
		V_FixSlashes( sourcepath );
	...
	V_FixSlashes( outsong );
}

void CMP3Player::PlaySong( intp songIndex, float skipTime /*= 0.0f */ ) 
{
	...
	char drymix[ 512 ];
	V_sprintf_safe( drymix, "#%s", soundname );
	...
	V_sprintf_safe( durationstr, "0:00 / %i:%02i", m_nSongMinutes, m_nSongSeconds );
...
void CMP3Player::SelectedSongs( SongListSource_t from, CUtlVector< intp >& songIndexList )
{
	...
	for ( auto idx : songIndexList )
	{
		m_SelectedSongs.AddToTail( idx );
	}
}

void CMP3Player::OnPlay()
{
	intp c = m_SelectedSongs.Count();

	for ( intp i = 0 ; i < c; ++i )
	{
		intp songIndex = m_SelectedSongs[ i ];
...
void CMP3Player::OnTick()
{
	...
	if ( playing )
	{
		...
		if ( m_flSongDuration >= 0.001f )
		{
			...
			char durationstr[ 256 ];
			V_sprintf_safe( durationstr, "%i:%02i / %i:%02i", minutes, seconds, m_nSongMinutes, m_nSongSeconds );
...
void CMP3Player::GoToNextSong( int skip )
{
	...
	if ( m_PlayList.Count() > 0 )
	{
		if ( shuffle )
		{
			m_nCurrentPlaylistSong = random->RandomIntp( 0, m_PlayList.Count() - 1 );
		}
		else
		{
	...
	else
	{
		if ( shuffle )
		{
			nextSong = random->RandomIntp( 0, m_Files.Count() - 1 );
		}
		else
		{
...
void CMP3Player::PopulateLists()
{
	...
	intp c = dir->m_FilesInDirectory.Count();
	if ( !c )
	{
		return;
	}
...
MP3File_t *CMP3Player::GetSongInfo( intp songIndex )
{
	if ( songIndex < 0 || songIndex >= m_Files.Count() )
	{
		return NULL;
...
void CMP3Player::AddToPlayList( intp songIndex, bool playNow )
{
...
void CMP3Player::RemoveFromPlayList( intp songIndex )
{
...
void CMP3Player::RestoreSongs( KeyValues *songs )
{
	...
	for ( KeyValues *song = songs->GetFirstSubKey(); song != NULL; song = song->GetNextKey() )
	{
		...
		char shortname[ 512 ];
		V_strcpy_safe( shortname, song->GetString( "short", "" ) );
		
		char filename[ 512 ];
		V_strcpy_safe( filename, song->GetString( "filename", "" ) );
...
void CMP3Player::RestoreDirectories( KeyValues *dirs )
{
	for ( KeyValues *dir = dirs->GetFirstSubKey(); dir != NULL; dir = dir->GetNextKey() )
	{
		...
		if ( dirpath  )
		{
			intp sdi = FindSoundDirectory( dirpath );
			if ( sdi == m_SoundDirectories.InvalidIndex() )
			{
...
void CMP3Player::SaveDbFile( int level, CUtlBuffer& buf, MP3File_t *file, intp filenumber )
{
	bpr( level, buf, "file\n" );
	bpr( level, buf, "{\n" );

	bpr( level + 1, buf, "filenumber %zd\n", filenumber );
...
void CMP3Player::FlattenDirectoryFileList_R( MP3Dir_t *dir, CUtlVector< intp >& list )
{
	intp c = dir->m_FilesInDirectory.Count();
	for ( intp i = 0; i < c; ++i )
	{
		intp songIndex = dir->m_FilesInDirectory[ i ];
...
void CMP3Player::SaveDb( char const *filename )
{
	...
	{
		Warning( "Unable to open mp3 database '%s' for writing.\n", filename );
	}
}
		...
		if ( V_strieq( song->GetName(), "song" ) )
		{
			...
			if ( idx == -1 )
			{
				...
				else if ( dirname )
				{
					...
					intp dirnum = FindSoundDirectory( dirname );
					if ( dirnum == -1 )
					{
		...
		for ( intp i = 0; i < c; ++i )
		{
			...
			if ( g_pFullFileSystem->String( song.filename, fn ) )
			{
				...
				if ( song.dirnum >= 0 )
				{
					...
					V_strcpy_safe( dirname, sd->m_Root.String() );
				}
...
void CMP3Player::LoadSettings()
{
	...
	{
		Warning( "Unable to load mp3 player settings '%s'.\n", MP3_SETTINGS_FILE );
		return;
	}
...
void CMP3Player::OnDirectorySelected( KeyValues *params )
{
	...
	{
		char dir[ 512 ];
		V_strcpy_safe( dir, fullpath );
		V_StripTrailingSlash( dir );
...
void CMP3Player::SetPlayListSong( intp listIndex )
{
	if ( m_PlayList.Count() > 0 )
	{
...
// When a changelevel occurs, the engine calls StopAllSounds several times, and
// the OnTick handler thinks the song has finished playing and so it moves to
// the next song.  This causes the play list to skip ahead by > 1 song during a 
// level change.
class CMP3PlayerGameSystem : public CAutoGameSystem
{
public:
	CMP3PlayerGameSystem() = default;
