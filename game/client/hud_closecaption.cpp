...
public:
	...
	bool GetStream( OUT_Z_BYTECAP(bufSizeInBytes) wchar_t *buf, intp bufSizeInBytes )
	{
		...
		intp curlen = 0;
		intp maxlen = bufSizeInBytes / static_cast<intp>(sizeof(wchar_t));
		...
		for ( intp i = 0; i < c; ++i )
		{
			...
			V_wcscat( buf, caption->stream, bufSizeInBytes / static_cast<intp>(sizeof(wchar_t)) );
			if ( i < c - 1 ) 
			{
				V_wcscat( buf, L" ", bufSizeInBytes / static_cast<intp>(sizeof(wchar_t)) );
			}
