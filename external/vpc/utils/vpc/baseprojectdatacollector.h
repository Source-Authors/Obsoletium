...
#ifndef VPC_BASEPROJECTDATACOLLECTOR_H_
#define VPC_BASEPROJECTDATACOLLECTOR_H_
...
class CSpecificConfig {
 ...
 public:
  ...
  KeyValues *m_pKV;
  bool m_bFileExcluded;  // Is the file that holds this config excluded from the
                         // build?
...
class CFileConfig {
 public:
  ...
  CSpecificConfig *GetOrCreateConfig(const char *pConfigName,
                                     CSpecificConfig *pParentConfig);
 ...
 public:
  ...
  CUtlString
      m_Filename;  // "" if this is the config data for the whole project.
  int m_nInsertOrder;
};
...
// It just collects interesting file properties into KeyValues and then the
// project file generator is responsible for using that data to write out a
// project file.
//
class CBaseProjectDataCollector : public IBaseProjectGenerator {
 ...
 public:
  ...
  // Configuration data is specified in between these calls and inside
  // BeginPropertySection/EndPropertySection. If bFileSpecific is set, then the
  // configuration data only applies to the last file added.
  virtual void StartConfigurationBlock(const char *pConfigName,
                                       bool bFileSpecific);
  virtual void EndConfigurationBlock();

  // These functions are called when it enters a section like $Compiler,
  // $Linker, etc. In between the BeginPropertySection/EndPropertySection, it'll
  // call HandleProperty for any properties inside that section.
  //
  // If you pass pCustomScriptData to HandleProperty, it won't touch the global
  // parsing state - it'll parse the platform filters and property value from
  // pCustomScriptData instead.
  virtual bool StartPropertySection(configKeyword_e keyword,
                                    bool *pbShouldSkip = NULL);
  virtual void HandleProperty(const char *pProperty,
                              const char *pCustomScriptData = NULL);
  virtual void EndPropertySection(configKeyword_e keyword);

  // Files go in folders. The generator should maintain a stack of folders as
  // they're added.
  virtual void StartFolder(const char *pFolderName);
  virtual void EndFolder();

  // Add files. Any config blocks/properties between StartFile/EndFile apply to
  // this file only. It will only ever have one active file.
  ...
  virtual bool RemoveFile(
      const char *pFilename);  // returns ture if a file was removed

 public:
  // This is called in EndProject if bAutoCleanupAfterProject is set.
  void Term();
  static void DoStandardVisualStudioReplacements(const char *pStartString,
                                                 const char *pFullInputFilename,
                                                 char *pOut, int outLen);
 ...
 public:
  ...
  // Either m_BaseConfigData or one of the files.
  CUtlStack<CFileConfig *> m_CurFileConfig;
...
#endif  // VPC_BASEPROJECTDATACOLLECTOR_H_
