...
static char GetNextToken(void) {
  ...
  while (mExpression[mCurPosition] == ' ') ++mCurPosition;
...
//	Utility funcs
//-----------------------------------------------------------------------------
static void FreeNode(ExprNode* node) { delete node; }

static ExprNode* AllocateNode(void) { return new ExprNode; }
...
static bool IsIdentifierOrConstant(const char token) {
  ...
  if (token == '$') {
    ...
    int i = 0;
    mIdentifier[i++] = token;
    while ((isalnum(mExpression[mCurPosition]) ||
            mExpression[mCurPosition] == '_') &&
...
static void MakeExprNode(ExprTree& tree, char token, Kind kind, ExprTree left,
                         ExprTree right) {
  tree = AllocateNode();
  tree->left = left;
  tree->right = right;
...
//-----------------------------------------------------------------------------
bool EvaluateExpression(bool& result, const char* InfixExpression,
                        GetSymbolProc_t pGetSymbolProc) {
  if (!InfixExpression) return false;
