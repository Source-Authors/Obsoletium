...
const char *CVPC::GetTargetPlatformName() {
  for (int i = 0; i < m_Conditionals.Count(); i++) {
    ...
    if (pConditional->type == CONDITIONAL_PLATFORM &&
        pConditional->m_bDefined) {
...
bool CVPC::IsPlatformDefined(const char *pName) {
  for (int i = 0; i < m_Conditionals.Count(); i++) {
    if (m_Conditionals[i].type == CONDITIONAL_PLATFORM &&
        !V_stricmp(pName, m_Conditionals[i].name.String())) {
...
//	Case Insensitive
//-----------------------------------------------------------------------------
conditional_t *CVPC::FindOrCreateConditional(const char *pName, bool bCreate,
...
void CVPC::SetConditional(const char *pString, bool bSet) {
  ...
  conditional_t *pConditional =
      FindOrCreateConditional(pString, true, CONDITIONAL_CUSTOM);
  if (!pConditional) {
...
//-----------------------------------------------------------------------------
bool CVPC::ConditionHasDefinedType(const char *pCondition,
                                   conditionalType_e type) {
  char symbol[MAX_SYSTOKENCHARS];

  for (int i = 0; i < m_Conditionals.Count(); i++) {
    if (m_Conditionals[i].type != type) continue;
...
bool CVPC::ResolveConditionalSymbol(const char *pSymbol) {
  ...
  conditional_t *pConditional = FindOrCreateConditional(
      (char *)pSymbol + offset, false, CONDITIONAL_NULL);
  if (pConditional) {
...
bool CVPC::EvaluateConditionalExpression(const char *pExpression) {
  ...
  ResolveMacrosInConditional(pExpression, conditionalBuffer,
                             sizeof(conditionalBuffer));
  ...
  bool bValid = ExpressionHandler.Evaluate(
      bResult, conditionalBuffer, ::ResolveSymbol, ::SymbolSyntaxError);
  if (!bValid) {
