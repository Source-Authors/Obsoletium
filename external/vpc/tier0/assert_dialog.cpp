...
PLATFORM_INTERFACE bool ShouldUseNewAssertDialog() {
  static bool bMPIWorker =
      (_tcsstr(Plat_GetCommandLine(), _T("-mpi_worker")) != NULL);
  if (bMPIWorker) {
    return false;
  }
...
PLATFORM_INTERFACE bool DoNewAssertDialog(const tchar *pFilename, int line,
  ...
  if (AssertStack()) {
    ...
    Warning(
        "%s (%d) : Assertion callstack...(NOT IMPLEMENTED IN NEW LOGGING "
        "SYSTEM.)\n",
        pFilename, line);
  ...
  _snprintf(cmdString, sizeof(cmdString),
            "Assert() 0x%.8x File: %s\tLine: %d\t%s",
            &g_VXConsoleAssertReturnValue, pFilename, line, pExpression);
  XBX_SendRemoteCommand(cmdString, false);

  // We sent a synchronous message, so g_xbx_dbgVXConsoleAssertReturnValue
  // should have been overwritten by now
  if (g_VXConsoleAssertReturnValue == -1) {
    ...
    switch (g_VXConsoleAssertReturnValue) {
      ...
      default:
        // Error... just break
        XBX_Error(
            "DoNewAssertDialog: invalid Assert response returned from "
            "VXConsole - breaking to debugger");
        g_bBreak = true;
        break;
    }
  }
#elif defined(_PS3)
  // There are a few ways to handle this sort of assert behavior with the PS3 /
  // Target Manager API. One is to use a DebuggerBreak per usual, and then
  // SNProcessContinue in the TMAPI to make the game resume after a breakpoint.
  // (You can use snIsDebuggerPresent() to determine if the debugger is
  // attached, although really it doesn't matter here.) This doesn't work
  // because the DebuggerBreak() is actually an interrupt op, and so Continue()
  // won't continue past it -- you need to do that from inside the ProDG
  // debugger itself. Another is to wait on a mutex here and then trip it from
  // the TMAPI, but there isn't a clean way to trip sync primitives from TMAPI.
  ...
  if (!g_hTier0Instance || !ThreadInMainThread()) {
    int result = MessageBox(NULL, pExpression, "Assertion Failed",
