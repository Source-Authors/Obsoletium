...
//								WARNING: pchType must be a
//static (since we keep a copy of it around for a while) 			pvObj -
//Pointer to the object we represent
//			pchName -			Name of the individual object we
//represent 								WARNING: pchName must be a static (since we keep a copy of it around
//for a while)
//			pValObjectparent-	Our parent object (ie, the object
//that our object is a member of) 			pValObjectPrev -	Object that precedes us
//in the linked list (we're 								always added to the end)
//-----------------------------------------------------------------------------
void CValObject::Init(tchar *pchType, void *pvObj, tchar *pchName,
                      CValObject *pValObjectParent,
                      CValObject *pValObjectPrev) {
  m_nUser = 0;

  // Initialize pchType:
  if (NULL != pchType) {
    V_strncpy(m_rgchType, pchType,
              (int)(sizeof(m_rgchType) / sizeof(*m_rgchType)));
  } else {
    m_rgchType[0] = '\0';
  }
...
void CValObject::ClaimMemoryBlock(void *pvMem) {
  ...
  // If we have a parent object, let it know about the memory (it'll recursively
  // call up the tree)
