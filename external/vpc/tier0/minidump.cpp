...
// MiniDumpWriteDump() function declaration (so we can just get the function
...
//			minidumpType				- type of
//minidump to
// write. 			ptchMinidumpFileNameBuffer	- if not-NULL points
// to a writable tchar
// buffer 										  of length at
// least _MAX_PATH to contain the name of the written minidump file on return.
//-----------------------------------------------------------------------------
bool WriteMiniDumpUsingExceptionInfo(
    [[maybe_unused]] unsigned int uStructuredExceptionCode,
    ExceptionInfo_t *pExceptionInfo, uint32 minidumpType,
  ...
  if (pfnMiniDumpWrite) {
    ...
    ::GetModuleFileNameW(NULL, rgchModuleName,
                         sizeof(rgchModuleName) / sizeof(tchar));
#else
    ::GetModuleFileName(NULL, rgchModuleName,
                        sizeof(rgchModuleName) / sizeof(tchar));
#endif
...
#ifdef TCHAR_IS_WCHAR
    HANDLE hFile =
        ::CreateFileW(rgchFileName, GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
                      CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
    HANDLE hFile =
        ::CreateFile(rgchFileName, GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
                     CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
    ...
    if (hFile) {
      ...
      bMinidumpResult = (*pfnMiniDumpWrite)(
      ...
      if (bMinidumpResult) {
        ...
        if (ptchMinidumpFileNameBuffer) {
          ...
          while ((*(pTgt++) = *(pSrc++)) != tchar(0)) continue;
        }
      }
    ...
    if (!bMinidumpResult) {
      ...
      _sntprintf(rgchFailedFileName, ARRAYSIZE(rgchFailedFileName),
                 "(failed)%s", rgchFileName);
...
void InternalWriteMiniDumpUsingExceptionInfo(
    unsigned int uStructuredExceptionCode, ExceptionInfo_t *pExceptionInfo) {
  ...
  uint32 iType =
      MINIDUMP_WithDataSegs | MINIDUMP_WithIndirectlyReferencedMemory;
  if (!WriteMiniDumpUsingExceptionInfo(uStructuredExceptionCode, pExceptionInfo,
                                       iType)) {
...
static LONG STDCALL
ValveUnhandledExceptionFilter(_EXCEPTION_POINTERS *pExceptionInfo) {
  uint uStructuredExceptionCode =
      pExceptionInfo->ExceptionRecord->ExceptionCode;
  g_UnhandledExceptionFunction(uStructuredExceptionCode,
...
//-----------------------------------------------------------------------------
typedef void (*FnMiniDumpInternal_t)(unsigned int uStructuredExceptionCode,
...
void WriteMiniDump() {
  ...
  __except (g_pfnWriteMiniDump(0, (ExceptionInfo_t *)GetExceptionInformation()),
...
void CatchAndWriteMiniDump(FnWMain pfn, int argc, tchar *argv[]) {
  ...
  const _se_translator_function oldSetFunction{
