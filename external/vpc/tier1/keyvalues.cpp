...
const char *KeyValues::GetName(void) const {
  return this ? KeyValuesSystem()->GetStringForSymbol(MAKE_3_BYTES_FROM_1_AND_2(
...
//-----------------------------------------------------------------------------
KeyValues *KeyValues::GetFirstSubKey() { return this ? m_pSub : NULL; }

//-----------------------------------------------------------------------------
// Purpose: Return the next subkey
//-----------------------------------------------------------------------------
KeyValues *KeyValues::GetNextKey() { return this ? m_pPeer : NULL; }
  ...
  if (dat) {
    ...
    switch (dat->m_iDataType) {
      ...
      case TYPE_PTR:
        ...
        break;
      case TYPE_UINT64: {
        swprintf(wbuf, V_ARRAYSIZE(wbuf), L"%lld",
                 *((uint64 *)(dat->m_sValue)));
        SetWString(keyName, wbuf);
      } break;
...
bool KeyValues::Dump(IKeyValuesDumpContext *pDump, int nIndentLevel /* = 0 */) {
  ...
  // Dump values
  for (KeyValues *val = this ? GetFirstValue() : NULL; val;
       val = val->GetNextValue()) {
    if (!pDump->KvWriteValue(val, nIndentLevel + 1)) return false;
  }

  // Dump subkeys
  for (KeyValues *sub = this ? GetFirstTrueSubKey() : NULL; sub;
       sub = sub->GetNextTrueSubKey()) {
