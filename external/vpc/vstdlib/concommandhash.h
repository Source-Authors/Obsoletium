...
#ifndef VPC_VSTDLIB_CONCOMMANDHASH_H_
#define VPC_VSTDLIB_CONCOMMANDHASH_H_
...
class CConCommandHash {
 public:
  ...
  // Invalid handle.
  static CCommandHashHandle_t InvalidHandle(void) {
    return (CCommandHashHandle_t)~0;
  }
  ...
  inline ConCommandBase *&operator[](const CCommandHashIterator_t &iter) {
    return (*this)[iter.handle];
  }
  inline ConCommandBase *const &operator[](
      const CCommandHashIterator_t &iter) const {
    return (*this)[iter.handle];
  }
 ...
 protected:
  ...
  struct HashEntry_t {
    ...
    HashEntry_t() : m_uiKey(0), m_Data(nullptr) {};
  };
...
inline CConCommandHash::CCommandHashHandle_t CConCommandHash::Find(
    const char *name) const {
  return Find(name, HashStringCaseless(name));
}

inline ConCommandBase *&CConCommandHash::operator[](
    CCommandHashHandle_t hHash) {
  return (m_aDataPool[hHash].m_Data);
}

inline ConCommandBase *const &CConCommandHash::operator[](
    CCommandHashHandle_t hHash) const {
  return (m_aDataPool[hHash].m_Data);
}
...
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashIterator_t CConCommandHash::Next(
    const CConCommandHash::CCommandHashIterator_t &iter) const {
  const CCommandHashHandle_t invalidIndex = m_aDataPool.InvalidIndex();
  {
...
bool CConCommandHash::IsValidIterator(
    const CCommandHashIterator_t &iter) const {
  return ((iter.bucket >= 0) && (m_aDataPool.IsValidIndex(iter.handle)));
}

inline CConCommandHash::HashKey_t CConCommandHash::Hash(
    const ConCommandBase *cmd) {
...
#endif  // VPC_VSTDLIB_CONCOMMANDHASH_H_
