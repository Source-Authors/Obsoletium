...
static ConnectionRegistration_t
    s_pConnectionRegistration[ARRAYSIZE(g_pInterfaceGlobals) + 1];

void RegisterInterface(CreateInterfaceFn factory, const char *pInterfaceName,
                       void **ppGlobal) {
  if (!(*ppGlobal)) {
    *ppGlobal = factory(pInterfaceName, NULL);
    if (*ppGlobal) {
      Assert(s_nRegistrationCount < ARRAYSIZE(s_pConnectionRegistration));
      ConnectionRegistration_t &reg =
          s_pConnectionRegistration[s_nRegistrationCount++];
...
void ReconnectInterface(CreateInterfaceFn factory, const char *pInterfaceName,
  ...
  *ppGlobal = factory(pInterfaceName, NULL);
  ...
  for (int i = 0; i < s_nRegistrationCount; ++i) {
    ...
    if (reg.m_ppGlobalStorage != ppGlobal) continue;
  ...
  if (s_nRegistrationCount < 0) {
    ...
    for (int i = 0; i < nFactoryCount; ++i) {
      for (int j = 0; j < ARRAYSIZE(g_pInterfaceGlobals); ++j) {
        RegisterInterface(pFactoryList[i],
                          g_pInterfaceGlobals[j].m_pInterfaceName,
                          (void **)g_pInterfaceGlobals[j].m_ppGlobal);
      }
    }
  } else {
    ...
    // Warning("APPSYSTEM: ConnectInterfaces() was called twice for the same
    // DLL.\nThis is expected behavior in building reslists, but questionable
    // otherwise.\n");
    for (int i = 0; i < nFactoryCount; ++i) {
      for (int j = 0; j < ARRAYSIZE(g_pInterfaceGlobals); ++j) {
        ReconnectInterface(pFactoryList[i],
                           g_pInterfaceGlobals[j].m_pInterfaceName,
                           (void **)g_pInterfaceGlobals[j].m_ppGlobal);
      }
    }
...
void DisconnectInterfaces() {
  ...
  if (--s_nConnectionCount < 0) return;
...
void ReconnectInterface(CreateInterfaceFn factory, const char *pInterfaceName) {
  for (int i = 0; i < ARRAYSIZE(g_pInterfaceGlobals); ++i) {
    if (strcmp(g_pInterfaceGlobals[i].m_pInterfaceName, pInterfaceName))
      continue;
    ReconnectInterface(factory, g_pInterfaceGlobals[i].m_pInterfaceName,
                       (void **)g_pInterfaceGlobals[i].m_ppGlobal);
  }
}
