...
// Hidden. Doesn't appear in find or auto complete. Like DEVELOPMENTONLY, but
// can't be compiled out.
#define FCVAR_HIDDEN (1 << 4)

// ConVar only

// It's a server cvar, but we don't send the data since it's a
...
// This cvar's string cannot contain unprintable characters ( e.g.,
// used for player name etc ).
#define FCVAR_PRINTABLEONLY (1 << 10)
// If this is a FCVAR_SERVER, don't log changes to the log file /
...
// client (skipped for local client, of course). If a change is requested it
// must come from the console (i.e., no remote client changes). If a value is
// changed while a server is active, it's replicated to all connected clients
//
...
// (via IServerPluginHelpers::StartQueryCvarValue).
...
// used as a debugging tool necessary to check material system
// thread convars
