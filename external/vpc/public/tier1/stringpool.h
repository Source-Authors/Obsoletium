...
#ifndef VPC_TIER1_STRINGPOOL_H_
#define VPC_TIER1_STRINGPOOL_H_

#include "utlrbtree.h"
#include "utlvector.h"
#include "utlbuffer.h"

// Purpose: Allocates memory for strings, checking for duplicates first, reusing
// exising strings if duplicate found.
...
class CStringPool {
 public:
  ...
  // given current uses of CStringPool, which assume they can copy string
  // pointers without any refcounts.
...
class CCountedStringPool {
 public:
  // HACK: hash_item_t structure should not be public.
  struct hash_item_t {
    char *pString;
 ...
 public:
  CCountedStringPool(
      StringPoolCase_t caseSensitivity = StringPoolCaseInsensitive);
  ...
  // These are only reliable if there are less than 64k strings in your string
  // pool
