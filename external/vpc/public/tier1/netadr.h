...
#ifndef VPC_TIER1_NETADR_H_
#define VPC_TIER1_NETADR_H_
...
struct netadr_t {
  ...
  netadr_t(uint unIP, uint16 usPort) {
    SetIP(unIP);
    SetPort(usPort);
    SetType(NA_IP);
  }
  ...
  // members are public to avoid to much changes
  netadrtype_t type;
  unsigned char ip[4];
  unsigned short port;
};

#endif  // VPC_TIER1_NETADR_H_
