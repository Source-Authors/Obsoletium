...
#ifndef VPC_TIER1_KEYVALUES_H_
#define VPC_TIER1_KEYVALUES_H_
...
//
// Used in vgui for message parameters and resource files.  Destructor deletes
// all child KeyValues nodes Data is stored in key (string names) -
// (string/int/float) value pairs called nodes.

// About KeyValues Text File Format:
//
// It has 3 control characters '{', '}' and '"'.  Names and values may be
// quoted or 	not.  The quote '"' character must not be used within name or
// values, only for quoting whole tokens.  You may use escape sequences wile
// parsing and add within a quoted token a \" to add quotes within your name or
// token.
//
// When using Escape Sequence the parser must now that by setting
// KeyValues::UsesEscapeSequences(true), which it's off by default.
//
// Non-quoted tokens ends with a whitespace, '{', '}' and '"'.  So you may use
// '{' and '}' within quoted tokens, but not for non-quoted tokens.
//
...
// recursively.
//
// Whitespaces are space, return, newline and tabulator.  Allowed
// Escape sequences are \n, \t, \\, \n and \".  The number character '#' is used
// for macro purposes (eg #include), don't use it as first character in key
// names.
class KeyValues {
 public:
  ...
  // exhausted.  In general this is preferable for game code for performance
  ...
  // objects are allocated or it will result in undefined behavior.  If
  // you use the growable string table, you cannot share KeyValues pointers
  // directly with any other module.  You can serialize them across module
  // boundaries.  These limitations are acceptable in the Steam backend code
  // this option was written for, but may not be in other situations.  Make sure
  // to understand the implications before using this.
  static void SetUseGrowableStringTable(bool bUseGrowableTable);

  explicit KeyValues(const char *setName);

  // AutoDelete class to automatically free the keyvalues.  Simply construct it
  // with the keyvalues you allocated and it will free them when falls out of
  // scope.  When you decide that keyvalues shouldn't be deleted call
  // Assign(nullptr) on it.  If you constructed AutoDelete(nullptr) you can
  // later assign the keyvalues to be deleted with Assign(pKeyValues).
  class AutoDelete {
   public:
  ...
  //
  // AutoDeleteInline is useful when you want to hold your keyvalues object
  // inside and delete it right after using. You can also pass temporary
  // KeyValues object as an argument to a function by wrapping it into
  // KeyValues::AutoDeleteInline instance:   call_my_function(
  // KeyValues::AutoDeleteInline( new KeyValues( "test" ) ) )
  //
  class AutoDeleteInline : public AutoDelete {
   public:
    explicit inline AutoDeleteInline(KeyValues *pKeyValues)
        : AutoDelete(pKeyValues) {}
  ...
  bool LoadFromFile(IBaseFileSystem *filesystem, const char *resourceName,
                    ...
                    GetSymbolProc_t pfnEvaluateSymbolProc = NULL);
  ...
  KeyValues *CreateNewKey();  // creates a new key, with an autogenerated name.
                              // name is guaranteed to be an integer, of value 1
                              // higher than the highest other integer key name
  void AddSubKey(KeyValues *pSubkey);  // Adds a subkey. Make sure the subkey
                                       // isn't a child of some other keyvalues
  void RemoveSubKey(
      KeyValues *subKey);  // removes a subkey from the list, DOES NOT DELETE IT
  void InsertSubKey(int nIndex,
                    KeyValues *pSubKey);  // Inserts the given sub-key before
                                          // the Nth child location
  bool ContainsSubKey(
      KeyValues *pSubKey);  // Returns true if this key values contains the
                            // specified sub key, false otherwise.
  void SwapSubKey(KeyValues *pExistingSubKey,
                  KeyValues *pNewSubKey);  // Swaps an existing subkey for a new
                                           // one, DOES NOT DELETE THE OLD ONE
                                           // but takes ownership of the new one
  void ElideSubKey(
      KeyValues *pSubKey);  // Removes a subkey but inserts all of its children
                            // in its place, in-order (flattens a tree, like
                            // firing a manager!)
  ...
  //
  // These functions can be used to treat it like a true key/values tree instead
  // of confusing values with keys.
  //
  ...
  bool GetBool(const char *keyName = NULL, bool defaultValue = false) {
    return GetInt(keyName, defaultValue ? 1 : 0) ? true : false;
  }
  ...
  bool GetBool(int keySymbol, bool defaultValue = false) {
    return GetInt(keySymbol, defaultValue ? 1 : 0) ? true : false;
  }
  ...
  enum types_t : char {
    ...
    TYPE_COLOR,
    TYPE_UINT64,
    TYPE_COMPILED_INT_BYTE,  // hack to collapse 1 byte ints in the compiled
                             // format
  ...
  void UnpackIntoStructure(struct KeyValuesUnpackStructure const *pUnpackTable,
                           void *pDest);
  ...
  enum MergeKeyValuesOp_t {
    MERGE_KV_ALL,
    MERGE_KV_UPDATE,  // update values are copied into storage, adding new keys
                      // to storage or updating existing ones
    MERGE_KV_DELETE,  // update values specify keys that get deleted from
                      // storage
    MERGE_KV_BORROW,  // update values only update existing keys in storage,
                      // keys in update that do not exist in storage are
                      // discarded
  };
 ...
 protected:
  ...
  //&buffer, int indentLevel ); 	void WriteConvertedString( CUtlBuffer
  ...
  bool ReadAsBinaryPooledFormat(CUtlBuffer &buf, IBaseFileSystem *pFileSystem,
  ...
  uint32 m_iKeyNameCaseSensitive1 : 8;  // 1st part of case sensitive symbol
                                        // defined in KeyValueSystem
  ...
  uint16 m_iKeyNameCaseSensitive2;  // 2nd part of case sensitive symbol defined
                                    // in KeyValueSystem;
...
enum KeyValuesUnpackDestinationTypes_t {
  ...
  UNPACK_TYPE_FOUR_FLOATS,   // dest is an array of 4 floats. source is a string
                             // like "1 2 3 4"
