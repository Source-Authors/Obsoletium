...
#ifndef VPC_TIER1_UTLGRAPH_H_
#define VPC_TIER1_UTLGRAPH_H_
...
class CUtlGraph {
 public:
  ...
  static I InvalidIndex() {
    return CUtlMap<NodeID_t, vecEdges_t *>::InvalidIndex();
  }
...
bool CUtlGraph<T, C>::AddEdge(T SourceNode, T DestNode, C nCost) {
  ...
  FOR_EACH_VEC(vecEdges, iEdge) {
    if (vecEdges[iEdge].m_DestinationNode == iDstNode) return false;
  }
#endif
...
bool CUtlGraph<T, C>::RemoveEdge(T SourceNode, T DestNode) {
  ...
  if (!m_Nodes.IsValidIndex(iSrcNode)) return false;

  I iDstNode = m_Nodes.Find(DestNode);
  if (!m_Nodes.IsValidIndex(iDstNode)) return false;
...
void CUtlGraph<T, C>::CreatePathMatrix() {
  ...
  FOR_EACH_MAP_FAST(m_Nodes, iNode) {
    ...
    FOR_EACH_VEC(vecEdges, iEdge) {
      ...
      vecPaths[edge.m_DestinationNode].m_DestinationNode =
  ...
  for (int k = 0; k < n; ++k) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      int iFromIToK = k;
      bool bFromIToK =
          destMapFromI[iFromIToK].m_DestinationNode != InvalidIndex();
      CostType_t cIToK =
          (bFromIToK) ? destMapFromI[iFromIToK].m_EdgeCost : INT_MAX;

      for (int j = 0; j < n; ++j) {
        if (!m_Nodes.IsValidIndex(j)) continue;

        // Path from I to J already?
        int iFromIToJ = j;
        bool bFromIToJ =
            destMapFromI[iFromIToJ].m_DestinationNode != InvalidIndex();
        CostType_t cIToJ =
            (bFromIToJ) ? destMapFromI[iFromIToJ].m_EdgeCost : INT_MAX;

        // Path from K to J?
        int iFromKToJ = j;
        bool bFromKToJ =
            destMapFromK[iFromKToJ].m_DestinationNode != InvalidIndex();
        CostType_t cKToJ =
            (bFromKToJ) ? destMapFromK[iFromKToJ].m_EdgeCost : INT_MAX;
...
bool CUtlGraphVisitor<T, C>::Begin(T StartNode) {
  ...
  FOR_EACH_VEC(m_vecFringeQueue, iFringe) {
    m_vecNodesVisited[m_vecNodesVisited.AddToTail()] =
        m_vecFringeQueue[iFringe].m_DestinationNode;
  }
...
bool CUtlGraphVisitor<T, C>::Advance() {
  ...
  if (m_iVisiting >= m_vecVisitQueue.Count()) {
    ...
    FOR_EACH_VEC(m_vecVisitQueue, iNode) {
      ...
      FOR_EACH_VEC(vecEdges, iEdge) {
        Edge_t &edge = vecEdges[iEdge];
        if (m_vecNodesVisited.InvalidIndex() ==
            m_vecNodesVisited.Find(edge.m_DestinationNode)) {
          m_vecNodesVisited[m_vecNodesVisited.AddToTail()] =
              edge.m_DestinationNode;
