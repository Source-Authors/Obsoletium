...
#ifndef VPC_TIER1_UTLFIXEDMEMORY_H_
#define VPC_TIER1_UTLFIXEDMEMORY_H_
...
#define UTLFIXEDMEMORY_TRACK_ALLOC()                              \
  MemAlloc_RegisterAllocation("||Sum of all UtlFixedMemory||", 0, \
...
#define UTLFIXEDMEMORY_TRACK_FREE()                                   \
  if (!m_pMemory)                                                     \
    ...
    MemAlloc_RegisterDeallocation("||Sum of all UtlFixedMemory||", 0, \
                                  NumAllocated() * sizeof(T),         \
                                  NumAllocated() * sizeof(T), 0)
#else
...
class CUtlFixedMemory {
 ...
 public:
  ...
  bool IsValidIterator(const Iterator_t& it) const {
    return it.m_pBlockHeader && it.m_nIndex >= 0 &&
           it.m_nIndex < it.m_pBlockHeader->m_nBlockSize;
  }
...
#endif  // VPC_TIER1_UTLFIXEDMEMORY_H_
