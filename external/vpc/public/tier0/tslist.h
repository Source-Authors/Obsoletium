...
union TSLHead_t {
  struct Value_t {
    TSLNodeBase_t *Next;
// <sergiy> Depth must be in the least significant halfword when atomically
// loading into register,
//          to avoid carrying digits from Sequence. Carrying digits from Depth
//          to Sequence is ok, because Sequence can be pretty much random. We
//          could operate on both of them separately, but it could perhaps (?)
//          lead to problems with store forwarding. I don't know 'cause I didn't
//          performance-test or design original code, I'm just making it work on
//          PowerPC.
 ...
 public:
  ...
  TSLNodeBase_t *Push(TSLNodeBase_t *pNode) {
    ...
    for (;;) {
      ...
      newHead.value32.DepthAndSequence =
          oldHead.value32.DepthAndSequence + 0x10001;
  ...
  TSLNodeBase_t *Pop() {
    ...
    TSLNodeBase_t *pNode =
        (TSLNodeBase_t *)InterlockedPopEntrySListAcquire(&m_Head);
#else
    ...
    for (;;) {
      oldHead.value64 = m_Head.value64;
      if (!oldHead.value.Next) return NULL;
  ...
  TSLNodeBase_t *Detach() {
    ...
    do {
      ...
      // <sergiy> the reason for AND'ing it instead of poking a short into
      // memory
      //          is probably to avoid store forward issues, but I'm not sure
      //          because I didn't construct this code. In any case, leaving it
      //          as is on big-endian
      newHead.value32.DepthAndSequence =
          oldHead.value32.DepthAndSequence & 0xffff0000;
...
class TSLIST_HEAD_ALIGN CTSPool : public CTSListBase {
 ...
 public:
  ...
  FORCEINLINE T *Get() { return GetObject(); }
...
class TSLIST_HEAD_ALIGN CTSListWithFreeList : public CTSListBase {
 public:
  ...
  void FreeNode(Node_t *pNode) { m_FreeList.Push(pNode); }
...
class TSLIST_HEAD_ALIGN CTSQueue {
 public:
  ...
  Node_t *Push(Node_t *pNode) {
    ...
    FinishPush(pNode, oldTail);  // This can fail if another thread pushed
...
#define TSQUEUE_BAD_NODE_LINK ((Node_t *)INT_TO_POINTER(0xdeadbeef))
    ...
    for (;;) {
      head.value.sequence =
          *pHeadSequence;  // must grab sequence first, which allows condition
                           // below to ensure pNext is valid
      ThreadMemoryBarrier();  // need a barrier to prevent reordering of these
                              // assignments
      ...
      if (pNext &&
          head.value.sequence ==
              *pHeadSequence)  // Checking pNext only to force optimizer to not
                               // reorder the assignment to pNext and the
                               // compare of the sequence
      {
        if (bTestOptimizer) {
 ...
 private:
  ...
  bool InterlockedCompareExchangeNodeLink(NodeLink_t volatile *pLink,
    ...
    return ThreadInterlockedAssignIf64((int64 *)pLink, value.value64,
                                       comperand.value64);
  }
