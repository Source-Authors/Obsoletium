...
typedef union EVENT_MASK(MOB_load_replay) {
  struct {
    ...
    uint16 PARTIAL_DATA : 1;  // Replayed because of partially overlapped data
                              // access between the load and store operations },
    uint16 UNALGN_ADDR : 1;  // Replayed because the lower 4 bits of the linear
                             // address do not match between the load and store
                             // operations } }
  };
  uint16 flat;
...
typedef union EVENT_MASK(BSQ_cache_reference) {
  struct {
    ...
    uint16 WR_2ndL_MISS : 1;  // Writeback lookup from DAC misses the 2nd level
                              // cache } }
  };
  uint16 flat;
...
typedef union EVENT_MASK(FSB_data_activity) {
  struct {
    ...
    uint16
        DRDY_DRV : 1;  // Count when this processor drives data onto the bus },
    uint16
        DRDY_OWN : 1;  // Count when this processor reads data from the bus },
    ...
    uint16 DBSY_OWN : 1;    // Count when this processor reserves the bus for
                            // sampling data },
    uint16 DBSY_OTHER : 1;  // Count when the bus is reserved for driving data
                            // this processor will not sample } }
  };
  uint16 flat;
...
typedef union EVENT_MASK(WC_Buffer) {
  struct {
    ...
    uint16
        WCB_HITM_EVICT : 1;  // store encountered a Hit Modified condition } }
  };
  uint16 flat;
