...
/*
   ...
   ConMsg() etc. functions. There are channels defined in the new system through
   which all old messages are routed; see LOG_GENERAL, LOG_CONSOLE,
   LOG_DEVELOPER, etc.
        ...
        log channel macros:
                ...
                DEFINE_LOGGING_CHANNEL_NO_TAGS( LOG_ChannelName, "ChannelName",
   [Flags], [MinimumSeverity], [Color] );
   ...
   [Flags], [MinimumSeverity], [Color] ); ADD_LOGGING_CHANNEL_TAG( "Tag1" );
        ...
        by the first parameter (in this example, LOG_ChannelName).  This channel
   ID can be used by various LoggingSystem_** functions to manipulate the
   channel settings.

        The optional [Flags] parameter is an OR'd together set of
   LoggingChannelFlags_t values (default: 0).

        The optional [MinimumSeverity] parameter is the lowest threshold
        above which messages will be processed (inclusive).  The default is
   LS_MESSAGE, which results in all messages, warnings, and errors being logged.
        ...
        Logging channels can have their minimum severity modified by name, ID,
   or tag.

        Logging channels are not hierarchical since there are situations in
   which a channel needs to belong to multiple hierarchies.  Use tags to create
        ...
        3) Sentient robot to clean up the thousands of places using the
   old/legacy logging system.
*/
