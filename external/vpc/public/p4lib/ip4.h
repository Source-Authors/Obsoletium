...
#ifndef VPC_P4LIB_IP4_H_
#define VPC_P4LIB_IP4_H_
...
// descriptions should be limited to this size!
#define P4_MAX_INPUT_BUFFER_SIZE 16384
...
abstract_class IP4 : public IAppSystem {
 public:
  ...
  virtual bool SyncFile(
      const char *pFullPath,
      int nRevision =
          -1) = 0;  // default revision is to sync to the head revision
  ...
  virtual bool GetClientSpecForDirectory(const char *pFullPathDir,
                                         char *pClientSpec, int nMaxLen) = 0;
  ...
  // the search path. NOTE: All directories in a path need to use the same
  // clientspec or this function will return false. It returns false if it
  // didn't find a p4config file.
  ...
  virtual CUtlVector<P4File_t> &GetFileListUsingClientSpec(
      const char *pPath, const char *pClientSpec) = 0;
  ...
  // sets the name of the changelist to open files under, NULL for "Default"
  // changelist
