...
#ifndef VPC_TIER2_TIER2_H_
#define VPC_TIER2_TIER2_H_
...
// command line. Note that this function may modify argc/argv because it filters
// out arguments (like -allowdebug).
...
enum FadeMode_t {
  // These map directly to cpu_level, and g_aFadeData contains settings for each
  // given cpu_level. The exception is 'FADE_MODE_LEVEL', which refers to
  // level-specific values in the map entity.
  FADE_MODE_NONE = 0,
  FADE_MODE_LOW,
  FADE_MODE_MED,
  FADE_MODE_HIGH,
...
struct FadeData_t {
  // Size (height in pixels) above which objects start to fade in
  float m_flPixelMin;
  // Size (height in pixels) above which objects are fully faded in
  float m_flPixelMax;
  // Reference screen res w.r.t which the above pixel values were chosen
  float m_flWidth;
  // Scale factor applied before entity distance-based fade is calculated
