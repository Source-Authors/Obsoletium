...
class CP4 : public CBaseAppSystem<IP4> {
 public:
  ...
  virtual void GetDepotFilePath(char *depotFilePath, const char *filespec,
                                int size);
  virtual void GetClientFilePath(char *clientFilePath, const char *filespec,
                                 int size);
  virtual void GetLocalFilePath(char *localFilePath, const char *filespec,
                                int size);
  virtual CUtlVector<P4File_t> &GetFileList(const char *path);
  virtual CUtlVector<P4File_t> &GetFileListUsingClientSpec(
      const char *pPath, const char *pClientSpec);
  virtual void GetOpenedFileList(CUtlVector<P4File_t> &fileList,
                                 bool bDefaultChangeOnly);
  virtual void GetOpenedFileList(const char *pRootDirectory,
                                 CUtlVector<P4File_t> &fileList);
  virtual void GetOpenedFileListInPath(const char *pPathID,
                                       CUtlVector<P4File_t> &fileList);
  virtual void GetFileListInChangelist(unsigned int changeListNumber,
                                       CUtlVector<P4File_t> &fileList);

  virtual CUtlVector<P4Revision_t> &GetRevisionList(const char *path,
                                                    bool bIsDir);
  ...
  virtual bool SubmitFiles(int nCount, const char **ppFullPathList,
  ...
  virtual bool GetClientSpecForFile(const char *pFullPath, char *pClientSpec,
                                    int nMaxLen);
  virtual bool GetClientSpecForDirectory(const char *pFullPathDir,
                                         char *pClientSpec, int nMaxLen);
  virtual bool GetClientSpecForPath(const char *pPathId, char *pClientSpec,
                                    int nMaxLen);
  ...
  bool DepotFileToLocalFile(const char *pDepotFile, char *pLocalFile,
                            int nBufLen);
 ...
 private:
  ...
  typedef void (*PerforceOp_t)(int nCount, const char **ppFullPathList,
                               const char *pDescription);
  bool PerformPerforceOp(PerforceOp_t op, int nCount,
                         const char **ppFullPathList, const char *pDescription);
  bool PerformPerforceOp(const char *pOperation, int nCount,
                         const char **ppFullPathList);
  bool PerformPerforceOp(const char *pOperation, const char *pFullPath);

  bool PerformPerforceOpCurChangeList(const char *pOperation, int nCount,
                                      const char **ppFullPathList);
  bool PerformPerforceOpCurChangeList(const char *pOperation,
...
CScopedClientSpec::CScopedClientSpec(const char *pClientSpec) {
  ...
  V_strncpy(m_pOldClientSpec, s_p4.GetClientApi().GetClient().Text(),
            sizeof(m_pOldClientSpec));
...
CScopedDirClientSpec::CScopedDirClientSpec(const char *pFullPathDir) {
  m_bClientSpecNeedsRestore = false;
  char pClientSpec[MAX_PATH];
  if (s_p4.GetClientSpecForDirectory(pFullPathDir, pClientSpec,
                                     sizeof(pClientSpec))) {
    V_strncpy(m_pOldClientSpec, s_p4.GetClientApi().GetClient().Text(),
              sizeof(m_pOldClientSpec));
    if (V_stricmp(m_pOldClientSpec, pClientSpec)) {
...
//-----------------------------------------------------------------------------
// Purpose: utility function to split a typical P4 line output into var and
// value
//-----------------------------------------------------------------------------
static void SplitP4Output(const char *data, char *pszCmd, char *pszInfo,
                          int bufLen) {
...
class CDataRetrievalUser : public ClientUser {
 public:
  CUtlVector<T> &GetData() { return m_Data; }
  ...
  virtual void OutputRecord(T &obj, const char *pszVar,
                            const char *pszInfo) = 0;
...
class CFileUser : public CDataRetrievalUser<P4File_t> {
 public:
  ...
  void RetrieveFilesInChangelist(unsigned int changeListNumber,
                                 CUtlVector<P4File_t> &fileList) {
    BeginChangelistProcess();
    InitRetrievingData(&fileList);
    char changeListString[32];
    V_snprintf(changeListString, sizeof(changeListString), "%d",
               changeListNumber);
 ...
 private:
  void ComputeLocalFileNames(CUtlVector<P4File_t> &fileList) {
    ...
    V_snprintf(pMatchPattern, sizeof(pMatchPattern), "//%s/",
               s_p4.GetClientApi().GetClient().Text());
    int nLen = V_strlen(pMatchPattern);

    int nCount = fileList.Count();
    for (int i = 0; i < nCount; ++i) {
      if (fileList[i].m_bDir) continue;

      const char *pClientPath = s_p4.String(fileList[i].m_sClientFile);
      if (V_stristr(pClientPath, pMatchPattern) != pClientPath) continue;

      char pLocalPath[MAX_PATH];
      V_ComposeFileName(s_p4.GetLocalRoot(), pClientPath + nLen, pLocalPath,
                        sizeof(pLocalPath));
  ...
  void ComputeLocalDirectoryNames(CUtlVector<P4File_t> &fileList) {
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      if (!s_p4.DepotFileToLocalFile(pDepotPath, pLocalPath,
                                     sizeof(pLocalPath)))
        continue;
  ...
  void OutputRecordInternal(P4File_t &file, const char *szCmd,
                            const char *szInfo) {
    if (!V_strcmp(szCmd, "headRev")) {
    ...
    } else if (m_bChangesRecord &&
               (!V_strcmp(szCmd, "user") || !V_strcmp(szCmd, "client") ||
                !V_strcmp(szCmd, "time") || !V_strcmp(szCmd, "desc") ||
                !V_strcmp(szCmd, "status") || !V_strcmp(szCmd, "oldChange") ||
      ...
      if (!V_stricmp(szCmd, "dir")) {
        ...
        if (m_bChangesRecord) {
          char pLocalPath[MAX_PATH];
          if (s_p4.DepotFileToLocalFile(szInfo, pLocalPath,
                                        sizeof(pLocalPath))) {
  ...
  virtual void OutputRecord(P4File_t &file, const char *szCmd,
                            const char *szInfo) {
    P4File_t *pFile = &file;
    if (m_bChangesRecord) {
      char tmpCmd[1024];
...
class CRevisionHistoryUser : public CDataRetrievalUser<P4Revision_t> {
 public:
  void RetrieveHistory(const char *path, bool bDir) {
    ...
    // set to view long output, to show the time, and to have a maximum number
    // of results
 ...
 private:
  virtual void OutputRecord(P4Revision_t &revision, const char *szCmd,
                            const char *szInfo) {
...
class CClientSpecUser : public CDataRetrievalUser<P4Client_t> {
 ...
 private:
  virtual void OutputRecord(P4Client_t &client, const char *szCmd,
                            const char *szInfo) {
...
class CClientspecMap {
 public:
  ...
  void AddVarToFullClientSpec(const char *variable, const char *data) {
    V_snprintf(&m_szFullClientspec[m_iFullClientspecWritePosition],
               CLIENTSPEC_BUFFER_SIZE - m_iFullClientspecWritePosition,
               "%s: %s\n\n", variable, data);
    m_iFullClientspecWritePosition +=
        strlen(&m_szFullClientspec[m_iFullClientspecWritePosition]);
  }

  void AddStringToFullClientSpec(const char *szString) {
    V_strncpy(&m_szFullClientspec[m_iFullClientspecWritePosition], szString,
              CLIENTSPEC_BUFFER_SIZE - m_iFullClientspecWritePosition);
    m_iFullClientspecWritePosition +=
        strlen(&m_szFullClientspec[m_iFullClientspecWritePosition]);
  }
  ...
  void ReadViewLine(const char *viewLine) {
    CUtlBuffer parse(viewLine, V_strlen(viewLine),
    ...
    if (V_strlen(record.m_szDepotPath) < 1) return;
    ...
    if (record.m_szDepotPath[0] == '-') {
      ...
      record.m_bNegative = true;
      memmove(record.m_szDepotPath, record.m_szDepotPath + 1,
              sizeof(record.m_szDepotPath) - 1);
    }
  ...
  void ReadClientspec(const char *clientspec) {
    ...
    if (pStr) {
      ...
      CUtlBuffer parse(pStr, V_strlen(pStr),
                       CUtlBuffer::TEXT_BUFFER | CUtlBuffer::READ_ONLY);
    ...
    if (pStr) {
      ...
      while (parse.IsValid()) {
        ...
        // get the depot path
        parse.GetString(record.m_szDepotPath, sizeof(record.m_szDepotPath));
        if (V_strlen(record.m_szDepotPath) < 1) break;
  ...
  void WriteClientspec(char *pszDest, int destSize) {
    ...
    for (int i = 0; i < m_PathMap.Count(); i++) {
      ...
      V_strncat(szView, record.m_szDepotPath, sizeof(szView),
                COPY_ALL_CHARACTERS);
      V_strncat(szView, " ", sizeof(szView), COPY_ALL_CHARACTERS);
      V_strncat(szView, record.m_szClientPath, sizeof(szView),
                COPY_ALL_CHARACTERS);
    ...
    if (pStr) {
      ...
      V_strncpy(pDest, szView,
                sizeof(m_szFullClientspec) - (pDest - m_szFullClientspec));
    }
  ...
  void RemovePathFromClient(const char *path) {
    ...
    for (int i = 0; i < m_PathMap.Count(); i++) {
      // look for the first path that matches, comparing the two strings up to
      // before the "/..." in szDepotPath
      if (!V_strnicmp(m_PathMap[i].m_szDepotPath, path,
                      V_strlen(m_PathMap[i].m_szDepotPath) - 4)) {
        ...
        V_snprintf(negativeRecord.m_szDepotPath,
                   sizeof(negativeRecord.m_szDepotPath), "%s/...", path);
        ...
        int iPos = 0;
        while (baseRecord.m_szDepotPath[iPos] ==
               negativeRecord.m_szDepotPath[iPos])
          ++iPos;

        char *pszNegPaths = negativeRecord.m_szDepotPath + iPos;

        // append the neg paths to the existing baseRecord.szClientPath
        V_strncpy(negativeRecord.m_szClientPath, baseRecord.m_szClientPath,
                  sizeof(negativeRecord.m_szClientPath));
        ...
        V_strncat(negativeRecord.m_szClientPath, "/",
                  sizeof(negativeRecord.m_szClientPath), COPY_ALL_CHARACTERS);
        V_strncat(negativeRecord.m_szClientPath, pszNegPaths,
                  sizeof(negativeRecord.m_szClientPath), COPY_ALL_CHARACTERS);
  ...
  void GetCommonDepotRoot(char *pszDest, int destSize) {
    ...
    for (int i = 1; i < nCount; i++) {
      if (m_PathMap[i].m_bNegative) continue;

      // see how much we compare
      for (char *pszD = pszDest, *pszS = m_PathMap[i].m_szDepotPath;
           *pszD && *pszS; ++pszD, ++pszS) {
        if (*pszD != *pszS) {
          *pszD = 0;
          break;
...
class CClientspecEditUser : public ClientUser {
 public:
  ...
  bool m_bReadDataVar;  // if we encountered a "data" variable, turn off the
                        // other parsing
...
class CChangelistCreateUser : public CDataRetrievalUser<int> {
 public:
  void CreateChangelist(const char *pDescription) {
    ...
    m_pDescription = (pDescription && pDescription[0])
                         ? pDescription
                         : "I'm a loser who didn't type a description. Mock me "
 ...
 private:
  ...
  virtual void InputData(StrBuf *strbuf, Error *e) {
    ...
    V_snprintf(svChangelist, sizeof(svChangelist),
               "Change:\tnew\n\nClient:\t%s\n\nUser:\t%s\n\nStatus:\tnew\n\n"
               "Description:\n\t%s\n\nFiles:\n\n",
               activeClient.m_sName.String(), activeClient.m_sUser.String(),
               m_pDescription);
  ...
  virtual void OutputRecord(int &nChangelist, const char *szCmd,
                            const char *szInfo) {
    if (!V_strcmp(szCmd, "Change")) {
...
class CChangelistFindUser : public CDataRetrievalUser<ChangelistDesc_t> {
 public:
  void ListChangelists(void) {
    ...
    char *argv[] = {"-l",      "-t", "-s",
                    "pending", "-c", s_p4.GetClientApi().GetClient().Text(),
                    NULL};
 ...
 private:
  virtual void OutputRecord(ChangelistDesc_t &cl, const char *szCmd,
                            const char *szInfo) {
    if (!V_strcmp(szCmd, "change")) {
      cl.id = atoi(szInfo);
    } else if (!V_strcmp(szCmd, "time")) {
...
class CErrorHandlerUser : public ClientUser {
 public:
  ...
  virtual void OutputError(const char *errBuf) {
    m_errorSeverity = max(
        m_errorSeverity,
        E_WARN);  // this is a guess - it could have been E_FATAL or E_FAILED
  ...
  virtual void HandleError(Error *err) {
    ...
    if (V_strstr(buf.Text(), "can't edit exclusive file already opened")) {
      m_errorSeverity = max(
          m_errorSeverity, E_WARN);  // s_p4 sends this is an info message, even
                                     // though the file doesn't get edited!
    }
  ...
  // even though the s_p4 documentation claims that HandleError (and therefore
  // OutputError) should get called via the defalt Message implementation
  virtual void Message(Error *err) { HandleError(err); }

  void ResetErrorState() {
    m_errorSeverity = E_EMPTY;
    m_errorBuf.Clear();
  }
  ErrorSeverity GetErrorState() { return m_errorSeverity; }
  const char *GetErrorString() { return m_errorBuf.Text(); }
  ...
  enum Flags {
    ...
    eFilterClUselessSpew = 1 << 1,  // Filter "already opened", "add of
                                    // exisiting", "can't change" msgs
  };

  // Sets the new flag mask, add prevails over remove if the flag specified in
  // both. SetFlags( x, 0 ) - enables flag x SetFlags( 0, x ) - removes flag x
  // SetFlags( mask, ~0 ) - completely sets the flag mask
  uint32 SetFlags(uint32 uiAdd, uint32 uiRemove) {
    uint32 uiOld = m_uiFlags;
    m_uiFlags = ((m_uiFlags & ~uiRemove) | uiAdd);
    return uiOld;
  }
...
bool CErrorHandlerUser::ShallOutputErrorStringBuffer(StrBuf &buf) {
  ...
  if (m_uiFlags & eFilterClUselessSpew) {
    if (V_strstr(pText, "already opened for edit")) return false;
    if (V_strstr(pText, "already opened for add")) return false;
    if (V_strstr(pText, "currently opened for edit")) return false;
    if (V_strstr(pText, "currently opened for add")) return false;
    if (V_strstr(pText, "add of existing file")) return false;
    if (V_strstr(pText, "add existing file")) return false;
    if (V_strstr(pText, "can't change from")) return false;
    if (V_strstr(pText, "no such file")) return false;
    if (V_strstr(pText, "not on client")) return false;
  }
...
class CSubmitUser : public ClientUser {
 public:
  void Submit(int nChangeList) {
    ...
    // We have to set up the description when creating the changelist in the
    // first place
    m_pDescription = NULL;
    char pBuf[128];
...
find_changelists:
  ...
  for (int k = 0; k < g_ChangelistFindUser.GetData().Count(); ++k) {
    ...
    if (symDesc == cl.m_sDescription) mapFoundChangelists.Insert(cl.id, &cl);
  }

  if (mapFoundChangelists.Count())
    iFoundChangelist =
        mapFoundChangelists.Key(mapFoundChangelists.FirstInorder());
  ...
  if (iFoundChangelist > 0) {
    ...
    iCreatedEmptyCl = g_ChangelistCreateUser.GetData().Count()
                          ? g_ChangelistCreateUser.GetData()[0]
                          : 0;
    if (!iCreatedEmptyCl) return 0;

    // Now we have a changelist created, check again to avoid multithreading
    // bugs
...
void CP4::RefreshClientData() {
  if (!m_bConnectedToServer) return;
...
// Convert a depot file to a local file user the current client mapping
//-----------------------------------------------------------------------------
bool CP4::DepotFileToLocalFile(const char *pDepotFile, char *pLocalPath,
                               int nBufLen) {
  // Need to construct valid local paths since opened doesn't do it for us
  char pClientPattern[MAX_PATH];
  V_snprintf(pClientPattern, sizeof(pClientPattern), "//%s/",
             m_Client.GetClient().Text());
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (V_stristr(pDepotFile, pMatchingString) != pDepotFile) continue;

    // Skip subdirectories if it's in the directory
    if (bInDirectory) {
      const char *pRelativePath = pDepotFile + nLen - 1;
      if (strchr(pRelativePath, '\\') || strchr(pRelativePath, '/')) continue;
    }
    ...
    if (!bClientRecursive && !bClientInDirectory) continue;

    char pTruncatedClientPath[MAX_PATH];
    V_strncpy(pTruncatedClientPath, pClientPath,
              bClientRecursive ? nPathLen - 2 : nPathLen);
    ...
    V_strncpy(szLocalRootWithoutSlashes, s_p4.GetLocalRoot(),
              sizeof(szLocalRootWithoutSlashes));
    V_StripTrailingSlash(szLocalRootWithoutSlashes);

    if (nClientLen < nPathLen - 3) {
      V_snprintf(pLocalPath, nBufLen, "%s\\%s%s", szLocalRootWithoutSlashes,
                 pTruncatedClientPath + nClientLen,
                 bRecursive ? pDepotFile + nLen - 3 : pDepotFile + nLen - 1);
    } else {
      V_snprintf(pLocalPath, nBufLen, "%s\\%s", szLocalRootWithoutSlashes,
                 bRecursive ? pDepotFile + nLen - 3 : pDepotFile + nLen - 1);
    }
    V_FixSlashes(pLocalPath);
    nMatchCount = nLen;
...
//-----------------------------------------------------------------------------
const char *CP4::String(CUtlSymbol s) const { return s.String(); }
...
int CP4::GetDepotRootLength() {
  if (!IsConnectedToServer()) return -1;
...
//-----------------------------------------------------------------------------
void CP4::GetDepotFilePath(char *depotFilePath, const char *filespec,
                           int size) {
  ...
  V_strncpy(depotFilePath, g_WhereUser.GetData()[0].m_sDepotFile.String(),
            size);
}
...
//-----------------------------------------------------------------------------
void CP4::GetClientFilePath(char *clientFilePath, const char *filespec,
                            int size) {
  if (!IsConnectedToServer()) {
    if (size > 0) {
      clientFilePath[0] = '\0';
  ...
  g_WhereUser.RetrieveWhereabouts(filespec);
  V_strncpy(clientFilePath, g_WhereUser.GetData()[0].m_sClientFile.String(),
            size);
}
...
//-----------------------------------------------------------------------------
void CP4::GetLocalFilePath(char *localFilePath, const char *filespec,
                           int size) {
  if (!IsConnectedToServer()) {
    if (size > 0) {
      localFilePath[0] = '\0';
  ...
  g_WhereUser.RetrieveWhereabouts(filespec);
  V_strncpy(localFilePath, g_WhereUser.GetData()[0].m_sLocalFile.String(),
            size);
}
...
//-----------------------------------------------------------------------------
CUtlVector<P4File_t> &CP4::GetFileListUsingClientSpec(const char *pPath,
...
void CP4::GetFileListInChangelist(unsigned int changeListNumber,
                                  CUtlVector<P4File_t> &fileList) {
  if (!IsConnectedToServer()) {
    fileList.RemoveAll();
    return;
...
void CP4::GetOpenedFileList(const char *pRootDirectory,
                            CUtlVector<P4File_t> &fileList) {
  if (!IsConnectedToServer()) {
    fileList.RemoveAll();
    return;
...
void CP4::GetOpenedFileListInPath(const char *pPathID,
                                  CUtlVector<P4File_t> &fileList) {
  if (!IsConnectedToServer()) {
    fileList.RemoveAll();
    return;
...
//-----------------------------------------------------------------------------
P4Client_t &CP4::GetActiveClient() { return m_ActiveClient; }
...
//-----------------------------------------------------------------------------
// Finds a client spec for a directory. Is destructive to the passed-in
// directory
//-----------------------------------------------------------------------------
bool CP4::GetClientSpecForDirectory(const char *pFullPathDir, char *pClientSpec,
                                    int nMaxLen) {
  ...
  do {
    ...
    if (FileExists(pP4ConfigPath)) {
      char temp[1024];
      CUtlBuffer buf(temp, sizeof(temp),
                     CUtlBuffer::TEXT_BUFFER | CUtlBuffer::EXTERNAL_GROWABLE);
      if (ReadFile(pP4ConfigPath, NULL, buf)) {
        while (buf.IsValid()) {
          char token[256], value[256];
          if (!buf.ParseToken(&breaks, token, sizeof(token))) break;
          if (!buf.GetToken("=")) break;
          if (!buf.ParseToken(&breaks, value, sizeof(value))) break;
...
//-----------------------------------------------------------------------------
bool CP4::GetClientSpecForFile(const char *pFullPath, char *pClientSpec,
                               int nMaxLen) {
...
//-----------------------------------------------------------------------------
bool CP4::GetClientSpecForPath(const char *pPathId, char *pClientSpec,
                               int nMaxLen) {
  if (!IsConnectedToServer()) {
    if (nMaxLen > 0) {
      pClientSpec[0] = '\0';
...
#else
  if (g_pFileSystem->GetSearchPath(pPathId, false, pPathBuf,
                                   sizeof(pPathBuf)) == 0)
    return false;
#endif
...
void CP4::SetOpenFileChangeList(const char *pChangeListName) {
  if (!m_sChangeListName.Length() || !pChangeListName || !*pChangeListName ||
...
char const *CP4::GetOpenFileChangeListNum() {
  if (m_sChangeListName.Length()) {
    if (!m_nCachedChangeListNumber) {
      m_nCachedChangeListNumber =
          FindOrCreateChangelist(m_sChangeListName.String());
      if (m_nCachedChangeListNumber)
        m_sCachedChangeListNum.Format("%d", m_nCachedChangeListNumber);
    }

    if (m_nCachedChangeListNumber) return m_sCachedChangeListNum.String();
  }
  return NULL;
}
...
static bool MakeFilesWritable(int nCount, const char **ppFullPathList) {
  ...
  for (int k = 0; k < nCount; ++k) {
    ...
    if (!access(szFile, 02)) continue;
    if (!chmod(szFile, _S_IWRITE | _S_IREAD)) continue;
    bResult = false;
  }
...
bool CP4::OpenFileForEdit(const char *fullpath) {
  if (!PerformPerforceOpCurChangeList("edit", fullpath)) return false;
...
bool CP4::SyncFile(const char *pFullPath, int nRevision) {
  ...
  if (nRevision >= 0) {  // sync to a specific revision
    V_snprintf(szFileOptions, sizeof(szFileOptions), "%s#%d", pFullPath,
               nRevision);
  } else {  // sync to the head revision
...
// Helper for operations on multiple files
//-----------------------------------------------------------------------------
bool CP4::PerformPerforceOp(PerforceOp_t op, int nCount,
                            const char **ppFullPathList,
                            const char *pDescription) {
  ...
  V_strncpy(pOldClientSpec, m_Client.GetClient().Text(),
            sizeof(pOldClientSpec));
  ...
  while (nFirstIndex < nCount) {
    ...
    for (i = nFirstIndex; i < nCount; ++i) {
      if (s_p4.GetClientSpecForFile(ppFullPathList[i], pClientSpec,
                                    sizeof(pClientSpec))) {
...
void SimplePerforceOpCurChangeList(int nCount, const char **ppFullPathList,
  ...
  if (char const *szClNum = s_p4.GetOpenFileChangeListNum()) {
    ...
    arrArgv[0] = "-c";
    arrArgv[1] = szClNum;
    for (int k = 0; k < nCount; ++k) arrArgv[2 + k] = ppFullPathList[k];
    arrArgv[nCount + 2] = NULL;

    s_p4.GetClientApi().SetArgv(nCount + 2,
                                const_cast<char **>(arrArgv.Base()));
    s_p4.GetClientApi().Run(s_pOperation, &g_ErrorHandlerUser);
  } else {
    s_p4.GetClientApi().SetArgv(nCount, const_cast<char **>(ppFullPathList));
    s_p4.GetClientApi().Run(s_pOperation, &g_ErrorHandlerUser);
...
bool CP4::PerformPerforceOpCurChangeList(const char *pOperation, int nCount,
                                         const char **ppFullPathList) {
  if (m_sChangeListName.Length()) {
    s_pOperation = pOperation;
    return PerformPerforceOp(SimplePerforceOpCurChangeList, nCount,
                             ppFullPathList, NULL);
  } else {
...
bool CP4::PerformPerforceOpCurChangeList(const char *pOperation,
                                         const char *pFullPath) {
  g_ErrorHandlerUser.ResetErrorState();
...
void SubmitPerforceOp(int nCount, const char **ppFullPathList,
                      const char *pDescription) {
  Assert(nCount > 0);
  if (nCount <= 0) return;
  ...
  if (g_ChangelistCreateUser.GetData().Count() == 0) {
    g_ErrorHandlerUser.SetErrorString(
        "Failed to create changelist for submit operation", E_FATAL);
    return;
  }
...
bool CP4::SubmitFiles(int nCount, const char **ppFullPathList,
                      const char *pDescription) {
  return PerformPerforceOp(SubmitPerforceOp, nCount, ppFullPathList,
                           pDescription);
}
...
void CP4::OpenFileInP4Win(const char *pFullPath) {
  ...
  if (GetClientSpecForFile(pFullPath, pClientSpec, sizeof(pClientSpec))) {
    V_snprintf(pSystem, sizeof(pSystem), "p4win -q -c %s -s %s", pClientSpec,
               pFullPath);
  } else {
...
bool CP4::IsFileInPerforce(const char *fullpath) {
  ...
  if (g_FileUser.GetData().Count() == 0) return false;
...
P4FileState_t CP4::GetFileState(const char *pFullPath) {
  if (!IsConnectedToServer()) return P4FILE_UNOPENED;

  CScopedFileClientSpec spec(pFullPath);

  g_FileUser.RetrieveOpenedFiles(pFullPath);
  if (g_FileUser.GetData().Count() == 0) return P4FILE_UNOPENED;
...
bool CP4::GetFileInfo(const char *pFullPath, P4File_t *pFileInfo) {
  ...
  if (g_FileUser.GetData().Count() != 1) return false;
...
bool CP4::IsConnectedToServer(bool bRetry) {
  if (bRetry && m_bConnectedToServer) {
    // don't comment this back in until it's called less often to avoid spamming
    // the server (currently, this is called every time the ifm file menu is
    // opened) this means that we won't detect losing the server until after a
    // failed Run() cmd -jd

    //		m_Client.Run( "monitor show" ); // do something to test if the
    //server's still up
    if (m_Client.Dropped()) {
      Shutdown();
    }
...
//-----------------------------------------------------------------------------
// retrieves the last error from the last op (which is likely to span multiple
// lines) this is only valid after OpenFile[s]For{Add,Edit,Delete} or
// {Submit,Revert}File[s]
//-----------------------------------------------------------------------------
const char *CP4::GetLastError() { return g_ErrorHandlerUser.GetErrorString(); }
