...
bool SaveResList(const CUtlRBTree<CUtlString, int> &list,
  ...
  if (fh != FILESYSTEM_INVALID_HANDLE) {
    for (int i = list.FirstInorder(); i != list.InvalidIndex();
      ...
      g_pFullFileSystem->Write(list[i].String(), Q_strlen(list[i].String()),
                               fh);
...
void MergeResLists(CUtlVector<CUtlString> &fileNames, char const *pchOutputFile,
                   char const *pchSearchPath) {
  CUtlRBTree<CUtlString, int> sorted(0, 0, ReslistLogLessFunc);
...
IResListGenerator *CreateReslistGenerator() { return new CResListGenerator(); }
