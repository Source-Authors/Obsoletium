...
int main(int argc, char **argv) {
  ...
  if (argc != 5) {
    fprintf(stderr, "format is 'rt_test src_image dest_image xsize ysize'\n");
    return 1;
  }
  ...
  // render a simple scene of a terrain, using a bitmap for color data and its
  // alpha channel for the height
    ...
    for (int x = 0; x < src_texture.Width - 1; x++) {
      ...
      for (int iy = 0; iy < 2; iy++)
        for (int ix = 0; ix < 2; ix++) {
          ...
          if ((x + ix == src_texture.Width - 1) ||
              (y + iy == src_texture.Height - 1))
            vecVerts[ix][iy].y = 0;
          else
      ...
      rt_Env.AddTriangle(id++, vecVerts[0][0], vecVerts[1][0], vecVerts[1][1],
                         vecColor);
      rt_Env.AddTriangle(id++, vecVerts[0][0], vecVerts[0][1], vecVerts[1][1],
                         vecColor);
    }

  rt_Env.AddTriangle(id++, Vector(0, 0, -.2), Vector(.2, 0, .2),
                     Vector(-.2, 0, .2), Vector(0, 0, 1));
  ...
  // lets render a frame
  auto *buf =
      reinterpret_cast<uint32 *>(MemAlloc_AllocAligned(xsize * ysize * 4, 16));
  ...
  for (int i = 0; i < 10; i++) {
    rt_Env.RenderScene(xsize, ysize, xsize, buf, EyePos,
                       Vector(-1, 0, 1) - EyePos, Vector(1, 0, 1) - EyePos,
                       Vector(-1, 0, -1) - EyePos, Vector(1, 0, -1) - EyePos);
  }
